...
function installDetachedDependencies(detachedAppRoot) {
  ...
  try {
    ...
    const output = `${error.stdout || ''}${error.stderr || ''}`.trim();
    fail(`immutable detached dependency installation failed: ${output.slice(-1200) || error.message}`);
  }
...
function withDetachedSubject(context, callback) {
  ...
  try {
    ...
    installDetachedDependencies(executionContext.appRoot);
