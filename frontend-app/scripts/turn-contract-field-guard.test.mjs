...
const terminalRuntimePath =
  "frontend-app/src/entities/client/model/helpers/assistantEventRuntime/turnLifecycle.js";
...
describe("turn contract production field guard", () => {
  it("resolves canonical schemas, production validators, consumers, and mapper fields", () => {
    expect(validateTurnContractFieldGuard({ repoRoot })).toEqual({
      schemaCount: 3,
      mapperCount: 1,
    });
  });
  ...
  it("fails when the terminal consumer-chain registry is missing a required downstream sink", () => {
    ...
    registry.jsTerminalChains = registry.jsTerminalChains.filter(
      (chain) => chain.name !== "terminal-public-error-clipboard-sink",
    );
    expect(() =>
      validateTurnContractFieldGuard({
        repoRoot,
        sourceOverrides: new Map([[registryPath, JSON.stringify(registry)]]),
      }),
    ).toThrow(
      "JS terminal chain registry missing=terminal-public-error-clipboard-sink",
    );
  });

  it("fails when a terminal consumer-chain registry symbol is stale", () => {
    const registry = JSON.parse(read(registryPath));
    const chain = registry.jsTerminalChains.find(
      (entry) => entry.name === "terminal-public-error-projection",
    );
  ...
  it("fails when the terminal notice implementation is renamed without its registry", () => {
    ...
    const mutated = source.replace(
      ...
      "function renamedTerminalNotice(terminal, deps) {",
    );
  ...
  it("fails when terminal projection deletes the public-error resolver", () => {
    ...
    const mutated = source.replace(
      ...
      "const publicError = terminal.publicError;",
    );
    expect(mutated).not.toBe(source);
    expect(() =>
      validateTurnContractFieldGuard({
      ...
      "terminal-public-error-projection missing call publicErrorForRemoteTerminal",
    );
  });
  ...
  it("fails when terminal projection restores the raw public-error sink", () => {
    ...
    expect(() =>
      ...
      "terminal-public-error-projection retains forbidden projection publicError=terminal.publicError",
    );
  });
  ...
  it("fails when terminal notice restores the raw public-error message sink", () => {
    const source = read(terminalRuntimePath);
    const mutated = source.replace(
      "publicErrorForRemoteTerminal(terminal.publicError).message",
      "(publicErrorForRemoteTerminal(terminal.publicError), terminal.publicError.message)",
    );
    expect(mutated).not.toBe(source);
    expect(() =>
      validateTurnContractFieldGuard({
      ...
      "terminal-public-error-notice retains forbidden member path terminal.publicError.message",
    );
  });
  ...
  it("fails when timeline rendering stops projecting publicError into its error boundary", () => {
    ...
    const mutated = source.replace(
      ...
      "const error = message.legacyPublicError;",
    );
    expect(mutated).not.toBe(source);
    expect(() =>
      validateTurnContractFieldGuard({
      ...
      "terminal-timeline-render missing member path message.publicError",
    );
  });
  ...
  it("fails when terminal clipboard restores the raw public-error input", () => {
    const source = read(timelineMessagePath);
    const mutated = source.replace(
      "terminalDiagnosticText(publicError)",
      "terminalDiagnosticText(error)",
    );
    expect(mutated).not.toBe(source);
    expect(() =>
      validateTurnContractFieldGuard({
      ...
      "terminal-public-error-clipboard-sink missing call argument terminalDiagnosticText=publicError",
    );
  });
  ...
  it("fails when terminal clipboard reads a raw diagnostic field", () => {
    ...
    expect(() =>
      ...
      "terminal-public-error-clipboard-sink retains forbidden member path error.diagnosticId",
    );
  });
  ...
  it("fails when terminal diagnostic sanitizer stops using the generated validator", () => {
    ...
    expect(() =>
      ...
      "frontend-app/src/shared/ui/publicError.js:safeRemoteDiagnosticId missing call validatePublicErrorV1",
    );
  });
  ...
  it("fails when the runtime terminal consumer stops calling its validator", () => {
    ...
    const mutated = source.replace(
      "validateTurnTerminalV2(payload)",
      "payload",
    );
    expect(mutated).not.toBe(source);
    expect(() =>
...
`;
    ...
    expect(() =>
      ...
      "validator namespace member validators.validateTurnTerminalV2 escapes direct calls",
    );
  });
  ...
  it.each([
    ...
    [
      "namespace optional member call",
      "validators?.validateTurnTerminalV2(payload)",
    ],
...
`;
  ...
  return importedTerminalValidator(payload);
    ...
    "ignores a shadowed named validator %s",
    (_label, importLine, declaration) => {
      const source = `${read(runtimePath)}\n${importLine}\n${declaration}\n`;
      expect(
        validateTurnContractFieldGuard({
          repoRoot,
...
`;
  ...
  it("does not let a shadowed validator call satisfy a registered consumer", () => {
    ...
    expect(() =>
      ...
      `${runtimePath}:parseRuntimeTurnTerminal missing call validateTurnTerminalV2`,
    );
  });
  ...
  it.each([
    ...
    "fails when a one-level barrel %s adds an unregistered consumer",
    (_label, importLine, call, symbol) => {
  ...
  it.each([
    ...
    "resolves a registered %s consumer by exact path, symbol, and call",
    (_label, importLine, call, symbol, barrel) => {
      const source = `${read(runtimePath)}\n${importLine}\nexport function ${symbol}(payload) { return ${call}; }\n`;
      const registry = JSON.parse(read(registryPath));
...
`;
  ...
  it.each([
    [
      "exported arrow",
      "export const registeredConsumer = (payload) => validateTurnTerminalV2(payload);",
      "registeredConsumer",
    ],
    [
      "object method",
      "export const registeredConsumers = { parse(payload) { return validateTurnTerminalV2(payload); } };",
      "registeredConsumers.parse",
    ],
  ])(
    "resolves a registered %s consumer by exact path, symbol, and call",
    (_label, declaration, symbol) => {
      const source = `${read(runtimePath)}\n${declaration}\n`;
      const registry = JSON.parse(read(registryPath));
      registry.schemas.TurnTerminalV2.jsConsumers.push({
        path: runtimePath,
  ...
  it("fails when a registry locator becomes stale", () => {
    ...
    const mutated = source.replace(
      '"symbol": "parseRuntimeTurnTerminal"',
      '"symbol": "missingRuntimeTurnTerminal"',
    );
    expect(mutated).not.toBe(source);
    expect(() =>
      validateTurnContractFieldGuard({
