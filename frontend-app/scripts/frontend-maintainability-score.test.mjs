...
describe('executable evidence registry', () => {
  ...
  it('requires non-zero mutation RED and immutable source binding for every T01 case', () => {
    ...
    const check = control.allOf[0];
    const context = inspectTargetRepository({ repoRoot: createStagedSubjectFrontendSnapshot(), requireClean: true });
    const mutationDocument = JSON.parse(readFileSync(join(scriptRoot, 'failure-matrix-mutations.json'), 'utf8'));
  ...
  it('accepts source-hashed T03 v2 evidence while retaining v1-only validation for other runners', () => {
    ...
    const t01Context = inspectTargetRepository({ repoRoot: createStagedSubjectFrontendSnapshot(), requireClean: true });
    const t01Options = { context: t01Context, control: t01Control, check: t01Check, startedAt: Date.now() - 100, finishedAt: Date.now() + 100 };
    const t01Report = failureMatrixEvidence(t01Context);
