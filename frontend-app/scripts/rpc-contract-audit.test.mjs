  ...
  return {
    ...
    handler: `
      ...
      export function attachActiveThreadRpcRuntime(runtime) {
        runtime.activeThreadRPC = async (action, rpc) => {
          ...
          if (notifyThreadActionFailure({ action, addWarning: runtime.addWarning, notifyAction: runtime.notifyAction, ${resultProperty}: result })) return false
          runtime.notifyAction('success', 'success')
          return true
        }
      }
    `,
    regression: `
      ...
      it('reports interrupt ok:false as warning without showing success', async () => {
        const runtime = { notifyAction: vi.fn(), addWarning: vi.fn() }
        ...
        ${regressionBetween}
        expect(runtime.notifyAction).toHaveBeenCalledWith('turn already completed', 'warning')
        expect(runtime.notifyAction).not.toHaveBeenCalledWith('success', 'success')
      })
    `,
  }
}
...
async function createRealResultHandledShadow(overrides = {}) {
  const synthetic = Object.keys(overrides).length > 0
  const sources = synthetic ? realResultHandledSources(overrides) : {
...
describe('rpc contract audit', () => {
  ...
  it('rejects a decoy runtime flow when the exact activeThreadRPC assignment passes response instead of result', async () => {
    const repoRoot = await createMutatedRealResultHandledShadow({
      runtime: (source) => source
        .replace(
          'notifyThreadActionFailure({ action, addWarning, notifyAction, result, threadId })',
          'notifyThreadActionFailure({ action, addWarning, notifyAction, response: result, threadId })',
        )
        .concat(`
  ...
  it('rejects a nested handler-call decoy inside the exact activeThreadRPC arrow', async () => {
    const repoRoot = await createMutatedRealResultHandledShadow({
      runtime: (source) => source
        .replace(
          'notifyThreadActionFailure({ action, addWarning, notifyAction, result, threadId })',
