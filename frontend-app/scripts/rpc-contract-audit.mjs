...
const RPC_RESPONSE_VALIDATORS_RUNTIME_PATH = 'frontend-app/src/shared/api/backendResponseValidatorsRuntime.js'
const RPC_MATRIX_PATH = 'frontend-app/src/shared/api/backendApi.contractMatrix.js'
const SIDEBAR_GO_STATE_PATH = 'internal/module/uistate/state.go'
...
export function formatRpcAuditReport(report) {
  return [
    ...
    `Sidebar required field drift: ${report.sidebarRequiredFieldFindings.length}`,
...
export function assertAuditPasses(report) {
  const findings = [
    ...
    report.sidebarRequiredFieldFindings,
...
export function collectSidebarRequiredFieldFindingsFromSources({
  ...
  const { consumerFields, registryUsedByRequiredCheck } = parseSidebarRuntimeRequiredFields(runtimeSource)
...
function parseRequiredGoStructJSONTags(source, symbol) {
  ...
  return uniqueSorted(fields.filter((field) => field.required).map((field) => field.name))
}
...
function parseSidebarRuntimeRequiredFields(source) {
  ...
  for (const statement of ast.program.body) {
    ...
    if (declaration?.type === 'VariableDeclaration' && declaration.kind === 'const') {
      for (const declarator of declaration.declarations) {
        if (declarator.id.type !== 'Identifier' || declarator.id.name !== 'SIDEBAR_REQUIRED_RESPONSE_KEYS') continue
        if (registry) {
  ...
  const consumerFields = registry.init.elements.map((element, index) => (
    stringLiteralValue(element, 'SIDEBAR_REQUIRED_RESPONSE_KEYS[' + index + ']')
  ))
  ...
  return {
    consumerFields: uniqueSorted(consumerFields),
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const failures = [
    ...
    ['Sidebar required field drift', report.sidebarRequiredFieldFindings],
