  ...
  for (const statement of consumer.body.body) {
    ...
    for (const item of statement.declarations) {
      if (
        ...
        && item.init.async
        && item.init.params.length === 2
        && item.init.params[0].type === 'Identifier' && item.init.params[0].name === 'action'
        && item.init.params[1].type === 'Identifier' && item.init.params[1].name === 'rpc'
      ) {
        runtimeBinding = item.id.name
        runtimeArrow = item.init
      }
    }
  }
  if (!runtimeArrow || runtimeBinding !== 'activeThreadRPC' || runtimeArrow.body.type !== 'BlockStatement') return false
  const exposed = consumer.body.body.some((statement) => {
    if (statement.type !== 'ExpressionStatement' || statement.expression.type !== 'CallExpression') return false
    const call = statement.expression
    if (
    ...
    const property = call.arguments[1].properties.find((item) => (
      item.type === 'ObjectProperty' && staticPropertyKeyName(item) === 'activeThreadRPC'
    ))
    return property?.value.type === 'Identifier' && property.value.name === runtimeBinding
  })
  if (!exposed) return false
  let matched = false
  walkAstWithAncestors(runtimeArrow.body, (node, ancestors) => {
    if (matched || node.type !== 'VariableDeclarator' || node.id.type !== 'Identifier') return
    if (ancestors.some((ancestor) => isFunctionNode(ancestor))) return
    if (
      node.init?.type !== 'AwaitExpression'
      || node.init.argument.type !== 'CallExpression'
      || node.init.argument.callee.type !== 'Identifier'
      || node.init.argument.callee.name !== 'rpc'
      || bindingShadowsNameAt(ancestors, 'rpc')
    ) return
    const resultName = node.id.name
    walkAstWithAncestors(runtimeArrow.body, (call, callAncestors) => {
      if (callAncestors.some((ancestor) => isFunctionNode(ancestor))) return
      if (
        matched || call.start <= node.end || call.type !== 'CallExpression'
        || call.callee.type !== 'Identifier' || call.callee.name !== handlerName
        || bindingShadowsNameAt(callAncestors, handlerName)
        || call.arguments.length !== 1 || call.arguments[0].type !== 'ObjectExpression'
      ) return
      const property = call.arguments[0].properties.find((item) => (
        item.type === 'ObjectProperty' && staticPropertyKeyName(item) === 'result'
      ))
      if (property?.value.type === 'Identifier' && property.value.name === resultName) matched = true
    })
  })
  return matched
}
