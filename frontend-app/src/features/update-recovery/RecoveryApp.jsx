...
const EMPTY_STATE = Object.freeze({ status: 'loading', value: null, error: '' });
...
function RecoveryApp({ client, confirmRestore = window.confirm }) {
  ...
  const runAction = useCallback(async (action) => {
    ...
    try {
      ...
      setView({ status: 'ready', value, error: '' });
    } catch (error) {
      setView((current) => ({
        ...current,
        status: 'error',
  ...
  return (
    <main className="recovery-shell">
      ...
      {view.error && <div className="recovery-error" role="alert">{view.error}</div>}
