...
openEdit: (item) => {
saveGenerationRef.current += 1; setForm(promptFormFromItem(item)); setEditorOpen(true); setNotice('');
},
savePrompt: () => runUIAction('prompt.save', savePrompt),
removePrompt: (item) => {
if (fallbackMode) { setNotice('当前为只读降级，暂不支持删除'); return; }
if (!item.id || actioning) return;
runUIAction('prompt.delete', () => removePromptItem({ cwd, item, refreshPromptSurface, setActioning, setNotice }));
},
copyPrompt: (item) => {
if (!item.id || actioning) return;
runUIAction('prompt.copy', () => copyPromptItem(
{ cwd, item, fallbackMode, setActioning, setNotice },
), { retryable: true });
},
setLaunchPrompt: (item) => {
if (!canForceLaunchPrompt(item) || actioning) return;
runUIAction('prompt.launch.set', () => setLaunchPreference(
{ cwd, item, queryClient, setActioning, setNotice },
));
},
clearLaunchPrompt: () => {
if (actioning) return;
runUIAction('prompt.launch.clear', () => clearLaunchPreference(
{ cwd, queryClient, setActioning, setNotice },
));
},
}; }
...
setWizardDraft(pendingDraftFromItem(item)); setWizardOpen(true); setNotice('');
},
discardDraft: (item) => {
const draftKey = item.draftKey || item.id;
if (!draftKey || actioning) return;
runUIAction('prompt.draft.discard', () => discardPromptDraftItem(
{ cwd, item, refreshPromptSurface, setActioning, setNotice },
));
},
handleWizardSaved: async () => {
setWizardOpen(false); setWizardDraft(null);
await refreshPromptSurface({ force: true });
...
}; const handleSaveProfile = () => runUIAction('prompt.profile.save', saveProfile); const handleImportMemory = () => { setters.setWizardDraft({ kind: 'recall', rawInput: '', scope: 'project' }); setters.setWizardOpen(true);
