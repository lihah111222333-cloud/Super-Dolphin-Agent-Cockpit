...
it('选中 Agent 消失后回退到根 Agent', async () => {
  const { rerender } = render(<TestChatPageWrapper store={boardStore()} projectPath="/repo/app" rightPanelOpen={true} />);
...
it('悬浮看板与收起按钮支持键盘聚焦与 aria 标注', () => {
  ...
  const collapse = screen.getByRole('button', { name: '收起 Agents 状态' });
  collapse.focus();
