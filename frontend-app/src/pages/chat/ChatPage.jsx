...
function RuntimeActivityPanel({
  ...
  useEffect(() => {
    runtimePopupOpenRef.current = Boolean(activeStat || activeWarning);
  }, [activeStat, activeWarning]);
...
function RuntimeStatList({ activeStat, onStatKeyDown, onToggleStat, statItems, tokenUsage }) {
  return (
    <ul className="runtime-icons" aria-label="工具调用统计">
      ...
      <li className="runtime-context" title={tokenUsage ? `上下文使用率 ${tokenUsage.usedPercent.toFixed(1)}%` : '等待后端同步上下文使用率'}>
...
function RuntimeStatItem({ activeStat, item, onKeyDown, onToggle }) {
  ...
  return (
    <li
      className="runtime-stat-listitem"
    >
      <button
        type="button"
