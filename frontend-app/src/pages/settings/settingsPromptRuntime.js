...
import { isCurrentPreferenceRequest } from "./settingsPageRuntime.js";
...
function usePromptSettingsLoadActions(input) {
  const {
    copy,
    cwd,
    nextPromptLoadRequest,
    nextVisibilityLoadRequest,
    setCurrentScopeCwd,
    setDefaultHint,
    setEffectiveHint,
    setHint,
  ...
  const loadScope = useCallback(
    () => loadPromptScope(setCurrentScopeCwd),
    [setCurrentScopeCwd],
  );
  const loadVisibility = useCallback(
    () =>
      loadInjectedPromptVisibility({
        copy,
        cwd,
        isCurrent: nextVisibilityLoadRequest(),
        setNotice,
        setShowInjected,
      }),
    [copy, cwd, nextVisibilityLoadRequest, setNotice, setShowInjected],
  );
  return { loadPrompt, loadScope, loadVisibility };
}

function usePromptSettingsMutationActions(input, loadVisibility) {
  const {
    copy,
    cwd,
    defaultHint,
    ...
    setSaving,
    setShowInjected,
    setShowInjectedSaving,
    setUsingDefault,
  } = input;
  const save = useCallback(
    () =>
      runUIAction("settings.prompt.save", () =>
        saveLspPromptHintState({
          copy,
          cwd,
          hint,
          saving,
  ...
  const reset = useCallback(
    () =>
      runUIAction("settings.prompt.reset", () =>
        saveLspPromptHintState({
          copy,
          cwd,
          hint: "",
      ...
      runUIAction(
        ...
        () =>
          copyEffectivePromptHint(
            promptDisplayHint(effectiveHint, defaultHint, copy),
            copy,
            setNotice,
          ),
        { retryable: true },
      ),
    [copy, defaultHint, effectiveHint, setNotice],
  );
  const toggleVisibility = useCallback(
    (event) =>
      runUIAction("settings.prompt.visibility.save", () =>
        saveInjectedPromptVisibility({
          copy,
          cwd,
          event,
          loadVisibility,
          saving: showInjectedSaving,
          setNotice,
          setSaving: setShowInjectedSaving,
          setShowInjected,
        }),
      ),
    [
  ...
  return { copyPrompt, reset, save, toggleVisibility };
}

function usePromptSettingsActions(input) {
  const loadActions = usePromptSettingsLoadActions(input);
  return {
    ...loadActions,
    ...usePromptSettingsMutationActions(input, loadActions.loadVisibility),
  };
}
...
function usePromptSettings(cwd, copy) {
  ...
  useEffect(() => {
    runBackgroundAction("settings.prompt.bootstrap", () =>
      Promise.all([
        loadLspPromptState({
          copy,
          cwd,
...
function promptDisplayHint(
  effectiveHint,
  defaultHint,
  copy = APP_COPY.zh.settings,
) {
...
function promptModeLabel(loading, usingDefault, copy = APP_COPY.zh.settings) {
  ...
  return usingDefault
    ? copy.promptCard.defaultMode
    : copy.promptCard.customMode;
}
...
async function loadLspPromptState(state) {
  ...
  try {
    const res = normalizePromptHintResponse(
      await readLspPromptHint({ cwd: state.cwd }),
    );
    ...
    if (isCurrentPreferenceRequest(state.isCurrent))
      state.setNotice({
        level: "error",
        message: state.copy.promptCard.loadFailed,
      });
    throw error;
  } finally {
    if (isCurrentPreferenceRequest(state.isCurrent)) state.setLoading(false);
...
async function loadInjectedPromptVisibility({
  ...
  try {
    ...
    if (isCurrentPreferenceRequest(isCurrent))
      setNotice({ level: "error", message: copy.promptCard.loadToggleFailed });
    throw error;
  }
}
...
async function saveLspPromptHintState(state) {
  ...
  try {
    const res = normalizePromptHintResponse(
      await writeLspPromptHint({ cwd: state.cwd, hint: state.hint }),
    );
    state.setEffectiveHint(res.hint);
    state.setDefaultHint(res.defaultHint);
    state.setHint(res.overrideHint);
    ...
    state.setNotice({
      level: "info",
      message: res.usingDefault
        ? state.copy.promptCard.restored
        : state.copy.promptCard.saved,
    });
  } catch (error) {
    state.setNotice({
      level: "error",
      message: state.copy.promptCard.saveFailed,
    });
    throw error;
  } finally {
...
async function copyEffectivePromptHint(text, copy, setNotice) {
  ...
  try {
    ...
    setNotice({
      level: ok ? "info" : "error",
      message: ok ? copy.promptCard.copied : copy.promptCard.copyFailed,
    });
  } catch (error) {
...
async function saveInjectedPromptVisibility(state) {
  ...
  try {
    ...
    setNotice({
      level: "info",
      message: next
        ? copy.promptCard.showInjectedSaved
        : copy.promptCard.hideInjectedSaved,
    });
  } catch (error) {
