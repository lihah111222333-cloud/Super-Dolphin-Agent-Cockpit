...
import {
  ProviderPropertiesCard,
  ProviderSettingsPanel,
} from "./components/ProviderSettingsPanels.jsx";
import { PromptSettingsCard } from "./components/PromptSettingsCard.jsx";
import {
  AboutPanel,
  RuntimeSettingsPanels,
} from "./components/SettingsSystemPanels.jsx";
...
import {
  appUpdateCurrentVersionLabel,
  useSettingsRuntime,
} from "./settingsRuntimeHook.js";
import "./SettingsPage.css";

const { getVideoApiKey, setVideoApiKey } = settingsPageService;

const selectSettingsProjectScope = (state) => ({
  activeProject: state.activeProject,
  cwd: state.cwd,
});

const selectSettingsLogs = (state) => ({
  logEntries: state.logEntries,
  logLevel: state.logLevel,
  setLogLevel: state.setLogLevel,
});

function SettingsLogCard({ copy }) {
  const { logEntries, logLevel, setLogLevel } = useClientStore(
    useShallow(selectSettingsLogs),
  );
  return (
    <UILogCard
      copy={copy}
      loadLogs={loadSettingsDashboardLogs}
...
function SettingsPage({
  ...
  const { activeProject, cwd: storeCwd } = useClientStore(
    useShallow(selectSettingsProjectScope),
  );
  const cwd =
    normalizeSettingsCwd(projectPath) ||
    normalizeSettingsCwd(activeProject) ||
    normalizeSettingsCwd(storeCwd);
  const runtime = useSettingsRuntime(cwd, copy);
  const provider = useProviderPreferences(
    cwd,
    runtime.form.activeProvider,
    copy,
  );
...
function MobileAccountPanel({ copy = APP_COPY.zh.settings, cwd, runtime }) {
  ...
  const provider =
    PROVIDER_LABELS[runtime.form.activeProvider] ||
    runtime.form.activeProvider ||
    "Codex";
  return (
    <section
      className="settings-mobile-account"
      data-testid="settings-mobile-account"
      aria-label={copy.mobileAccount}
    >
      <header>
        <button type="button" aria-label={copy.menu} disabled>
          <Menu size={18} />
        </button>
        <h2>{APP_BRAND_NAME}</h2>
        <div className="settings-mobile-avatar" aria-label={copy.avatar}>
          SY
        </div>
      </header>
      ...
      <div className="settings-mobile-card is-disabled">
        ...
        <button
          type="button"
          data-testid="settings-mobile-logout-button"
          disabled
        >
          {copy.logoutTab}
        </button>
    ...
    <section className="settings-page" data-testid="settings-page">
      <PageHeader
        ...
        actions={
          <button
            ...
            type="button"
            data-testid="settings-refresh-build-button"
            onClick={() => void runtime.refreshBuildInfo()}
          >
            {copy.refreshBuildInfo}
          </button>
        }
      />
      ...
      <div className="panel-body" data-testid="settings-panel-body">
        <AboutPanel
          ...
          cwd={cwd}
          runtime={runtime}
          updateCurrentVersion={appUpdateCurrentVersionLabel(runtime.buildInfo)}
        />
        <RuntimeSettingsPanels copy={copy} runtime={runtime} />
        <ProviderSettingsPanel
          copy={copy}
          runtime={runtime}
          viewConfig={providerSettingsViewConfig}
        />
        ...
        <VideoSettingsCard
          copy={copy}
          getApiKey={getVideoApiKey}
          setApiKey={setVideoApiKey}
        />
        <SettingsLogCard copy={copy} />
      </div>
    </section>
  );
}
...
function SettingsNotices({ error, status }) {
  return (
    <>
      {status ? (
        <output className="settings-page-notice settings-status">
          {status}
        </output>
      ) : null}
      {error ? (
        <p className="settings-page-notice danger-text" role="alert">
          {error}
        </p>
      ) : null}
    </>
