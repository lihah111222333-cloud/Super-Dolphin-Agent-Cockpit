...
import { PROVIDER_LABELS } from "./settingsProviderConfig.js";
import {
  runBackgroundAction,
  runUIAction,
...
function useBuiltinToolsSettings(cwd, copy) {
  ...
  const applyPayload = useCallback(
    (payload) => setTools(normalizeBuiltinTools(payload)),
    [],
  );
  const load = useCallback(
    () =>
      runUIAction(
        "settings.builtin.load",
        () =>
          loadBuiltinTools({
            applyPayload,
            copy,
            cwd,
            isCurrent: nextLoadRequest(),
            setLoading,
            setNotice,
          }),
        { retryable: true },
      ),
    [applyPayload, copy, cwd, nextLoadRequest],
  );
  const toggleTool = useCallback(
    (tool) =>
      runUIAction("settings.builtin.toggle", () =>
        toggleBuiltinTool({
          applyPayload,
          copy,
          cwd,
    ...
    [applyPayload, copy, cwd, savingIds],
  );
  const toggleGroup = useCallback(
    (key) => setExpandedGroups((prev) => ({ ...prev, [key]: !prev[key] })),
    [],
  );
  const groups = useMemo(
    () => builtinToolGroups(tools, builtinsCopy),
    [builtinsCopy, tools],
  );
  useEffect(() => {
    runBackgroundAction("settings.builtin.bootstrap", () =>
      loadBuiltinTools({
        applyPayload,
        copy,
        cwd,
  ...
  return {
    expandedGroups,
    filteredCount: tools.filter((tool) => tool.replacedBy || !tool.enabled)
      .length,
    groups,
...
async function loadBuiltinTools(state) {
  ...
  try {
    ...
    if (isCurrentPreferenceRequest(isCurrent))
      setNotice({ level: "error", message: copy.builtins.loadFailed });
    throw error;
  } finally {
    if (isCurrentPreferenceRequest(isCurrent)) setLoading(false);
  }
...
async function toggleBuiltinTool(state) {
  ...
  setTools((prev) =>
    prev.map((item) =>
      item.id === tool.id ? { ...item, enabled: nextEnabled } : item,
    ),
  );
  try {
    applyPayload(
      await writeBuiltinTool({ cwd, id: tool.id, enabled: nextEnabled }),
    );
    setNotice({
      level: "info",
      message:
        ...
        " " +
        (nextEnabled
          ? copy.builtins.enabledSuffix
          : copy.builtins.disabledSuffix),
    });
  } catch (error) {
    setTools((prev) =>
      prev.map((item) =>
...
function builtinToolGroups(tools, copy) {
  ...
  return [
    builtinToolGroup(
      "native-hard",
      copy.nativeHard,
      disabled.filter((tool) => builtinToolEnforcement(tool) === "native-hard"),
      copy.nativeHardNote,
      copy,
    ),
    builtinToolGroup(
      "effect-hard",
      copy.effectHard,
      disabled.filter((tool) => builtinToolEnforcement(tool) === "effect-hard"),
      copy.effectHardNote,
      copy,
    ),
    builtinToolGroup(
      "soft-audit",
      copy.softAudit,
      disabled.filter((tool) => builtinToolEnforcement(tool) === "soft-audit"),
      copy.softAuditNote,
      copy,
    ),
...
function builtinToolGroup(key, label, tools, note, copy) {
  ...
  return {
    ...
    key,
    label: label + copy.countOpen + tools.length + copy.countClose,
    note,
    tools,
  };
}
...
function builtinUnfilteredGroup(tools, copy) {
  ...
  return {
    ...
    key: "unfiltered",
    label:
      copy.unfiltered + copy.countOpen + available.length + copy.countClose,
    tools: available,
  };
}
...
function toolMetaText(tool, copy) {
  ...
  const provider = firstPresentText(
    PROVIDER_LABELS[tool.provider],
    tool.provider,
  );
...
function groupSummary(group, copy) {
  if (group.key === "unfiltered")
    return copy.availableCount.replace("{count}", group.tools.length);
