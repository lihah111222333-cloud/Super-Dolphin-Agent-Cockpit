...
function UILogLevelRow({ logsCopy, store }) {
  return (
    <div className="settings-stall-row settings-log-control-row">
      ...
      <select id="settings-log-level-select" className="settings-stall-input settings-log-level-select" data-testid="settings-log-level-select" value={store.logLevel} onChange={(event) => runUIAction('settings.logs.level', () => store.setLogLevel(event.target.value))}>
