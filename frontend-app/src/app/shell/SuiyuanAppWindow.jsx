...
function AppUpdateBanner({ copy = APP_COPY.zh.update, updateBanner }) {
	...
	const version = firstPresentText(updateBanner.update.version, updateBanner.update.artifact?.version);
...
function SuiyuanNavButton({ activePage, copy, item, memoryBadgeCount, setActivePage }) {
	...
	return (
		<button type="button" className={`suiyuan-nav-item${active ? ' active' : ''}`} onClick={() => setActivePage(item.id)} aria-label={label} aria-current={active ? 'page' : undefined}>
...
function SuiyuanChatNavGroup({ copy, item, projectPath, sidebar, store }) {
	...
	return (
		<div className="suiyuan-chat-nav-group">
			<SuiyuanNavButton activePage={activePage} copy={copy} item={item} memoryBadgeCount={0} setActivePage={setActivePage} />
			{activePage === 'chat' ? <div className="suiyuan-chat-project-tree"><ChatSidebarProjectTree copy={copy.workbench} projectPath={projectPath} setActivePage={setActivePage} store={store} /></div> : null}
		</div>
	);
}
...
function SuiyuanSidebar({ copy, projectPath, sidebar, store }) {
	...
	return (
		<aside id="app-sidebar" className={`app-sidebar suiyuan-sidebar${isOpen ? ' is-open' : ''}`} data-testid="app-sidebar" aria-label={copy.workbench.ariaLabel}>
			<div className="suiyuan-brand-block">
				<span className="suiyuan-brand-light-mark" data-testid="suiyuan-brand-light-logo" aria-hidden="true"><Sailboat size={14} strokeWidth={2} /></span>
				<img className="suiyuan-brand-dark-mark" data-testid="suiyuan-brand-dark-logo" src={suiyuanBrandIcon} alt="" aria-hidden="true" />
				<div className="suiyuan-brand-meta"><strong>{APP_BRAND_NAME}</strong><span>AI Desktop</span></div>
				<button type="button" className="suiyuan-sidebar-collapse" aria-label={copy.workbench.collapse} title={copy.workbench.collapse} aria-controls="app-sidebar" onClick={closeSidebar}><PanelLeftClose size={17} aria-hidden="true" /></button>
			</div>
			<button type="button" className="suiyuan-new-chat" aria-label={copy.workbench.newChat} onClick={startNewChat}><Plus size={18} aria-hidden="true" /><span>{copy.workbench.newChat}</span></button>
			<nav className="suiyuan-nav" data-testid="sidebar-nav" aria-label="Suiyuan navigation">
				<SuiyuanChatNavGroup copy={copy} item={SUIYUAN_NAV_ITEMS[0]} projectPath={projectPath} sidebar={sidebar} store={store} />
				{SUIYUAN_NAV_ITEMS.slice(1).map((item) => <SuiyuanNavButton key={item.id} activePage={activePage} copy={copy} item={item} memoryBadgeCount={memoryBadgeCount} setActivePage={setActivePage} />)}
			</nav>
			<div className="suiyuan-sidebar-footer">
				<button type="button" className={`suiyuan-footer-item${activePage === 'settings' ? ' active' : ''}`} aria-label={copy.workbench.settings} onClick={() => setActivePage('settings')}><SettingsIcon size={15} aria-hidden="true" /><span>{copy.workbench.settings}</span></button>
				<a href="https://github.com/anthropic-ai/super-agent-v3" target="_blank" rel="noopener noreferrer" className="suiyuan-footer-item"><HelpCircle size={15} aria-hidden="true" /><span>{copy.workbench.help || 'Help'}</span></a>
			</div>
		</aside>
	);
}
	...
	return (
		<nav className="suiyuan-mobile-nav" data-testid="mobile-nav" aria-label={copy.workbench.mobileNavAriaLabel}>
			{SUIYUAN_MOBILE_NAV_ITEMS.map((item) => {
				...
				return <button key={item.id} type="button" className={`suiyuan-mobile-nav-item${active ? ' active' : ''}`} onClick={() => setActivePage(item.id)} aria-label={label} aria-current={active ? 'page' : undefined}><Icon size={20} aria-hidden="true" /><span>{label}</span></button>;
			})}
		</nav>
	);
}
...
function useBoundAppCommandRuntime(options) {
	...
	return useMemo(() => (overrides ? createAppCommandRuntime({
		...
		bindings: {
			[APP_COMMAND_IDS.PALETTE_OPEN]: { run: () => setPaletteOpen(true) },
			[APP_COMMAND_IDS.CHAT_NEW]: { run: startNewChat },
			[APP_COMMAND_IDS.SETTINGS_OPEN]: { run: () => setActivePage('settings') },
			[APP_COMMAND_IDS.SIDEBAR_TOGGLE]: { run: () => setSidebarOpen((open) => !open) },
...
function AppCommandPalette({ copy, onClose, open, runtime }) {
	...
	return <CommandPalette open={open} commands={runtime.commands} execute={runtime.execute} onClose={onClose} copy={copy} />;
}

export function SuiyuanAppWindow({ language, shell, shellLayoutStore, shortcutController, getShortcutPlatform, store }) {
	const { memoryBadge, projectPath, rightPanelOpen, setRightPanelOpen, theme, toggleTheme, updateBanner } = shell;
	...
	if (currentPageState.activePage !== store.activePage) setCurrentPageState({ activePage: store.activePage, workflowView: 'automation' });
	...
	const currentPageLabel = currentPageLabelOverride || appPageTitleLabel(store.activePage, copy);
	...
	const setActivePageFromSidebar = useCallback((page) => {
		...
		const isTest = typeof globalThis !== 'undefined' && globalThis.process?.env?.NODE_ENV === 'test';
		if (isTest || (typeof window !== 'undefined' && window.innerWidth <= 920)) setSidebarOpen(false);
	}, [store]);
	const handleWorkflowViewChange = useCallback((workflowView) => setCurrentPageState({ activePage: 'workflows', workflowView: textValue(workflowView) || 'automation' }), []);
	...
	return (
		<div className={`sa-window suiyuan-shell${sidebarOpen ? ' sidebar-open' : ' sidebar-collapsed'}`} data-command-palette-open={paletteOpen} data-brand="suiyuan" data-theme={theme} data-testid="frontend-app">
			{shortcutController?.status === 'error' ? <output role="alert" data-testid="shortcut-config-error">{shortcutController.error}</output> : null}
			<button type="button" className="workbench-toggle" aria-label={sidebarOpen ? copy.workbench.close : copy.workbench.open} aria-controls="app-sidebar" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen((open) => !open)}><SidebarToggleIcon size={22} aria-hidden="true" /></button>
			...
			<div className="sa-body suiyuan-shell-body">
				{!sidebarOpen ? <button type="button" className="sidebar-expand-trigger" aria-label={copy.workbench.expand} title={copy.workbench.expand} onClick={() => setSidebarOpen(true)}><PanelLeftOpen size={20} aria-hidden="true" /></button> : null}
				<SuiyuanSidebar copy={copy} projectPath={projectPath} sidebar={{ activePage: store.activePage, closeSidebar, isOpen: sidebarOpen, memorySimilarCount: memoryBadge.memorySimilarCount, setActivePage: setActivePageFromSidebar, startNewChat }} store={store} />
				<main className="sa-main suiyuan-main">
					<header className="suiyuan-top-appbar" aria-label="Suiyuan app bar">
						<div className="suiyuan-appbar-title"><span>{copy.currentPagePrefix}</span><h1>{currentPageLabel}</h1></div>
						<div className="suiyuan-appbar-actions" aria-label="Workspace actions">
							<button type="button" className="suiyuan-icon-action" aria-label={copy.workbench.notifications} title={copy.workbench.notifications} onClick={() => setActivePageFromSidebar('observability')}><Bell size={15} aria-hidden="true" /></button>
							<button type="button" className="suiyuan-icon-action" aria-label={copy.workbench.history} title={copy.workbench.history} onClick={() => setActivePageFromSidebar('chat')}><Clock3 size={15} aria-hidden="true" /></button>
							<button type="button" className="suiyuan-icon-action" aria-label={`${copy.workbench.switchThemePrefix}${themeLabel}`} title={themeLabel} onClick={toggleTheme}><ThemeIcon size={15} aria-hidden="true" /></button>
							<button type="button" className="suiyuan-locale-action" aria-label={copy.switchLanguage} title={copy.switchLanguage} onClick={toggleLocale}>{locale.toUpperCase()}</button>
						</div>
					</header>
					<AppUpdateBanner copy={copy.update} updateBanner={updateBanner} />
					<div className="suiyuan-main-canvas"><Suspense fallback={<PageLoadingFallback />}><ActivePageContent activePage={store.activePage} copy={copy} store={store} projectPath={projectPath} memoryRevision={memoryBadge.memoryRevision} setMemoryPageSimilarCount={memoryBadge.setMemoryPageSimilarCount} onWorkflowViewChange={handleWorkflowViewChange} rightPanelOpen={rightPanelOpen} shellLayoutStore={shellLayoutStore} shortcutController={shortcutController} setRightPanelOpen={setRightPanelOpen} /></Suspense></div>
				</main>
			</div>
