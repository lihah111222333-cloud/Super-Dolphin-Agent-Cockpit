...
const runtime = vi.hoisted(() => ({
  ...
  backend: {
    ...
    onBridgeEvent: vi.fn((callback) => {
      ...
      return () => {
        if (runtime.bridgeCallback === callback) runtime.bridgeCallback = null;
      };
    }),
...
it('notifies once when an error patch arrives before duplicate agent failure events', async () => {
  resetClientStoreForTests({
    ...
    threads: [{ id: 'thread-1', agentId: 'agent-1', name: 'Existing', provider: 'codex', status: 'error' }],
    statuses: { 'thread-1': { status: 'error' } },
    activeTurnByThread: {},
  });
  registerBridgeEventHandlersForTest();
  runtime.backend.getSidebarState.mockClear();
  const event = {
    type: 'agent/failed',
    payload: {
      threadId: 'thread-1',
  ...
  expect(runtime.backend.getSidebarState).not.toHaveBeenCalled();
  expect(state.actionNotice).toEqual(expect.objectContaining({ message: '代理运行失败', tone: 'error' }));
  expect(state.warningEntries).toEqual([expect.objectContaining({
  ...
  expect(JSON.stringify({
    ...
    traces: runtime.backend.emitFrontendTraceEvent.mock.calls,
...
it('routes malformed bridge event parse failures into visible warnings', () => {
  resetClientStoreForTests({ cwd: '/repo/app', activeProject: '/repo/app', activeThreadId: 'thread-1' });
  registerBridgeEventHandlersForTest();
  ...
  expect(useClientStore.getState().warningEntries).toEqual([expect.objectContaining({
    level: 'error',
    event: 'bridge.event.parse_failed',
    fields: expect.objectContaining({ eventName: 'bridge-event', error: '[redacted]', rawLen: 10 }),
  })]);
...
it('routes bridge events without a method into visible warnings', () => {
  ...
  expect(useClientStore.getState().warningEntries).toEqual([expect.objectContaining({
    ...
    fields: expect.objectContaining({ payloadKeys: ['source', 'rawPreview'] }),
  })]);
