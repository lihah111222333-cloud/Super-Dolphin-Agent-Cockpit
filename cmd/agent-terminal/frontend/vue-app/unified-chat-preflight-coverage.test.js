...
function makeStores(opts = {}) {
  ...
  const projectStore = { state: reactive({ active: opts.active ?? '.', showModal: false, projects: opts.projects ?? ['.'], features: opts.features ?? {} }), projectOptions: { value: [] }, setActive: vi.fn(), quickAdd: vi.fn(), removeProject: vi.fn() };
...
beforeEach(() => {
  ...
  skill.reset.mockClear(); skill.resolve.mockReset().mockResolvedValue({ selectedSkills: [], manualSkillSelection: false }); composer.state.text = ''; composer.state.attachments = []; composer.clearComposer.mockClear(); composer.attachByPaths.mockClear();
...
describe('UnifiedChatPage preflight coverage', () => {
  ...
  it('covers send early return, selected thread reuse and thread bootstrap', async () => {
    ...
    composer.state.text = 'boot'; composer.state.attachments = [{ name: 'a.txt' }]; const { vm: bootVm, store: bootStore } = await createVm({ selectedId: '', active: '/repo', features: { launchSkillSelection: true } }); bootVm.toggleLaunchSkill('LaunchSkill'); await bootVm.send(); expect(bootStore.startThread).toHaveBeenCalledWith('/repo', { focusMode: 'chat', selectedSkills: ['LaunchSkill'], manualSkillSelection: true }); expect(bootStore.sendMessage).toHaveBeenCalledWith('thread-started', 'boot', [{ name: 'a.txt' }], { selectedSkills: ['LaunchSkill'], manualSkillSelection: true, cwd: '/repo' });
  });
