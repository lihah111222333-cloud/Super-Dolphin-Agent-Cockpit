...
describe('UnifiedChatPage preflight coverage', () => {
  ...
  it('covers send early return, selected thread reuse and thread bootstrap', async () => {
    ...
    composer.state.text = 'boot'; composer.state.attachments = [{ name: 'a.txt' }]; skill.resolve.mockResolvedValueOnce({ selectedSkills: [], manualSkillSelection: false }); const { vm: bootVm, store: bootStore } = await createVm({ selectedId: '', active: '/repo' }); await bootVm.send(); expect(bootStore.startThread).toHaveBeenCalledWith('/repo', { focusMode: 'chat' }); expect(bootStore.sendMessage).toHaveBeenCalledWith('thread-started', 'boot', [{ name: 'a.txt' }], { selectedSkills: [], manualSkillSelection: false, cwd: '/repo' });
  });
