...
type ProviderPreferenceEntry = { suffix: string; apply: (value: string) => void };
...
function setupProviderSettings(props: ProviderSettingsProps) {
  ...
  const summaryMode = ref('detailed') as { value: string };
  const approvalMode = ref('on-request') as { value: string };
  const effortMode = ref('xhigh') as { value: string };
  const providerModel = ref('gpt-5.4') as { value: string };
  const personality = ref('pragmatic') as { value: string };
  const { activeProjectCwd, withProjectCwd } = useSettingsScope(props.projectStore);

  function normalizeProviderID(value: unknown): string {
    const providerID = (value || '').toString().trim();
  ...
  async function loadProviderSettings(): Promise<void> {
    ...
    try {
      const raw = await readProviderPreference('sandbox', providerID);
      if (raw && typeof raw === 'string') applySandboxPayload(JSON.parse(raw));
      else if (raw && typeof raw === 'object') applySandboxPayload(raw);
    } catch {
      // ignore
    }
    const prefEntries: ProviderPreferenceEntry[] = [
      { suffix: 'summary', apply: (value) => { summaryMode.value = value; } },
      { suffix: 'approvalPolicy', apply: (value) => { approvalMode.value = value; } },
      { suffix: 'effort', apply: (value) => { effortMode.value = value; } },
      { suffix: 'model', apply: (value) => { providerModel.value = value; } },
      { suffix: 'personality', apply: (value) => { personality.value = value; } },
    ];
    const results = await Promise.allSettled(prefEntries.map((entry) => readProviderPreference(entry.suffix, providerID)));
    results.forEach((result, index) => {
      if (result.status === 'fulfilled' && result.value && typeof result.value === 'string') {
        prefEntries[index].apply(result.value);
      }
    });
  }
  ...
  async function saveProviderSettings(): Promise<void> {
    ...
    try {
      const providerID = normalizeProviderID(activeProvider.value);
  ...
  template: `
    ...
    <div class="data-card-vue" data-testid="settings-provider-sandbox-card">
      ...
      <div class="settings-stall-row" style="margin-top:8px; margin-bottom:12px">
        <select v-model="activeProvider" class="settings-stall-input" data-testid="settings-provider-active-select" style="width:220px" @change="loadProviderSettings">
      ...
      <div class="settings-stall-row" style="margin-top:8px">
        ...
        <select v-model="effortMode" class="settings-stall-input" data-testid="provider-effort-mode-select" style="width:260px">
          <option v-for="m in EFFORT_MODES" :key="m.value" :value="m.value">{{ m.label }}</option>
        </select>
      </div>
      <div class="settings-stall-row" style="margin-top:8px">
