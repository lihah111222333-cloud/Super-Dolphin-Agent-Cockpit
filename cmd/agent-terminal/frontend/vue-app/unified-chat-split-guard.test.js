...
describe('UnifiedChatPage split guard coverage', () => {
  ...
  it('covers format, status, cards and plan derived behavior', async () => {
    ...
    await vi.advanceTimersByTimeAsync(65000); expect(vm.activeStatusHeader.value).toBe('处理中'); expect(vm.displayStatusText.value).toBe('处理中'); expect(vm.activeStatusMeta.value).toContain('1m 05s'); expect(vm.activeStatusMeta.value).toContain('Esc 可中断');
  ...
  it('covers public action methods', async () => {
    ...
    const { vm, store, current } = await createVm({ selectedId: '', active: '/repo', status: { 'thread-started': 'running' }, interruptible: { 'thread-active': true, 'thread-started': true } });
    await vm.send(); expect(store.startThread).toHaveBeenCalledWith('/repo', { focusMode: 'chat' }); expect(store.sendMessage).toHaveBeenCalledWith('thread-started', 'hello', [{ name: 'a.txt' }], { selectedSkills: ['skillA'], manualSkillSelection: true, cwd: '/repo' }); expect(composer.clearComposer).toHaveBeenCalled(); expect(skill.reset).toHaveBeenCalled(); expect(autoScroll.schedule).toHaveBeenCalled();
    await vm.launchOne(); expect(current.value).toBe('thread-started'); const confirm = vi.fn(); await vm.interruptCurrent({ threadId: 'thread-started', confirm }); expect(confirm).toHaveBeenCalled(); current.value = 'thread-active'; await vm.compactCurrent(); await vm.forceCompleteCurrent(); await vm.recoverSelected(); vm.stopSelected();
    expect(store.compactThread).toHaveBeenCalledWith('thread-active'); expect(store.forceCompleteThread).toHaveBeenCalledWith('thread-active'); expect(store.recoverThread).toHaveBeenCalledWith('thread-active'); expect(store.stopThread).toHaveBeenCalledWith('thread-active', { source: 'ui_stop' });
