...
export function useAutoScroll(workspaceRef) {
  ...
  let userDraggingScrollbar = false, dragEndTimer = 0;
  ...
  function handleCollapsedContainer(el, retryCount, tryRestore) {
    ...
    logWarn('scroll', 'scroll.mutation.rebuild_timeout', {
      retryCount, scrollHeight: el.scrollHeight,
      clientHeight: el.clientHeight, savedScrollTop,
    });
  }

  function performMutationRestore(tryRestore, retryState) {
    mutationRestoreRaf = 0;
    if (programmaticScrollInProgress || userDraggingScrollbar) return;
  ...
  function scheduleScrollToBottom(force = false) {
    ...
    logDebug('scroll', 'scroll.to_bottom.called', { force, shouldAutoScroll: shouldAutoScroll.value, isAtBottom: isAtBottom.value, userDragging: userDraggingScrollbar });
    if (userDraggingScrollbar && !force) return;
  ...
  function restoreScrollPosition() {
    // 用户正在拖动滚动条时，跳过所有自动恢复
    if (userDraggingScrollbar) return;
    // 同步尝试恢复（如果 DOM 已就绪）
    const el = resolveChatScroller();
    if (el) {
      // 临时移除高度锁进行真实测量
  ...
  function onScrollbarMouseDown(e) {
    // 判断 mousedown 是否落在滚动条区域：offsetX 超出 el.clientWidth
    if (e.offsetX > e.currentTarget.clientWidth) {
      userDraggingScrollbar = true;
  ...
  function onScrollbarMouseUp() {
    if (!userDraggingScrollbar) return;
    // 延迟清除标记，给 onChatScroll 时间处理最终位置
    if (dragEndTimer) clearTimeout(dragEndTimer);
    dragEndTimer = setTimeout(() => {
      userDraggingScrollbar = false;
      dragEndTimer = 0;
      // 同步最终滚动位置到保存状态
      const el = resolveChatScroller();
      if (el) {
        savedScrollTop = el.scrollTop;
        savedDistFromBottom = el.scrollHeight - el.scrollTop - el.clientHeight;
        // 如果拖动结束后没有触底，则判定为意图保留在上方，取消自动滚动
        if (!isNearBottom(el) && shouldAutoScroll.value) {
          logDebug('scroll', 'scroll.user.drag_up_end', { savedScrollTop, savedDistFromBottom });
          shouldAutoScroll.value = false;
        }
      }
    }, 100);
  ...
  function onUserWheel(e) {
    if (e.deltaY < 0 && shouldAutoScroll.value) {
  ...
  function onUserKeyDown(e) {
    if (['ArrowUp', 'PageUp', 'Home'].includes(e.key) && shouldAutoScroll.value) {
  ...
  function attachScrollListener() {
    ...
    el.addEventListener('mousedown', onScrollbarMouseDown);
    el.addEventListener('wheel', onUserWheel, { passive: true });
    el.addEventListener('keydown', onUserKeyDown, { passive: true });
    if (typeof window !== 'undefined') window.addEventListener('mouseup', onScrollbarMouseUp);
    ...
    scrollListenerCleanup = () => {
      ...
      el.removeEventListener('mousedown', onScrollbarMouseDown);
      el.removeEventListener('wheel', onUserWheel);
      el.removeEventListener('keydown', onUserKeyDown);
      if (typeof window !== 'undefined') window.removeEventListener('mouseup', onScrollbarMouseUp);
  ...
  return { shouldAutoScroll, isAtBottom, scheduleScrollToBottom, scrollToTop, resetScrollState, saveScrollPosition, restoreScrollPosition, resolveChatScroller, /** @internal test-only */ _setUserDragging: (v) => { userDraggingScrollbar = v; } };
}
