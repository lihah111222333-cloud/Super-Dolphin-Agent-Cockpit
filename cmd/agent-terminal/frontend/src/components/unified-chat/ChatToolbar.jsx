...
export function ChatToolbar({
  ...
  const providerToggleLabel = useMemo(() => {
    return resolveProviderToggleLabel(useClaudeProvider);
  }, [useClaudeProvider]);

  const launchAgentLabel = '新对话';
  const launchAgentTitle = '新对话：发送第一条消息时才会创建会话';
  let recoverAgentLabel = '请先选择会话';
  let recoverAgentTitle = '请先选择会话';
  if (selectedThreadId) {
    recoverAgentLabel = '进程恢复';
    recoverAgentTitle = '手动杀进程并恢复连接';
  }
  if (recoveringSelected) {
    recoverAgentLabel = '恢复中';
    recoverAgentTitle = '进程恢复中…';
  }
    ...
    <div className="chat-toolbar unified-toolbar" style={{ position: 'relative' }} data-testid="chat-toolbar">
      ...
      {!isCmd && (
        <button
          ...
          aria-label={recoverAgentLabel}
          title={recoverAgentTitle}
...
ChatToolbar.template = `
  <div data-testid="chat-toolbar">
    <div data-testid="provider-toggle">
      <input class="provider-toggle-input" />
    </div>
