...
export const DagsPage = {
  ...
  setup(props, ctx) {
    const localUid = Math.random().toString(36).slice(2, 6);
    const emit = ctx?.emit || (() => {});
    ...
    watch(
      ...
      (newVal) => {
        console.log('[VM ' + localUid + '] activeCategory watch triggered, newVal:', newVal, 'rowsLength:', rows.value.length);
        if (!rows.value.length) { activeCategory.value = ''; categoryManuallySelected.value = false; return; }
        ...
        if (!activeTab || (!categoryManuallySelected.value && activeTab.count === 0)) {
          const next = firstAvailableCategory(categoryTabs.value);
          console.log('[VM ' + localUid + '] activeCategory next value decided:', next);
            ...
            <div v-else class="dag-console-list">
              ...
              >
                <span class="dag-console-row-main">
                  ...
                  <span class="dag-console-key">{{ row.key === '-' ? '' : row.key }}</span>
                </span>
