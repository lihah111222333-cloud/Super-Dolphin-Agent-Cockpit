...
const isSessionNotAvailableError = (err) => {
...
export const perfNow = () => typeof performance !== 'undefined' && typeof performance.now === 'function' ? performance.now() : Date.now();

export const waitMs = (ms) => new Promise((resolve) => globalThis.setTimeout(resolve, Math.max(0, Number(ms) || 0)));

const providerPreferenceScope = (p) => {
  const v = (p || '').toString().trim();
  if (!v || v === 'codex') return v;
  if (v === 'claude' || v.startsWith('claude-')) return 'claude';
  return v;
};

const normalizeCodexIdentityValue = (v, f) => typeof v === 'boolean' ? f : (normalizeProviderConfigValue(v) || f);

const buildCodexIdentityConfig = (h, i, m) => ({
  codexHome: normalizeCodexIdentityValue(h, CODEX_IDENTITY_DEFAULTS.codexHome),
  codexInstanceKey: normalizeCodexIdentityValue(i, CODEX_IDENTITY_DEFAULTS.codexInstanceKey),
  codexModelProvider: normalizeCodexIdentityValue(m, CODEX_IDENTITY_DEFAULTS.codexModelProvider),
});
...
export function saveActiveThread(ctx, id) {
  const { logInfo } = ctx; const next = id || '';
  if (ctx.state.activeThreadId === next) return;
  const prev = ctx.state.activeThreadId || ''; ctx.state.activeThreadId = next;
...
export function saveActiveCmdThread(ctx, id) {
  const { logInfo } = ctx; const next = id || ''; if (ctx.state.activeCmdThreadId === next) return;
  const prev = ctx.state.activeCmdThreadId || ''; ctx.state.activeCmdThreadId = next;
...
export async function renameThread(ctx, threadId, name) {
  ...
  if (ctx.state.agentMetaById && typeof ctx.state.agentMetaById === 'object') {
    const prevMeta = ctx.state.agentMetaById[id] || {};
    ctx.state.agentMetaById = {
      ...ctx.state.agentMetaById,
      [id]: {
        ...prevMeta,
        alias: nextName,
