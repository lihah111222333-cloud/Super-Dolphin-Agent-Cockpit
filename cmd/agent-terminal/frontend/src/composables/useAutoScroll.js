...
export function useAutoScroll(workspaceRef) {
  ...
  function onDOMMutation() {
    ...
    const tryRestore = () => { mutationRestoreRaf = safeRaf(() => performMutationRestore(tryRestore, retryState)); };
    tryRestore();
  }
  ...
  function scheduleScrollToBottom(force = false) {
    if (scrollTimer) safeCaf(scrollTimer);
    ...
    const doScroll = () => {
      scrollTimer = safeRaf(() => {
        ...
        scrollTimer = safeRaf(() => {
          ...
          safeRaf(() => programmaticScrollInProgress = false);
        });
      });
    };
  ...
  onBeforeUnmount(() => {
    scrollListenerCleanup();
    if (scrollTimer) safeCaf(scrollTimer);
    if (mutationRestoreRaf) safeCaf(mutationRestoreRaf);
