...
export function useComposerInterruptVue(props, emit, deps) {
  const pauseAcknowledged = Vue.ref(false);
  const interruptPending = Vue.ref(false);
  const interruptRequestThreadId = Vue.ref('');
  const interruptTimeoutId = Vue.ref(0);
