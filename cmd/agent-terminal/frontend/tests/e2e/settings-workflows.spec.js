...
test('settings page loads project-scoped preferences and saves prompt/provider changes', async ({ page }) => {
  ...
  await page.getByTestId('provider-model-select').selectOption('sonnet');
  ...
  await expect(page.getByTestId('settings-provider-sandbox-card')).toContainText('已保存：sonnet / medium / pragmatic');
