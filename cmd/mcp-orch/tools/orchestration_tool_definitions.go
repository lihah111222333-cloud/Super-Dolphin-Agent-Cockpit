...
func orchestrationToolDefinitions(svc contract.OrchestrationService) []ToolDefinition {
	return buildToolDefinitions(
		defineTool("launch_agent", "Launch a managed orchestration agent.", ObjectSchema(map[string]Schema{
			...
			"context_mode": EnumStringSchema("Optional context mode for the first turn. Defaults to minimal. Use minimal for prompt-only launches; use focused only when passing caller-selected context.", launchAgentContextModeEnum...),
			"context":      StringSchema("Optional focused-mode context selected by the parent agent. Required when context_mode is focused; rejected when context_mode is minimal."),
