...
func taskToolDefinitions(ports ToolPorts) []ToolDefinition {
	return buildToolDefinitions(
		...
		}, "status"), HandleUpdateNode(ports.NodeStatus), "workflow.node.update", ToolIdempotencyRecommended, "workflow.runtime.write"),
		...
		}), HandleStartDAG(ports.DAGRuntime), "workflow.run.start", ToolIdempotencyRecommended, "workflow.runtime.write"),
		...
		}), HandleListDAGs(ports.DAGRuntime), "workflow.dag.list", "workflow.definition.read"),
		...
		}), HandleListRuns(ports.DAGRuntime), "workflow.run.list", "workflow.runtime.read"),
		workflowDiagnosticsToolDefinition(ports.WorkflowDiagnostics),
		workflowRecoveryActionToolDefinition(ports.WorkflowRecovery),
