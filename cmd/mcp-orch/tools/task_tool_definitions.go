...
func applyOpsInputSchemaWithRuntimeState(state *toolRuntimeState) Schema {
	return ObjectSchema(map[string]Schema{
		...
		"action":       EnumStringSchema("Flat action. Omit or use apply_ops_raw with ops for legacy raw batches.", state.applyOpsActionEnum...),
		"ops":          ArraySchema(applyOpsOpSchemaWithRuntimeState(state), "Typed ops array; each item must include 'op' discriminator."),
		...
		"node_type":    EnumStringSchema("Node type for action=add_node. Hybrid is reserved until runtime support is complete.", state.creatableNodeTypeEnum...),
...
func taskToolDefinitions(ports ToolPorts) []ToolDefinition {
	return taskToolDefinitionsWithRuntimeState(ports, newToolRuntimeState())
}

func taskToolDefinitionsWithRuntimeState(ports ToolPorts, state *toolRuntimeState) []ToolDefinition {
	return buildToolDefinitions(
		defineTaskWriteTool("task_create_dag", "Create a new DAG template and its nodes in the orchestration store. Existing dag_key values are not replaced; scheduled triggers must be enabled later via task_dag_apply_ops with base_version and cron_expr. This does not start execution; if the user asked to run/execute now, call task_start_dag after create succeeds.", createDAGSchemaWithRuntimeState(state), handleCreateDAGWithRuntimeState(ports.DAGCreate, state), "workflow.dag.create", ToolIdempotencyRequired, "workflow.definition.write"),
		defineTaskWriteTool("task_dag_apply_ops", "Apply a typed ops batch or one flat action (add_node / update_node / remove_node / update_dag) with base_version OCC. Use action+flat fields for common edits; use ops for advanced raw batches.", applyOpsInputSchemaWithRuntimeState(state), handleApplyOpsWithRuntimeState(ports.DAGRuntime, state), "workflow.dag.apply_ops", ToolIdempotencyRequired, "workflow.definition.write"),
		defineTaskWriteTool("task_update_node", "Update the runtime status for a DAG node.", ObjectSchema(map[string]Schema{
			...
			"status":   EnumStringSchema("New node status.", state.updateNodeStatusEnum...),
			"result":   StringSchema("Optional result summary."),
		}, "status"), handleUpdateNodeWithRuntimeState(ports.NodeStatus, state), "workflow.node.update", ToolIdempotencyRecommended, "workflow.runtime.write"),
		...
		}, "assigned_to"), HandleDispatchNode(ports.NodeDispatch), "workflow.node.dispatch", ToolIdempotencyRecommended, "workflow.runtime.write"),
		defineTaskWriteTool("task_start_dag", "Trigger a DAG execution (creates a run and snapshots dag.version). The response includes run_id, scheduled_wakeups, and execution_state; if scheduled_wakeups=0 with waiting_for_assignee, call task_dispatch_node.", ObjectSchema(map[string]Schema{
			...
			"trigger_source":  EnumStringSchema("Trigger source.", state.startDAGTriggerEnum...),
			"idempotency_key": StringSchema("Optional, prevents duplicate run on retry."),
		}), handleStartDAGWithRuntimeState(ports.DAGRuntime, state), "workflow.run.start", ToolIdempotencyRecommended, "workflow.runtime.write"),
		...
		}), HandleTerminateDAG(ports.DAGRuntime), "workflow.run.terminate", ToolIdempotencyRecommended, "workflow.runtime.write"),
		...
		}), HandleDeleteDAG(ports.DAGDelete), "workflow.dag.delete", ToolIdempotencyRequired, "workflow.definition.write"),
		defineTaskReadTool("task_list_dags", "List DAGs for console views and final_output retention checks.", ObjectSchema(map[string]Schema{
			"status":  EnumStringSchema("Optional status filter.", state.listDAGsStatusEnum...),
		...
		}), handleListDAGsWithRuntimeState(ports.DAGRuntime, state), "workflow.dag.list", "workflow.definition.read"),
		...
		}), HandleGetDAG(ports.DAGRuntime), "workflow.dag.get", "workflow.definition.read"),
		...
		}), HandleGetRun(ports.DAGRuntime), "workflow.run.get", "workflow.runtime.read"),
		defineTaskReadTool("task_list_runs", "List recent runs for a DAG (object response wraps the runs slice for forward-compatibility). Status enum mirrors migration 0080 task_dag_runs.status CHECK.", ObjectSchema(map[string]Schema{
			...
			"status":  EnumStringSchema("Optional status filter.", state.listRunsStatusEnum...),
			"limit":   IntegerSchema("Optional max rows; defaults to 50 when 0/omitted."),
		}), handleListRunsWithRuntimeState(ports.DAGRuntime, state), "workflow.run.list", "workflow.runtime.read"),
		workflowDiagnosticsToolDefinition(ports.WorkflowDiagnostics),
		workflowRecoveryActionToolDefinitionWithRuntimeState(ports.WorkflowRecovery, state),
		...
		}), HandleDiagnoseDAGPromptIdentityGaps(ports.DAGIdentityDiagnostics), "workflow.dag.diagnose_identity", "workflow.definition.read"),
	)
}
...
func workflowRecoveryActionToolDefinitionWithRuntimeState(svc workflowRecoveryPort, state *toolRuntimeState) ToolDefinition {
	return defineTaskWriteTool("task_workflow_recovery_action", "Run a controlled workflow recovery action. cancel_with_cleanup is wired to task_terminate_dag; retry_failed_node is validated but blocked until the runtime reset/retry contract exists.", ObjectSchema(map[string]Schema{
		"pos":      StringSchema("Optional flattened run locator, e.g. dag:<dag_key>/run:<run_key>."),
		"action":   EnumStringSchema("Recovery action.", state.recoveryActionEnum...),
	...
	}, "action"), handleWorkflowRecoveryActionWithRuntimeState(svc, state), "workflow.recovery.action", ToolIdempotencyRecommended, "workflow.runtime.write")
}
