...
func workflowDiagnosticsToolDefinition(svc contract.OrchestrationService) ToolDefinition {
	return defineTool("task_workflow_diagnostics", "Lookup compact workflow diagnostics by trace_id, run_key, run_id, node_key, or child_thread_id. Returns derived run summaries and matching runtime nodes only.", ObjectSchema(map[string]Schema{
	...
	}), HandleWorkflowDiagnostics(svc))
}

func workflowRecoveryActionToolDefinition(svc contract.OrchestrationService) ToolDefinition {
	return defineTool("task_workflow_recovery_action", "Run a controlled workflow recovery action. cancel_with_cleanup is wired to task_terminate_dag; retry_failed_node is validated but blocked until the runtime reset/retry contract exists.", ObjectSchema(map[string]Schema{
		"pos":      StringSchema("Optional flattened run locator, e.g. dag:<dag_key>/run:<run_key>."),
	...
	}, "action"), HandleWorkflowRecoveryAction(svc))
}
