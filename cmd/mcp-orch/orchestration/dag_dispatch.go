...
import (
	...
	"github.com/lihah111222333-cloud/super-dolphin-agent/cmd/mcp-orch/orchestration/scheduledstart"
...
func (d *WakeupDispatcher) recordDispatchFailedMetric() {
	d.dagMetrics.IncDispatchFailed()
}

// recordDispatchRetryMetric 记录派发重试指标，并在达到阈值时生成告警载荷。
func (d *WakeupDispatcher) recordDispatchRetryMetric(w *taskdag.Wakeup, lastErr string) (DispatchRetryAlert, bool) {
	if w == nil {
		return DispatchRetryAlert{}, false
	}
	attemptCount := max(w.AttemptCount, 1)
	record := d.dagMetrics.RecordRetry(w.DagKey, w.NodeKey, attemptCount)
