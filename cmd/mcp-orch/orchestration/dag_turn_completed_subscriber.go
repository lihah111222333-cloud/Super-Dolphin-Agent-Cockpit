...
func advanceNodeDoneForSuccess(
	...
	if advanceNodeDone(ctx, deps.FlowStore, logger, node, result) && deps.NodeRouter != nil {
		deps.NodeRouter.invokeStateChangeHooksForTaskNode(ctx, node, nodeexec.NodeOutcome{Status: nodeexec.NodeStatusDone, Result: result})
	}
}
...
func advanceNodeFailed(
	...
	if advanceNodeFailedWithReason(ctx, flow, logger, node, reason) && router != nil {
		router.invokeTerminalFailureHooksForTaskNode(ctx, node, nodeexec.NodeOutcome{Status: nodeexec.NodeStatusFailed, ErrorSummary: reason})
	}
}
...
func advanceNodeFailedWithReason(
	...
	_, err := flow.FailNodeAndCancelDownstream(ctx, taskdag.FailNodeInput{DagKey: node.DagKey, NodeKey: node.NodeKey, RunID: taskNodeRunID(node), Reason: reason, FailFast: false})
	switch {
	case err == nil:
...
func advanceNodeDone(
	...
	_, err := flow.CompleteNodeAndScheduleDownstream(ctx, taskdag.CompleteNodeInput{Status: "done", Result: result, DagKey: node.DagKey, NodeKey: node.NodeKey, RunID: taskNodeRunID(node)})
	switch {
	case err == nil:
...
func materializeSharedfileAfterClaim(
	...
	if exists {
		...
		return encodeSharedfileResultRef(materialized.SharedfilePath), true
	}
...
func claimNodeOutputMaterialization(
	...
	_, err := claimer.ClaimNodeOutputMaterialization(ctx, taskdag.OutputMaterializationClaimInput{DagKey: node.DagKey, NodeKey: node.NodeKey, RunID: taskNodeRunID(node), Result: result})
	switch {
	case err == nil:
		return true
	case errors.Is(err, pgx.ErrNoRows) || platformdb.IsNotFound(err):
