...
// dispatcherStubStore 是 Phase 3.1 单测用的 taskdag.Store 假实现，
// 只关注 ClaimDueWakeups 行为：记录入参 + 按预设结果返回。其余方法返回
// 安全 zero / 错误，避免 dispatcher 误用未接通的 store 接口面。
...
func TestNewWakeupDispatcherRejectsNilStore(t *testing.T) {
	if _, err := NewWakeupDispatcher(nil, nil, WakeupDispatcherConfig{}); err == nil {
...
func TestWakeupDispatcherTickEmptyClaimReturnsZero(t *testing.T) {
	...
	d, err := NewWakeupDispatcher(store, nil, WakeupDispatcherConfig{})
	if err != nil {
...
func TestWakeupDispatcherTickReturnsClaimCountAndPreservesFence(t *testing.T) {
	...
	d, err := NewWakeupDispatcher(store, nil, WakeupDispatcherConfig{ClaimedBy: "mcp-orch-dispatcher-7"})
	if err != nil {
		t.Fatalf("NewWakeupDispatcher err = %v", err)
	}
	n, err := d.Tick(context.Background())
