...
//   - AgentExecutor  → F1.1：解码 node.config.exec → orchestration_launch_agent 拉子 agent
//   - AutomationExecutor → F2.1：解码 command_ref → command_get + 执行确定性任务
//   - HybridExecutor → F3.1：先 automation 后 agent verifier（高风险操作）

// AgentExecutor 是 node_type=agent 节点的执行器 stub。
type AgentExecutor struct{}

func (AgentExecutor) Execute(_ context.Context, _ Node, _ RunContext) (NodeOutcome, error) {
	return NodeOutcome{Status: NodeStatusDone}, nil
}

func (AgentExecutor) Hooks() map[HookPoint]HookHandler { return nil }
