...
UPDATE task_dag_nodes
SET status = sqlc.arg('status'), result = sqlc.arg('result')::jsonb, active_turn_id = NULL, active_wakeup_id = sqlc.arg('active_wakeup_id'),
...
UPDATE task_dag_nodes
SET status = sqlc.arg('status'), result = sqlc.arg('result')::jsonb, active_turn_id = NULL, active_wakeup_id = NULL, updated_at = NOW()
WHERE dag_key = sqlc.arg('dag_key') AND node_key = sqlc.arg('node_key')
  AND run_id = sqlc.arg('run_id')
  AND sqlc.arg('run_id')::bigint > 0
...
-- 不破坏旧调用。
UPDATE task_dag_nodes
SET status = sqlc.arg('status'), result = sqlc.arg('result')::jsonb, active_turn_id = NULL, active_wakeup_id = NULL,
