...
func TestGoAdapterUsesSharedDaemonIdleTimeoutMarkerOnWindows(t *testing.T) {
	...
	want := []string{"-remote.listen.timeout=2.5s"}
	if !slices.Equal(command.Args, want) {
		t.Fatalf("Windows gopls args = %#v, want shared daemon marker %#v", command.Args, want)
	}
}

func TestGoAdapterRejectsMissingResolvedIdleTimeoutOnWindows(t *testing.T) {
