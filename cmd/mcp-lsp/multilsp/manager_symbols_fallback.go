...
// fallbackEmptyLSPDocumentSymbols 在真实 LSP 返回空大纲时，为 JS/TS 文件调用 TypeScript 官方导航树。
// 它只在 LSP 已经被尝试且结果为空后触发，避免替代正常语言服务器结果。
func (m *manager) fallbackEmptyLSPDocumentSymbols(ctx context.Context, ref documentRef, symbols []protocol.DocumentSymbol) ([]protocol.DocumentSymbol, bool, error) {
	if len(symbols) > 0 || !isJSTSDocumentSymbolFallbackLanguage(ref.languageID) {
		return nil, false, nil
	}
	fallback, err := m.typeScriptNavigationDocumentSymbols(ctx, ref)
	if err != nil {
		return nil, true, err
	}
	if len(fallback) == 0 {
