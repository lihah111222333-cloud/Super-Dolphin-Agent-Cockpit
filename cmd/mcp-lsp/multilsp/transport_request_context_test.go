...
func TestTransportRequestWriteHonorsContext(t *testing.T) {
	...
	assertRequestDeadlineTerminatedTransport(t, tr)
	closeOriginalStdin()
}

func TestTransportRequestWriteHonorsCancellation(t *testing.T) {
	if os.Getenv("MCP_LSP_BLOCK_STDIN_CANCEL_HELPER") == "1" {
		time.Sleep(30 * time.Second)
		return
	}
	...
	tr, err := newTransport(transportOptions{
		Binary: os.Args[0],
		Args:   []string{"-test.run=^TestTransportRequestWriteHonorsCancellation$"},
		Env:    []string{"MCP_LSP_BLOCK_STDIN_CANCEL_HELPER=1"},
	})
	if err != nil {
		t.Fatalf("newTransport() error = %v", err)
	}
...
func assertRequestDeadlineTerminatedTransport(t *testing.T, tr *transport) {
	t.Helper()
	if !tr.closed.Load() {
