...
// idleRealToolPaths 是平台 fixture 解析出的真实 Node 与 TypeScript server 身份。
// 共享 E2E 只消费已解析路径和启动 PATH，不把某个平台的安装布局写进生命周期逻辑。
type idleRealToolPaths struct {
	nodePath         string
...
func TestMcpLSPBinaryRealTypeScriptLeaseSurvivesThenRecyclesExactTree_E2E(t *testing.T) {
	...
	toolPaths := resolveIdleRealToolPaths(t)
	resolvedNodePath := toolPaths.nodePath
	resolvedTypeScriptPath := toolPaths.typeScriptPath
	...
	defer cancel()
	client := startMcpLSPBinaryForTestWithEnv(t, ctx, binary, root, toolPaths.serverSearchPath, []string{
...
func TestMcpLSPBinaryStdioRemainsAlivePastCanonicalIdleTimeout_E2E(t *testing.T) {
	...
	eofClient := startMcpLSPBinaryForTestWithEnv(t, ctx, binary, root, toolPaths.serverSearchPath, []string{
		"MCP_LSP_IDLE_TIMEOUT=" + idleRealTimeout.String(),
	})
