...
func TestMcpLSPBinaryWJBootCandidatePublisherPlatformConstraintKeepsRootCohortConfig(t *testing.T) {
	...
	if ordinaryDiagnostics.IsError {
		t.Fatalf("WJBoot ordinary diagnostics returned MCP error; text=%q structured=%s stderr=%s",
			ordinaryDiagnostics.ContentText(), ordinaryDiagnostics.StructuredContent, client.stderr.String())
	}
	...
	if diagnostics.IsError {
		t.Fatalf("WJBoot candidate publisher diagnostics returned MCP error; text=%q structured=%s stderr=%s",
