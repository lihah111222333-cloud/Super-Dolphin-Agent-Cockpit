...
// TestSearchASTCancelsAtMaxResults 使用 POSIX shell fixture；!windows build tag 使该
// 平台依赖在源码选择阶段可见，不再通过 runtime Skip 隐藏。
func TestSearchASTCancelsAtMaxResults(t *testing.T) {
	...
	matches, err := SearchAST(context.Background(), ASTSearchOptions{
		Root: root, Path: "main.go", Query: "fmt.Println($A)", Language: "go", MaxResults: 2,
	})
