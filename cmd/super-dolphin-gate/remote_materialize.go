...
func verifyRemoteMaterializedSource(ctx context.Context, sourceRoot string, artifactRoot string, request remoteci.ShardRequest) error {
	manifest, err := remoteci.MaterializeVerifiedSourceBundle(ctx, artifactRoot, sourceRoot)
	if err != nil {
		return fmt.Errorf("verify and materialize remote source bundle: %w", err)
	}
	if !reflect.DeepEqual(manifest.Source, request.Source) || manifest.SourceTreeSHA != request.SourceTreeSHA {
		return errors.New("remote source manifest does not match shard source request")
	}
