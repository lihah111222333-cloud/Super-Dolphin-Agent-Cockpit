...
// validateGenerationOneLiveContainerTiming 校验回执观测完全位于 ECI 实际运行区间内。
func validateGenerationOneLiveContainerTiming(groupID string, container eci.ContainerStatus, observation cicontract.ProvisionCheckObservation) error {
	...
	completedAt := container.CurrentState.FinishTime.UTC().UnixMilli()
	if container.CurrentState.StartTime.IsZero() || container.CurrentState.FinishTime.IsZero() || completedAt < startedAt || observation.StartedAtUnixMS < startedAt || observation.CompletedAtUnixMS > completedAt {
