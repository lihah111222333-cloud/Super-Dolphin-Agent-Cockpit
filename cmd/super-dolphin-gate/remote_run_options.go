...
// loadRemoteRefreshConfig adds the publication boundary required only by a
// detached refresh worker. Normal CI must remain usable with an accepted
// ImageCache while that boundary is absent or invalid.
func loadRemoteRefreshConfig(path string) (remoteRunConfig, error) {
	config, err := loadRemoteRunConfig(path)
	if err != nil {
		return remoteRunConfig{}, err
	}
	if err := config.ValidateOCIRefresh(); err != nil {
		return remoteRunConfig{}, err
	}
	return config, nil
}
