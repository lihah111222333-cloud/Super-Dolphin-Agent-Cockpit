...
**真值镜像交付门禁。** 纳入版本控制的 [Git hooks](.githooks/README.md)、手动 `make ci-l0` 与 `make ci-l1` 以及发布校验执行两层职责：`pre-commit` 只对精确暂存树运行仓库代码守卫，不启动远程 CI；`pre-push`、手动 L0-L1 命令和发布校验才对精确推送树或提交运行失败即阻断的远程 ECI 门禁。GitHub 仅是 Git 远端，不提供 CI runner。远程配置、来源证明、结果权威、清理证据缺失或推送/发布门禁失败都会拒绝交付。`commit-msg` 仍要求中文提交文本和修复测试证据。
