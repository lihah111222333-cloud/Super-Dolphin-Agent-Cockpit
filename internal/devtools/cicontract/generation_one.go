...
const (
	...
	// the configured strict generation-one receipt consumed by normal run/hook.
	GenerationOneProvisionReceiptSchemaVersion uint32 = 4
	...
	// GenerationOneProvisionAuthority distinguishes external cloud evidence from
	// the repository normal run/hook bootstrap consumer.
...
// an external ECI operator and carried by remote run config. The embedded state is protocol data; SQLite remains
