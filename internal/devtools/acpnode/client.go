...
func NewClient(cfg LaunchConfig, factory ProcessFactory, reverse ReverseRequestHandler) (*Client, error) {
	...
	go c.runStdout(stdoutOwner)
	go c.runStderr(stderrOwner)
	return c, nil
}
...
func (c *Client) startWait() {
	c.waitOnce.Do(func() {
		...
		c.mu.Unlock()
		c.trackOwner(owner)
		go runProcessActionOwner(owner, c.p.Wait)
		go c.waitLoop(owner)
	})
}
...
func (c *Client) dispatchReverse(m Message) {
	...
	go c.runReverseRequest(ctx, key, cloneRaw(m.ID), m.Method, cloneRaw(m.Params))
}
