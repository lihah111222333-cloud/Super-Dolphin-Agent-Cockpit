...
func startProcessActionOwner(action func() error) *processActionOwner {
	...
	go runProcessActionOwner(owner, action)
	return owner
}
...
func startProcess(cfg LaunchConfig, factory ProcessFactory) (Process, error) {
	...
	defer cancel()
	go startProcessCall(ctx, factory, cfg, result)
	select {
	case outcome := <-result:
		if ctx.Err() != nil {
	...
	case <-ctx.Done():
		...
		case <-cleanupTimer.C:
			...
			go cleanupLateStartOwner(owner, cfg.ShutdownTimeout)
...
func waitProcess(process Process, timeout time.Duration) (bool, error) {
	return awaitProcessAction(startProcessActionOwner(process.Wait), timeout)
}
