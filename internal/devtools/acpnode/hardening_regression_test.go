...
func TestCancelCommitsAfterSuccessfulSendEvenIfContextIsCanceled(t *testing.T) {
	...
	go func() { done <- client.Cancel(ctx, "s1") }()
...
func TestCloseWaitsForGatedWriteOwnerAdmission(t *testing.T) {
	...
	client.writeAdmissionMu.Unlock()
	select {
	case <-process.stdin.started:
	case <-time.After(time.Second):
		t.Fatal("gated writer did not start after admission release")
	}
	var owner *writeOwner
	deadline = time.Now().Add(time.Second)
	for owner == nil && time.Now().Before(deadline) {
	...
	case <-client.Done():
	case <-time.After(time.Second):
		t.Fatal("client Done did not close after gated write owner joined")
	}
	select {
	case <-requestDone:
	case <-time.After(time.Second):
		t.Fatal("gated request did not return")
	}
}
...
func TestDelayedStreamErrorsAfterImmediateNilWaitRemainDurable(t *testing.T) {
	...
	for name, started := range map[string]<-chan struct{}{
		...
		case <-started:
		case <-time.After(time.Second):
			t.Fatalf("%s delayed reader did not start", name)
		}
	}
	...
	if err := client.WaitErr(); !errors.Is(err, stdoutErr) || !errors.Is(err, stderrErr) {
		t.Fatalf("WaitErr omitted delayed stream errors: %v", err)
	}
	if err := client.Err(); !errors.Is(err, stdoutErr) || !errors.Is(err, stderrErr) {
		t.Fatalf("Err omitted delayed stream errors: %v", err)
	}
	if err := client.Close(); !errors.Is(err, stdoutErr) || !errors.Is(err, stderrErr) {
		t.Fatalf("Close omitted delayed stream errors: %v", err)
	}
}
...
func TestInitializeRequiresCanonicalAgentCapabilities(t *testing.T) {
	...
	go func() { done <- client.Initialize(context.Background(), map[string]any{"name": "test"}) }()
	request := nextPeerMessage(t, process)
...
func TestLateStartOwnerTracksNonCooperativeFactoryAndSurfacesCleanup(t *testing.T) {
	...
	case <-factory.returned:
	case <-time.After(time.Second):
		t.Fatal("late factory did not return")
	}
	select {
	case <-late.Done():
	case <-time.After(time.Second):
		t.Fatal("late cleanup owner did not finish")
	}
	if !strings.Contains(late.Error(), "late wait cleanup failed") || !strings.Contains(late.Error(), "late kill cleanup failed") {
		t.Fatalf("late cleanup errors are not observable: %v", late.Err())
	}
}
