...
func (c *Client) startShutdown() {
	...
	c.closeMu.Unlock()
	if !started {
		launchACP(context.Background(), "acp failure shutdown", c.closeAfterFailure)
	}
}
...
func closeCloserBoundedTracked(closer io.Closer, timeout time.Duration, track func(trackedOwner)) error {
	...
	launchACP(context.Background(), "acp stream close", func() { runLifecycleAction(owner, closer.Close) })
	timer := time.NewTimer(timeout)
	defer timer.Stop()
	select {
...
func boundedLifecycleActionTracked(action func() error, timeout time.Duration, track func(trackedOwner)) error {
	...
	launchACP(context.Background(), "acp bounded lifecycle action", func() { runLifecycleAction(owner, action) })
	timer := time.NewTimer(timeout)
	defer timer.Stop()
	select {
	case err := <-owner.result:
