...
func (c *Client) send(m Message) error {
	return c.sendContext(context.Background(), m, c.cfg.ShutdownTimeout)
}
...
func (c *Client) reservePending() (string, json.RawMessage, *pendingCall, error) {
	...
	return key, id, call, nil
}

func (c *Client) allocateRequestID() (json.RawMessage, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.closed || c.terminated {
		return nil, ErrClientClosed
	...
	c.next++
	id := json.RawMessage(fmt.Sprintf("%d", c.next))
	return id, nil
}
