...
func TestInitializeRemoteBaselineGenerationOneRejectsOrphanHistory(t *testing.T) {
	...
	if _, err := database.Exec(`INSERT INTO duration_samples (accepted_generation, workload_id, command_digest, platform, runner, toolchain, succeeded, duration_ms) VALUES ('1','orphan','orphan','linux/amd64','eci','go',1,1)`); err != nil {
		t.Fatal(err)
	}
	if err := database.Close(); err != nil {
		t.Fatal(err)
