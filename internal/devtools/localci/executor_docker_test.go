...
func TestDockerExecutorAppliesIsolationAndResourceContract(t *testing.T) {
	...
	wantCreate := []string{
		...
		"registry.local/gate@" + digest("1"), "gate", "run",
	}
...
func TestDockerExecutorKillsAndRemovesOnWaitFailure(t *testing.T) {
	...
	evidence, err := executor.Run(context.Background(), containerRequest{Image: "repo@" + digest("1"), SourceDir: sourceDir, Command: []string{"gate"}})
...
func TestDockerExecutorPassesProfileDeadlineToWait(t *testing.T) {
	for _, release := range []bool{false, true} {
		...
		if _, err := executor.Run(context.Background(), containerRequest{Image: "repo@" + digest("1"), SourceDir: sourceDir, Command: []string{"gate"}, Release: release}); err != nil {
			t.Fatal(err)
		}
...
func TestDockerExecutorRejectsMountInjectionAndSourceEscape(t *testing.T) {
	...
	for _, source := range []string{sourceDir + ",dst=/host", outside} {
		if _, err := executor.Run(context.Background(), containerRequest{Image: "repo@" + digest("1"), SourceDir: source, Command: []string{"gate"}}); err == nil {
