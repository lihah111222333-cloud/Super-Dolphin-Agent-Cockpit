...
const (
	remoteShardBootstrapSH      = `set -eu; accepted_gate="/super-dolphin-gate"; "$accepted_gate" _remote-materialize; private_cache="/workspace/work/go-cache"; private_mod_cache="/tmp/init-go-mod-cache"; built_gate="/workspace/work/bin/super-dolphin-gate"; mkdir -p "$private_cache" "$private_mod_cache" "$(dirname "$built_gate")"; "$accepted_gate" worker go-module-overlay /opt/super-dolphin-gate/runtime/go-mod-cache "$private_mod_cache"; started="$(date +%s%3N)"; cd /workspace/source; cache_proxy="$accepted_gate worker go-cache-proxy --seed /opt/super-dolphin/cache/go-build --private $private_cache --metrics $private_cache/shard-compile.metrics"; env GOCACHE="$private_cache" GOCACHEPROG="$cache_proxy" GOMODCACHE="$private_mod_cache" GOPROXY=off GOSUMDB=off GOTOOLCHAIN=local CGO_ENABLED=0 /usr/local/go/bin/go build -mod=mod -trimpath -buildvcs=false -o "$built_gate" ./cmd/super-dolphin-gate; test -x "$built_gate"; finished="$(date +%s%3N)"; printf 'SUPER_DOLPHIN_SHARD_COMPILE started_at_unix_ms=%s completed_at_unix_ms=%s duration_ms=%s cache_metrics=%s\n' "$started" "$finished" "$((finished-started))" "$private_cache/shard-compile.metrics"`
