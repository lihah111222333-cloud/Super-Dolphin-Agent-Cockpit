...
func renderAuthorityTimingObservation(destination io.Writer, observation gate.TimingObservation) error {
	...
	if observation.Scope == "compile_group" {
		_, err := fmt.Fprintf(destination, "REMOTE_CI_COMPILE_GROUP_OBSERVATION shard=%s group_id=%s artifact_key=%s package_target=%s workload_ids=%v phase=%s started_at=%s completed_at=%s duration_ms=%d artifact_sha256=%s artifact_size=%d cache_hits=%d cache_misses=%d cache_puts=%d cache_status=%s status=%s exit_code=%d error_text=%s compile_command_digest=%s profile_digest=%s resource_class_id=%s resource_cpu=%g resource_memory_gib=%g execution_mode=%s go_cache_source=%s go_cache_status=%s\n", observation.ShardIdentity, observation.CompileGroupID, observation.CompileArtifactKey, observation.CompilePackageTarget, observation.CompileWorkloadIDs, observation.Phase, observation.StartedAt.UTC().Format(time.RFC3339Nano), observation.CompletedAt.UTC().Format(time.RFC3339Nano), observation.DurationMS, observation.CompileArtifactSHA256, observation.CompileArtifactSize, observation.CompileCacheHits, observation.CompileCacheMisses, observation.CompileCachePuts, observation.CompileCacheStatus, observation.CompileStatus, observation.CompileExitCode, observation.CompileErrorText, observation.CompileCommandDigest, observation.CompileProfileDigest, observation.CompileResourceClassID, observation.CompileResourceCPU, observation.CompileResourceMemoryGiB, observation.CompileExecutionMode, goEvidence.Source, goEvidence.Status)
		return err
	}
