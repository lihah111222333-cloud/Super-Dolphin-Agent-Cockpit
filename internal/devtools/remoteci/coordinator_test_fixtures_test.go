...
func writeRemoteRunFixtureFiles(t *testing.T, repository string) {
	...
	writeCoordinatorFixture(t, repository, "internal/devtools/gate/executor.go", "package gate\n\nfunc ensureJSONEOF() {}\nfunc executeProgram() error { return nil }\nfunc sourceVariantCount() {}\nfunc validateCommitSource() {}\nfunc validateOID() {}\nfunc validateRangeSource() {}\nfunc validateTreeSource() {}\n\nfunc ExecuteExecutor() { config := executorConfig{}; _ = config }\ntype executorConfig struct{}\n")
	...
	writeCoordinatorFixture(t, repository, "cmd/super-dolphin-gate/main.go", "package main\n\nfunc runRemoteMaterialize() error { return nil }\nfunc installAcceptedBootstrapManifest() error { return nil }\nfunc stageRemoteSourceObjects() error { return nil }\nfunc verifyRemoteMaterializedGateCLICompileClosure() error { return nil }\nfunc verifyRemoteOCIProjectCache() error { return nil }\nfunc verifyRemoteSourceManifestBinding() error { return nil }\nfunc runWorkerCLI() int { return 0 }\n")
	...
	writeCoordinatorFixture(t, repository, workerExecutionSourceManifestPath, "package remoteci\n\nfunc readSourceManifest() {}\n")
