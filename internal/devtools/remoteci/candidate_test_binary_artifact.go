...
func validGoTestBinaryBuild(pkg, mode, platform, toolchain string, cgoEnabled bool, flags []string) bool {
	...
	if pkg == "" || len(pkg) > 512 || strings.HasPrefix(pkg, "/") || (cleanPackage != pkg && pkg != "./"+cleanPackage) || (mode != "test" && mode != "race") || platform != "linux/amd64" || toolchain != gate.RequiredGoToolchain || !cgoEnabled {
		return false
	}
