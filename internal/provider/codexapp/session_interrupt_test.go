...
func newEarlyTerminalTurnFixture(t *testing.T) earlyTerminalTurnFixture {
	...
	s, err := newSession(context.Background(), testLoggerRuntime(t), startEarlyTerminalTurnStartServer(t), "agent-1", dispatcher, testApprovalManager(), nil, testSkillMetrics(t))
	if err != nil {
...
func newActiveInterruptTestSession(t *testing.T, serverURL string, dispatcher *unified.EventDispatcher) *session {
	t.Helper()
	s, err := newSession(context.Background(), testLoggerRuntime(t), serverURL, "agent-1", dispatcher, testApprovalManager(), nil, testSkillMetrics(t))
	if err != nil {
		t.Fatalf("newSession() error = %v", err)
	}
	s.setThreadID("thread-1")
