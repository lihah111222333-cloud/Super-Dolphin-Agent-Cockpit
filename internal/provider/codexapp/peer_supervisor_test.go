...
func TestPeerProcessEnvCarriesLegacySessionTokenAsCanonical(t *testing.T) {
	env, err := peerProcessEnv("mcp-orch", []string{"PATH=/bin", "GO_AGENT_MCP_SESSION_TOKEN=legacy-token", "GO_AGENT_CTL_BINARY_NAME=super-agent-debug.exe", "GO_AGENT_CTL_CLIENT_KIND=custom"}, nil)
	if err != nil {
		t.Fatalf("peerProcessEnv() error = %v", err)
	}
