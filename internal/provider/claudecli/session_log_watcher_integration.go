...
func (s *session) needsRestartLocked(next stagedSessionState) bool {
	return next.settingsChanged || s.transportFencedLocked() || !s.transport.readyForSend()
}

func (s *session) transportFencedLocked() bool {
	return s.transport != nil && s.fencedTransport == s.transport
}
...
func (s *session) restartSnapshotLocked() restartSnapshot {
	...
	return restartSnapshot{
		...
		fencedTransport:   s.fencedTransport,
	}
}
