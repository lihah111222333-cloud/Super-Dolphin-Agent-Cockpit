...
func baselineContracts() map[string]tableContract {
	return map[string]tableContract{
		...
		"task_dags":              {PrimaryKey: []string{"id"}, NotNull: []string{"dag_key", "title", "description", "status", "created_by", "metadata", "created_at", "updated_at", "trigger", "owner_id", "cron_expr", "version"}, Checks: []string{"json_valid(metadata)", "trigger IN ('manual', 'auto', 'scheduled', 'external')"}, Indexes: []string{"idx_task_dags_status", "idx_task_dags_next_run_scheduled"}},
