...
func baselineContracts() map[string]tableContract {
	return map[string]tableContract{
		...
		"terminal_outcome_heads":                {PrimaryKey: []string{"agent_id"}, NotNull: []string{"capability", "public_thread_id", "provider_turn_id", "session_id", "generation", "event_id", "terminal_identity", "expected_active_state", "state", "updated_at"}, Checks: []string{"capability = 'terminal_outcome_commit_v2'", "generation > 0", "state IN ('active', 'terminal')"}},
		"public_terminal_outcomes":              {PrimaryKey: []string{"agent_id"}, NotNull: []string{"schema_version", "projection_kind", "public_thread_id", "provider_turn_id", "session_id", "generation", "event_id", "terminal_identity", "public_outcome_json", "public_report", "occurred_at"}, Checks: []string{"schema_version = 2", "json_valid(public_outcome_json)", "generation > 0"}},
		"terminal_outcome_outbox":               {PrimaryKey: []string{"id"}, NotNull: []string{"event_id", "payload_json", "status", "claimed_by", "created_at"}, Checks: []string{"json_valid(payload_json)", "status IN ('pending', 'claimed', 'projected')"}, Indexes: []string{"idx_terminal_outcome_outbox_claim"}, ForeignKey: []foreignKeyContract{{Column: "event_id", Table: "public_terminal_outcomes"}}},
		"terminal_outcome_current_heads":        {PrimaryKey: []string{"agent_id"}, NotNull: []string{"capability", "public_thread_id", "provider_turn_id", "session_id", "generation", "expected_active_state", "version", "state", "terminal_event_id", "terminal_identity", "activated_at", "updated_at"}, Checks: []string{"capability = 'terminal_outcome_commit_v2'", "generation > 0", "version > 0", "state IN ('active', 'terminal')"}},
		"public_terminal_outcome_history":       {PrimaryKey: []string{"terminal_identity"}, NotNull: []string{"event_id", "agent_id", "head_version", "schema_version", "projection_kind", "public_thread_id", "provider_turn_id", "session_id", "generation", "expected_active_state", "public_outcome_json", "public_report", "occurred_at"}, Checks: []string{"head_version > 0", "schema_version = 2", "json_valid(public_outcome_json)", "generation > 0"}, Indexes: []string{"idx_public_terminal_history_agent_generation"}},
		"terminal_outcome_private_dag_payloads": {PrimaryKey: []string{"id"}, NotNull: []string{"terminal_identity", "owner_agent_id", "public_thread_id", "provider_turn_id", "payload_json", "created_at"}, Checks: []string{"json_valid(payload_json)"}, ForeignKey: []foreignKeyContract{{Column: "terminal_identity", Table: "public_terminal_outcome_history"}}},
		"terminal_outcome_outbox_v2":            {PrimaryKey: []string{"id"}, NotNull: []string{"terminal_identity", "event_id", "public_payload_json", "status", "claimed_by", "claim_token", "attempt_count", "last_error", "created_at"}, Checks: []string{"json_valid(public_payload_json)", "status IN ('pending', 'claimed', 'projected', 'poisoned')"}, Indexes: []string{"idx_terminal_outcome_outbox_v2_claim"}, ForeignKey: []foreignKeyContract{{Column: "terminal_identity", Table: "public_terminal_outcome_history"}, {Column: "private_dag_payload_id", Table: "terminal_outcome_private_dag_payloads"}}},
