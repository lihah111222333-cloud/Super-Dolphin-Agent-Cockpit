...
func baselineContracts() map[string]tableContract {
	return map[string]tableContract{
		...
		"mcp_tool_lifecycle_states": {PrimaryKey: []string{"workspace_root", "server_name", "tool_name"}, NotNull: []string{"workspace_root", "server_name", "tool_name", "lifecycle_state", "reason", "source", "updated_by", "created_at", "updated_at"}, Checks: []string{"workspace_root <> ''", "server_name <> ''", "tool_name <> ''", "lifecycle_state IN ('active', 'suspended', 'removed')", "source IN ('discovery', 'user', 'migration', 'system')"}},
