...
func CaptureCooperativeEndpointIdentity(endpoint string) (CooperativeEndpointIdentity, error) {
	if !filepath.IsAbs(endpoint) || filepath.Clean(endpoint) != endpoint {
		return CooperativeEndpointIdentity{}, errors.New("pidregistry: cooperative termination endpoint must be absolute and clean")
	}
	...
	if info.Mode()&os.ModeSocket == 0 {
		return CooperativeEndpointIdentity{}, errors.New("pidregistry: cooperative termination endpoint is not a Unix socket")
	}
	stat, ok := info.Sys().(*syscall.Stat_t)
	if !ok || stat.Uid != uint32(os.Geteuid()) {
		return CooperativeEndpointIdentity{}, errors.New("pidregistry: cooperative termination endpoint owner mismatch")
	}
...
func quarantineCooperativeTerminationEndpoint(
	...
	if err := unix.RenameatxNp(unix.AT_FDCWD, endpoint, unix.AT_FDCWD, quarantine, unix.RENAME_EXCL); err != nil {
...
func restoreCooperativeEndpointQuarantine(endpoint, quarantine string, primary error) error {
	if err := unix.RenameatxNp(unix.AT_FDCWD, quarantine, unix.AT_FDCWD, endpoint, unix.RENAME_EXCL); err != nil {
...
// dialVerifiedCooperativeEndpoint 将连接绑定到预期路径实例和 Darwin peer PID。
