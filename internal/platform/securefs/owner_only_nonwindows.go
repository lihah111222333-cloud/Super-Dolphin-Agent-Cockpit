...
func SyncDirectory(path string) error {
	...
	if err := errors.Join(syncErr, closeErr); err != nil {
		return fmt.Errorf("sync directory %s: %w", RedactPath(path), err)
	}
	return nil
}

func wrapWindowsPermissionError(err error, _ string) error {
	return err
}
...
func CheckExistingOwnerOnly(path string, info os.FileInfo) error {
	if info == nil {
		...
		if err != nil {
			return fmt.Errorf("inspect owner-only path %s: %s", RedactPath(path), SafeError(err))
		}
	}
