...
func TestDiffFallbackTracker_SkipsSeen(t *testing.T) {
	...
	tracker.handleToolCallEnd(diffFallbackEvent("agent-1", "thread-1", "call-seen", "code_run"))
	if len(emitted) != 0 {
...
func TestDiffFallbackTracker_EmitsDiff(t *testing.T) {
	...
	tracker.handleToolCallEnd(diffFallbackEvent("agent-2", "thread-2", "call-new", "code_run"))
	if len(emitted) != 1 {
	...
	if got.ToolName != "code_run" {
		t.Fatalf("emitted ToolName = %q, want code_run", got.ToolName)
	}
