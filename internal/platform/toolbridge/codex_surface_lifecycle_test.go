...
func TestCodexToolSurfaceDeniesDisabledManagedMCPDirectCalls(t *testing.T) {
	...
	h := &Handler{
		toolLifecycle: fakeMCPToolLifecycleReaderWithRecords(
			"/repo",
...
func TestCodexToolSurfaceLifecycleDoesNotBlockHostDirectOrSkillTools(t *testing.T) {
	...
	h := &Handler{
		...
		toolLifecycle:      &fakeMCPToolLifecycleReader{err: errors.New("lifecycle should not block host or skill tools")},
