...
// Renderer 独占三端渲染规则；构造后只读，可安全地由单个 Flusher 共享。
type Renderer struct {
	markdownEscapeReplacer *strings.Replacer
	slackMentionRE         *regexp.Regexp
	feishuMentionRE        *regexp.Regexp
	dingtalkMentionRE      *regexp.Regexp
}

// NewRenderer 创建三端共用的渲染规则 owner。
func NewRenderer() *Renderer {
	return &Renderer{
		markdownEscapeReplacer: strings.NewReplacer(
			`\`, `\\`,
			"*", `\*`,
			"_", `\_`,
			"`", "\\`",
...
func (r *Renderer) MarkdownEscape(s string) string {
	return r.markdownEscapeReplacer.Replace(s)
}

// StripMentions 移除广播式 mention token；需要定向 mention 时必须由模板常量显式生成。
func (r *Renderer) StripMentions(s string) string {
	s = r.slackMentionRE.ReplaceAllString(s, "")
	s = r.feishuMentionRE.ReplaceAllString(s, "")
	s = r.dingtalkMentionRE.ReplaceAllString(s, "")
	return s
}
...
func (r *Renderer) NormalizeBody(s string, maxBytes int) string {
	return Truncate(r.MarkdownEscape(r.StripMentions(s)), maxBytes)
}

// NormalizeTitle 使用与正文相同的清洗流程，但标题使用更小上限以适配卡片头部。
func (r *Renderer) NormalizeTitle(s string) string {
	return Truncate(r.MarkdownEscape(r.StripMentions(s)), 256)
}
