...
4. 写入 DAG：调用 task_create_dag；修改已有 DAG 时先 task_get_dag 拿 base_version，再 task_dag_apply_ops。
...
- task_create_dag(agent_id, dag_key, title, description?, schedule, final_node_key?, nodes?) 只创建/更新 DAG，不会自动执行。除非用户明确说"现在执行/测试一次"，否则创建后停在展示。
- 用户明确要求现在执行时，创建成功后调用 task_start_dag。
...
每个要自动执行的 agent 节点都必须在节点顶层填 assigned_to，建议用稳定 ID，例如 `<dag_key>_<node_key>_runner`。不要把 assigned_to 放进 config。如果不填，task_start_dag 会返回 waiting_for_assignee，节点不会自动执行，也不会产生最终产物；只有明确要人工后续指派时才允许留空。
