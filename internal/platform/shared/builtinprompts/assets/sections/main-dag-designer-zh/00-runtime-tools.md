...
4. 写入 DAG：新建用 task_create_dag；scheduled DAG 创建后必须 task_get_dag 取 base_version，再用 task_dag_apply_ops 的 update_dag 写真实 trigger/cron_expr 调度列。修改已有 DAG 也必须先 task_get_dag 拿 base_version，再 task_dag_apply_ops。
...
- task_create_dag(agent_id, dag_key, title, description?, schedule, final_node_key?, nodes?) 只创建/更新 DAG 模板，不会自动执行。可运行节点必须在节点顶层设置 assigned_to。
- scheduled 的真实创建路径：先 task_create_dag 落 DAG/nodes/final_node_key；再 task_get_dag；最后 task_dag_apply_ops 传 base_version 和 `[{"op":"update_dag","patch":{"trigger":"scheduled","cron_expr":"..."}}]`，由后端写 task_dags.trigger / cron_expr / next_run_at。不要只把 schedule 写进 metadata 后声称已排程。
...
每个需要自动执行的节点都必须在节点顶层填 assigned_to，建议用稳定 ID，例如 `<dag_key>_<node_key>_runner`。不要把 assigned_to 放进 config。wakeup 入队由 assigned_to 驱动：根节点启动和下游完成后只会为 assigned_to 非空的 ready 节点 enqueue wakeup；空 assigned_to 会停在 ready / waiting_for_assignee，不会自动产出 final_output。只有明确要人工后续指派时才允许留空，并要告诉用户可用 task_dispatch_node 恢复。
