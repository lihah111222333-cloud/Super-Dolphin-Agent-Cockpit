...
Video generation node example (use to_artifact when the deliverable is a binary file such as mp4 — never use to_sharedfile for binary): `{"node_key":"generate_video","title":"Generate video","node_type":"agent","assigned_to":"my_dag_video_runner","depends_on":[],"config":{"exec":{"prompt_key":"<video agent prompt_key>","provider":"<selected provider>","model":"<selected model>","cwd":"/absolute/project/cwd"},"first_turn":"Use the video_with_audio tool to generate a video. prompt: '<scene description>', voice_text: '<narration>'.","outputs":{"to_artifact":{"source_tool":"video_with_audio","source_path_field":"output_path","path_template":"dag/videos/{{date}}/run-{{run_id}}.mp4","content_type":"video/mp4","allowed_extensions":[".mp4"],"allowed_source_roots":["$HOME/Movies"],"max_bytes":524288000,"overwrite":"replace"}}}}`

- Use `to_artifact` to import a local binary file produced by an agent tool (mp4, image, etc.) into sharedfile storage. Key fields: `source_tool` = name of the tool that produced the file; `source_path_field` = the JSON key in that tool's result containing the file path (`video_with_audio` → `output_path`, `video_generate` → `local_path`); `path_template` supports `{{date}}` `{{run_id}}` etc. to avoid overwrites; `allowed_source_roots` must include the directory where the tool writes files (video tools default to `$HOME/Movies`).
- When the user asks to generate a video, always use `to_artifact`. `to_sharedfile` is for text only.
