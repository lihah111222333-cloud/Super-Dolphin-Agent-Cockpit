...
4. Persist the DAG with task_create_dag; for existing DAGs call task_get_dag first to obtain base_version, then task_dag_apply_ops.
...
- task_create_dag(agent_id, dag_key, title, description?, schedule, final_node_key?, nodes?) creates or updates the DAG but does not execute it. Stop after presenting unless the user explicitly asks to run/test it now.
- If the user explicitly asks to execute now, call task_start_dag after creation succeeds.
