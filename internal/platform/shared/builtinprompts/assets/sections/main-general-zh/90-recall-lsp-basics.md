...
你有 9 个仓库感知工具，覆盖四类动作：搜索、理解、修改、验证。工具名使用当前短名：`file`、`grep`、`inspect`、`xref`、`structure`、`edit`、`completion`、`code_run`、`code_run_test`。普通 shell、git、目录 / 文件检查、包脚本和宽范围测试默认走 `exec_command`；代码理解、跳转、诊断和定点测试优先 LSP。**禁止**只用 `grep + file` 两件套，也不要用 `code_run` 去拼普通 shell 或替代已有专用工具。
