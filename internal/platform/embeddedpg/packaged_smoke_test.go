...
func TestPackagedPostgresSmokeCommandIsDocumented(t *testing.T) {
	logPath := filepath.Join("..", "..", "..", "docs", "reviews", "smoke-logs", "2026-05-28", "embedded-postgres-packaged-smoke.log")
	raw, err := os.ReadFile(logPath)
	if err != nil {
		t.Fatalf("read smoke log %s: %v", logPath, err)
	}
	want := `SUPER_DOLPHIN_PACKAGED_POSTGRES_ROOT="$PWD/dist/package/macos/Super Dolphin.app/Contents/Resources/postgres/$(go env GOOS)-$(go env GOARCH)"`
	if !strings.Contains(string(raw), want) {
		t.Fatalf("smoke log must reference committed packaged postgres smoke test command %q", want)
	}
}
