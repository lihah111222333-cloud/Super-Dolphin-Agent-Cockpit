...
func TestNewServiceConfiguresProjectRootAndHTTPTimeout(t *testing.T) {
	...
	impl, ok := NewService(" /tmp/project ", testSkillMetrics(t), NewMirrorRootLockRegistry()).(*service)
	if !ok {
...
func TestNewServiceOmitsProjectSkillsRootWhenProjectRootEmpty(t *testing.T) {
	...
	impl, ok := NewService("   ", testSkillMetrics(t), NewMirrorRootLockRegistry()).(*service)
	if !ok {
		t.Fatal("NewService type assertion failed")
	}
	if impl.projectRoot != "" {
