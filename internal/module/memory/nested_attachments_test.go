...
func TestNestedResolveTurnAttachmentsLifecycle(t *testing.T) {
	...
	cfg := &Config{NestedMemory: NestedMemoryConfig{Enabled: true}}
	provider := NewClaudeMdSourcesProvider(cfg, nil, NewNestedRuntime(cfg, nil))
...
func TestNestedResolveTurnAttachmentsHardDeniesManagedPaths(t *testing.T) {
	...
	cfg := &Config{
		...
		NestedMemory:        NestedMemoryConfig{Enabled: true},
	}
	provider := NewClaudeMdSourcesProvider(cfg, nil, NewNestedRuntime(cfg, nil))
