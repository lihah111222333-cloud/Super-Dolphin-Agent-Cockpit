...
func TestNestedResolveConditionalDelta(t *testing.T) {
	...
	baseSources := ResolveClaudeMdSources(context.Background(), ClaudeMdResolveConfig{BuildCtx: buildCtx, MemoryConfig: &Config{}})
	gotSources := resolveNestedConditionalSources(context.Background(), buildCtx, &Config{}, target, baseSources)
