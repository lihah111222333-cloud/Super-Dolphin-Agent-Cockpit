...
func (p *ClaudeMdSourcesProvider) ResolveTurnAttachments(
	...
	if p == nil || p.nested == nil || !nestedMemoryEnabled(p.cfg) {
		return nil
	}
	if ResolveMemoryGate(buildCtx, p.cfg).DisableClaudeMds {
		return nil
	}
...
func (p *ClaudeMdSourcesProvider) appendNestedMemoryAttachments(
	...
	for _, source := range resolveNestedConditionalSources(ctx, buildCtx, p.cfg, target, baseSources) {
...
func nestedMemoryEnabled(cfg *Config) bool {
	if cfg == nil {
		return false
	}
	return cfg.NestedMemory.Enabled
}
