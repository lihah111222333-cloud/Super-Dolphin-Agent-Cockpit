...
func sessionGuidanceCases() []sessionGuidanceCase {
	return []sessionGuidanceCase{
		...
		},
		{
			...
			enabledTools: []string{"spawn_agent", "launch_agent", "get_agent_report", "get_agent_reports", "send_message", "interrupt_agent", "recover_agent", "stop_agent", "grep", "file"},
			flags:        map[string]bool{"persistent_subagent_default": true, "explore_agent_enabled": true},
			want: []string{
				"Do not launch a child agent for simple tasks",
				"`launch_agent`",
				...
				"`context_mode=\"forked\"`",
				"must inherit the current parent thread history",
