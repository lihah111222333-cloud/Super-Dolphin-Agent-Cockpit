...
func TestPersistThreadStateRejectsProviderThreadMismatch(t *testing.T) {
	t.Parallel()
	...
	if err == nil || !strings.Contains(err.Error(), "provider thread") {
		t.Fatalf("persistThreadState() error = %v, want provider thread mismatch", err)
	}
	if bindings.upsert.AgentID != "" {
		t.Fatalf("binding upsert = %#v, want none", bindings.upsert)
	}
	if threads.upsert.ThreadID != "" {
		t.Fatalf("thread upsert = %#v, want none", threads.upsert)
	}
}
