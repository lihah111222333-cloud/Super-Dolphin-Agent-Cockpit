...
func TestThreadAgents_CleanupOnStop(t *testing.T) {
	svc := &service{
		bindingStore:  &stubThreadBindingStore{binding: &bindingstore.Binding{AgentID: "agent-1", Provider: "codex", ProviderThreadID: "provider-thread-1", CodexThreadID: "thread-1"}},
		threadStore:   &stubThreadStore{thread: &threadstore.Thread{ThreadID: "thread-1", AgentID: "agent-1", Status: statusCreated}},
