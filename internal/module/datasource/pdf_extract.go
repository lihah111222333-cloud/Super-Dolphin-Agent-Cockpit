...
func extractPDFStreamBody(content []byte, bodyStart int, dictionary []byte) ([]byte, int, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("datasource: invalid pdf stream length: %w", err)
	}
	if found {
		if length > datasourceMaxImportBytes {
			return nil, 0, errDatasourceTextTooLarge
		}
		if bodyStart > len(content) || length > len(content)-bodyStart {
			return nil, 0, errors.New("datasource: truncated pdf stream length")
		}
		bodyEnd := bodyStart + length
		markerStart := skipPDFStreamLineBreak(content, bodyEnd)
		if !bytes.HasPrefix(content[markerStart:], []byte("endstream")) {
			return nil, 0, errors.New("datasource: pdf stream length exceeds declared boundary")
		}
	...
	if streamEndRelative < 0 {
		return nil, 0, errors.New("datasource: malformed pdf stream")
	}
