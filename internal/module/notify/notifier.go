// Package notify wires the MessageNotifier contract: bus subscribers
// (or any other producer) call TryEnqueue; the flusher Runner drains
// the queue and posts via the platform/ library.
//
// This module is core-side. The orch-side sibling (cmd/mcp-orch/notify)
// exists separately per the plan's dual-Fx-tree rule so both trees can
// share the platform/ library without either importing the other.
package notify
...
func NewNotifier(logger *slog.Logger, resolver platform.Resolver, capacity int) *Notifier {
	if logger == nil {
...
func (n *Notifier) TryEnqueue(ctx context.Context, req contract.NotifyRequest) error {
	...
	if _, err := n.resolver.Resolve(alias); err != nil {
		if errors.Is(err, platform.ErrAliasNotFound) {
			return contract.ErrNotifyAliasNotFound
		}
		return err
	}
...
func (n *Notifier) resolverForFlusher() platform.Resolver { return n.resolver }
