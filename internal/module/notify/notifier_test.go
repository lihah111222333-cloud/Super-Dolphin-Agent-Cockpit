...
// testResolver implements platform.Resolver for tests without touching
// the JSON parser.
type testResolver struct {
	known map[string]platform.ChannelConfig
}

func (r *testResolver) Resolve(alias string) (platform.ChannelConfig, error) {
	if r == nil {
		return platform.ChannelConfig{}, platform.ErrAliasNotFound
	}
	c, ok := r.known[alias]
	if !ok {
		return platform.ChannelConfig{}, platform.ErrAliasNotFound
...
func newResolver(aliases ...string) platform.Resolver {
	known := map[string]platform.ChannelConfig{}
	for _, a := range aliases {
		known[a] = platform.ChannelConfig{
