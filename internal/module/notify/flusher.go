...
// default.
func NewFlusher(logger *slog.Logger, notifier *Notifier, client *platform.WebhookClient, drainTimeout time.Duration) *Flusher {
	if logger == nil {
...
func (f *Flusher) render(cfg platform.ChannelConfig, msg contract.NotifyMessage) (string, []byte, string, error) {
	switch cfg.Platform {
	case platform.PlatformDingtalk:
		return platform.RenderDingtalk(cfg, msg, f.now().UnixMilli())
	case platform.PlatformFeishu:
		return platform.RenderFeishu(cfg, msg, f.now().Unix())
	case platform.PlatformSlack:
		return platform.RenderSlack(cfg, msg)
	default:
