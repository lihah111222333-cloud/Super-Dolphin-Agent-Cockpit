...
func TestProvideConfigUsesPackageOwnedProductionTrust(t *testing.T) {
	cfg, executable, resources, target, home := productionPackageConfigFixture(t, "darwin-arm64")
	clearUpdateOverrideEnvironment(t)
	t.Setenv(envSuperDolphinHome, home)
	got, handled, err := providePackageOwnedConfigForExecutableOnPlatform(cfg, executable, "darwin-arm64")
	if err != nil {
...
func TestProvideConfigRejectsProductionUpdateOverride(t *testing.T) {
	...
	if _, _, err := providePackageOwnedConfigForExecutableOnPlatform(cfg, executable, "darwin-arm64"); err == nil || !strings.Contains(err.Error(), envUpdateGitHubRepo) {
...
func TestProvideConfigRejectsForgedRuntimeResources(t *testing.T) {
	...
	if _, _, err := providePackageOwnedConfigForExecutableOnPlatform(cfg, executable, "darwin-arm64"); err == nil || !strings.Contains(err.Error(), envRuntimeResources) {
...
func productionPackageConfigFixture(t *testing.T, platform string) (*platformconfig.Config, string, string, string, string) {
	...
	trust := recovery.PackageTrust{
		SchemaVersion: recovery.PackageTrustSchemaVersion, Enabled: true, Production: true, Platform: platform,
