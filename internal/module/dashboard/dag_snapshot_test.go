...
func TestDashboardDAGSnapshotParsesSQLiteIntegerTimestamps(t *testing.T) {
	...
	if page.DAGs[0].FinishedAt == nil || !page.DAGs[0].FinishedAt.Equal(now.Add(-10*time.Minute)) {
		t.Fatalf("DAG FinishedAt = %#v, want %s", page.DAGs[0].FinishedAt, now.Add(-10*time.Minute))
	}
	requireSnapshotLatestRun(t, page.DAGs[0], "daily-brief#run-1", true)
	if page.DAGs[0].LatestRun.FinishedAt == nil || !page.DAGs[0].LatestRun.FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("LatestRun FinishedAt = %#v, want %s", page.DAGs[0].LatestRun.FinishedAt, now.Add(-30*time.Second))
	}
	...
	if detail.Nodes[0].FinishedAt == nil || !detail.Nodes[0].FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("Node FinishedAt = %#v, want %s", detail.Nodes[0].FinishedAt, now.Add(-30*time.Second))
	}
	...
	if runs[0].FinishedAt == nil || !runs[0].FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("ListDAGRuns()[0].FinishedAt = %#v, want %s", runs[0].FinishedAt, now.Add(-30*time.Second))
	}
	...
	if run.Run.FinishedAt == nil || !run.Run.FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("GetDAGRun().Run.FinishedAt = %#v, want %s", run.Run.FinishedAt, now.Add(-30*time.Second))
	}
	if len(run.Nodes) != 1 || run.Nodes[0].LastEventAt == nil || !run.Nodes[0].LastEventAt.Equal(now) {
		t.Fatalf("GetDAGRun().Nodes = %#v, want run node with SQLite millis timestamps", run.Nodes)
	}
	if run.Nodes[0].FinishedAt == nil || !run.Nodes[0].FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("GetDAGRun().Nodes[0].FinishedAt = %#v, want %s", run.Nodes[0].FinishedAt, now.Add(-30*time.Second))
	}
}
...
func TestDashboardDAGSnapshotReadsSQLiteIntegerTimestampsThroughDBQuery(t *testing.T) {
	...
	if len(run.Nodes) != 1 || run.Nodes[0].LastEventAt == nil || !run.Nodes[0].LastEventAt.Equal(now) {
		t.Fatalf("GetDAGRun().Nodes = %#v, want run node with SQLite integer timestamps", run.Nodes)
	}
	if run.Nodes[0].FinishedAt == nil || !run.Nodes[0].FinishedAt.Equal(now.Add(-30*time.Second)) {
		t.Fatalf("GetDAGRun().Nodes[0].FinishedAt = %#v, want %s", run.Nodes[0].FinishedAt, now.Add(-30*time.Second))
	}
