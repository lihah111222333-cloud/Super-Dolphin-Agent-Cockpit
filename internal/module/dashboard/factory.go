...
const (
	...
	logFieldTraceID      logFilterField = logFilterField(pkglogger.FieldTraceID)
	logFieldSpanID       logFilterField = logFilterField(pkglogger.FieldSpanID)
	logFieldParentSpanID logFilterField = logFilterField(pkglogger.FieldParentSpanID)
...
func (p logsParams) ToFilter(source string) LogFilter {
	return LogFilter{
		...
		TraceID:      util.FirstNonEmpty(p.TraceID, p.TraceIDSnake),
		SpanID:       util.FirstNonEmpty(p.SpanID, p.SpanIDSnake),
		ParentSpanID: util.FirstNonEmpty(p.ParentSpanID, p.ParentSpanIDSnake),
...
func newSystemLogListFilter(filter LogFilter) SystemLogFilter {
	return SystemLogFilter{
		...
		SpanID:       strings.TrimSpace(filter.SpanID),
		ParentSpanID: strings.TrimSpace(filter.ParentSpanID),
