...
func TestRegisterSubscriptions_ToolCallBeginAndEnd(t *testing.T) {
	...
	if items[0].CallID != "call-1" {
		t.Fatalf("items[0].CallID = %q, want %q", items[0].CallID, "call-1")
	}
	if items[0].RequestID != 42 {
		t.Fatalf("items[0].RequestID = %d, want 42", items[0].RequestID)
	}
	...
	if items[0].Success == nil {
		t.Fatal("items[0].Success = nil, want non-nil")
	}
	if !*items[0].Success {
...
func TestRegisterSubscriptions_ToolAndApprovalShareCallID(t *testing.T) {
	...
	findByKind := func(items []timeline.Item, kind string) *timeline.Item {
		for i := range items {
	...
	svc := timeline.New(nil, nil, 50)
	dispatcher := event.NewDispatcher()
	cancels := timeline.RegisterSubscriptions(dispatcher, svc, nil, func(threadID string) {
		updated <- threadID
	})
	defer func() {
		for _, cancel := range cancels {
	...
	if approvalItem.CallID != "call-1" {
		t.Fatalf("approvalItem.CallID = %q, want %q", approvalItem.CallID, "call-1")
	}
...
func TestRegisterSubscriptions_TimelineChangesNotifyOnUpdated(t *testing.T) {
	...
	waitForCondition(t, func() bool {
		items := svc.GetByThread("t1")
		return len(items) == 1 && items[0].CallID == "call-item"
	...
	waitForCondition(t, func() bool {
		items := svc.GetByThread("t1")
		return len(items) == 2 && items[1].CallID == "call-tool"
	...
	waitForCondition(t, func() bool {
		items := svc.GetByThread("t1")
		return len(items) == 3 && items[2].CallID == "call-approval"
