...
// PDF 允许 CR、LF 或 CRLF，这里只移动分隔符长度，保持正文偏移精确。
...
// skipPDFComment 跳过以百分号开头的 PDF 注释，返回换行前的位置。
...
// simplePDFEscape 处理 PDF literal string 标准单字符转义。
...
// skipPDFEscapedLineBreak 处理反斜杠续行中的 CRLF 特例。
...
// readPDFOctalEscape 读取最多三位八进制转义并返回对应字节值。
