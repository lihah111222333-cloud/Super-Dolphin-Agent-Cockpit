...
// DAGNotifierOption configures notify behavior.
...
func (n *DAGNotifier) Stop(ctx context.Context) error {
	...
	n.stopOnce.Do(func() {
		...
		if deadline, ok := waitCtx.Deadline(); !ok || time.Until(deadline) > dagNotifyDrainGrace {
			...
			waitCtx, cancel = kernel.WithTimeout(waitCtx, dagNotifyDrainGrace)
			defer cancel()
			_ = deadline
		}
		select {
...
func (n *DAGNotifier) Run(ctx context.Context) error {
	...
	n.Start()
	<-ctx.Done()
	cleanupCtx, cancel := kernel.WithTimeout(context.Background(), dagNotifyDrainGrace)
	defer cancel()
...
func (n *DAGNotifier) processEvent(ev taskdto.TaskNodeStatusChanged) {
	...
	ctx, cancel := kernel.WithTimeout(context.Background(), dagNotifyProcessTimeout)
	defer cancel()
