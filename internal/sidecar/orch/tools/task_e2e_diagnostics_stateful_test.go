...
func (s *diagnosticStatefulService) CreateDAG(
	_ context.Context,
	req contract.CreateDAGRequest,
...
// GetDAG 返回内存中的 DAG 明细，用于诊断工具读取路径测试。
...
// ListDAGs 返回排序后的 DAG 摘要，并允许测试校验扫描 limit。
func (s *diagnosticStatefulService) ListDAGs(
	_ context.Context,
	filter contract.ListDAGsFilter,
) ([]contract.DAGSummary, error) {
...
// ApplyOps 模拟 update_dag 补丁，覆盖定时触发等持久化后的读取状态。
func (s *diagnosticStatefulService) ApplyOps(
	_ context.Context,
	req contract.ApplyOpsRequest,
) (contract.ApplyOpsResponse, error) {
...
func (s *diagnosticStatefulService) dagForApplyOps(req contract.ApplyOpsRequest) (contract.DAGDetail, error) {
	...
	if detail.DAG.Version != req.BaseVersion {
		return contract.DAGDetail{}, fmt.Errorf(
			"base version %d does not match current version %d",
			req.BaseVersion,
			detail.DAG.Version,
		)
	}
...
// StartDAG 记录启动调用和 wakeup 数，验证非法 DAG 不会进入运行态。
func (s *diagnosticStatefulService) StartDAG(
	_ context.Context,
	req contract.StartDAGRequest,
) (contract.StartDAGResponse, error) {
	s.startDAGCalls++
	...
	s.runs = append(s.runs, contract.Run{
		RunKey: runKey,
		DagKey: req.DagKey,
		Status: "running",
	})
...
// ListRuns 返回最近运行快照，用于诊断输出补充历史状态。
func (s *diagnosticStatefulService) ListRuns(
	_ context.Context,
	req contract.ListRunsRequest,
) (contract.ListRunsResponse, error) {
