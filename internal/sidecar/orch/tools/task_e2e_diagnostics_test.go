...
// TestChatCreateAutomationDAGUsesBuiltinPromptWhenDBEmpty 验证 DB 为空时聊天创建 DAG 仍使用内置 prompt。
...
// TestDiagnoseDAGPromptIdentityGapsReportsOnlyInvalidNodes 验证诊断只报告身份缺失的无效节点。
func TestDiagnoseDAGPromptIdentityGapsReportsOnlyInvalidNodes(t *testing.T) {
	...
	assertGapMissingFields(
		...
		"bad-hybrid-codex",
		"codex-review",
		"config.exec.verifier.codex_home",
		"config.exec.verifier.codex_instance_key",
		"config.exec.verifier.codex_model_provider",
	)
	assertDiagnosticRunStatusAndRemediation(t, out)
	assertDiagnosticScanStats(t, out, 4, 10, false)
}

// TestDiagnoseDAGPromptIdentityGapsSeparatesDAGScanTruncationFromGapOutput 验证扫描截断。
// 诊断输出只报告真实问题，不把扫描 limit 误当成问题数量上限。
...
// TestDiagnoseDAGPromptIdentityGapsSingleDAGDoesNotMarkScanTruncated 验证单 DAG 诊断不会误报扫描截断。
...
// TestInvalidAgentDAGCreateFailsBeforeStoreStartAndWakeup 验证 agent 节点身份缺失时创建阶段 fail-fast。
...
// TestInvalidHybridVerifierDAGCreateFailsBeforeStoreStartAndWakeup 验证 hybrid verifier 缺身份时失败。
// 创建阶段必须在持久化、启动 DAG 和派发 wakeup 前阻断。
	...
	_, err := HandleCreateDAG(svc)(context.Background(), json.RawMessage(`{
		...
		"nodes":[{
				...
				"config":{"exec":{
					...
					"verifier":{"provider":"codex","cwd":"/repo/a"}
				}}
			}]
		}`))
	if err == nil {
...
// TestTaskDiagnoseDAGPromptIdentityGapsToolRegistered 验证只读 DAG 身份诊断工具已注册。
...
func discoverCodexProvider(t *testing.T) ProviderModels {
	t.Helper()
	result, err := HandleListModels(WithModelRegistry(modelregistry.NewStaticRegistry([]ProviderModels{
...
func assertPersistedDAGNodeExec(
	t *testing.T,
	node contract.DAGNode,
	promptKey string,
	provider string,
	model string,
) {
	...
	if diagnosticExecRuntimeIdentityMissing(config) {
		t.Fatalf("persisted node exec runtime identity incomplete = %+v", config)
	}
}

func assertCreatedDAGNodeExec(
	t *testing.T,
	node contract.CreateDAGNodeRequest,
	promptKey string,
	provider string,
	model string,
) {
...
func diagnosticExecRuntimeIdentityMissing(config createdNodeExec) bool {
	return config.CWD != "/repo/a" ||
		config.CodexHome == "" ||
		config.CodexInstanceKey == "" ||
...
func assertDiagnosticRunStatusAndRemediation(t *testing.T, out DAGPromptIdentityDiagnosticsOutput) {
	...
	for _, gap := range out.Gaps {
		if gap.RecentRunStatus != "failed" {
			t.Fatalf(
				"gap %s/%s recent run status = %q, want failed",
				gap.DagKey,
				gap.NodeKey,
				gap.RecentRunStatus,
			)
	...
	if diagnosticRemediationMissing(out) {
		t.Fatalf("diagnostic remediation/read_only = %#v", out)
	}
}

func diagnosticRemediationMissing(out DAGPromptIdentityDiagnosticsOutput) bool {
	return !out.ReadOnly ||
		!strings.Contains(out.Remediation, "task_dag_apply_ops") ||
		!strings.Contains(out.Remediation, "重建")
}
