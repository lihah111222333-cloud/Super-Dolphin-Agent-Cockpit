...
func TestBaselineMetricCacheStoresParsedAST(t *testing.T) {
	...
	node, err := parser.ParseFile(fset, path, []byte(source), parser.ParseComments|parser.SkipObjectResolution)
	if err != nil {
		t.Fatalf("parse fixture: %v", err)
	}
	cache := NewBaselineMetricCache()
	if err := cache.StoreParsedFile(path, fset, node); err != nil {
		t.Fatalf("store parsed fixture: %v", err)
	}
	if got, want := cache.Measure(path), MeasureBaselineFileMetrics(path); got != want {
		t.Fatalf("cached metrics = %#v, reparsed metrics = %#v", got, want)
	}
}
...
func TestMeasureBaselineFileMetricsPreservesNonNakedMetrics(t *testing.T) {
	t.Parallel()
	path := filepath.Join(t.TempDir(), "fixture.go")
	source := "package fixture\n\nfunc run() {\n\tgo func() {}()\n}\n"
	if err := os.WriteFile(path, []byte(source), 0o644); err != nil {
	...
	regular := MeasureFileMetrics(path)
	baseline := MeasureBaselineFileMetrics(path)
	if regular.NakedGoroutines != 0 {
		t.Fatalf("MeasureFileMetrics() naked goroutines = %d, want legacy zero", regular.NakedGoroutines)
	}
	if baseline.NakedGoroutines != 1 {
		t.Fatalf("MeasureBaselineFileMetrics() naked goroutines = %d, want 1", baseline.NakedGoroutines)
	}
	baseline.NakedGoroutines = 0
	if baseline != regular {
		t.Fatalf("baseline metrics changed non-baseline fields: regular=%#v baseline=%#v", regular, baseline)
	}
}

func TestMeasureBaselineFileMetricsSkipsGeneratedNakedGoroutines(t *testing.T) {
	t.Parallel()
	...
	if got := MeasureBaselineFileMetrics(path).NakedGoroutines; got != 0 {
		t.Fatalf("generated baseline naked goroutines = %d, want 0", got)
	}
}
