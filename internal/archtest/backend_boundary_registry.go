...
func defaultBackendBoundarySurfaces() []BackendBoundarySurface {
	return []BackendBoundarySurface{
		...
		backendBoundarySurface("internal/guards", "repository-level test guard surface", nil, []BoundaryGuardID{"code_size_budget", "ignored_go_tests", "rollback_skip_markers"}),
...
func commandBoundaryPatterns(patterns backendBoundaryPatterns) []string {
	return combineBoundaryPatterns(patterns.agentRuntime, patterns.agentTerminal, patterns.codexWorktree, patterns.releaseManifest, patterns.guard, patterns.updater)
}
