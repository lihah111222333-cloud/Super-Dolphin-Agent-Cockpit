...
func TestSessionPathsDependencyGuard(t *testing.T) {
	t.Parallel()
	root := repoRootForGuardTests(t)
	dir := filepath.Join(root, "internal", "platform", "sessionpaths")
	...
	for _, entry := range entries {
		...
		violations = append(violations, sessionPathsDependencyViolations(path, entry.Name())...)
	}
	if len(violations) > 0 {
		t.Fatalf("sessionpaths must stay a leaf stdlib package:\n%s", strings.Join(violations, "\n"))
	}
}

func sessionPathsDependencyViolations(path, name string) []string {
