...
func TestOrchestrationServiceTypeConsumersUseNarrowPorts(t *testing.T) {
	...
	pkgs := loadOrchestrationServiceTypeGuardPackages(t, root)
...
func mustLoadOrchestrationServiceTypeObject(t *testing.T, root string) *types.TypeName {
	...
	pkgs, err := packages.Load(cfg, "./internal/contract")
	if err != nil {
		t.Fatalf("load internal/contract: %v", err)
	}
	if packages.PrintErrors(pkgs) > 0 {
		t.Fatalf("load internal/contract returned package errors")
	}
	if len(pkgs) != 1 {
		t.Fatalf("load internal/contract returned %d package(s), want 1", len(pkgs))
	}
	obj := pkgs[0].Types.Scope().Lookup("OrchestrationService")
	target, ok := obj.(*types.TypeName)
	if !ok {
		t.Fatalf("internal/contract.OrchestrationService = %T, want *types.TypeName", obj)
	}
	return target
}

func loadOrchestrationServiceTypeGuardPackages(t *testing.T, root string) []*packages.Package {
	t.Helper()

	cfg := &packages.Config{
		Dir:     root,
		Mode:    packages.NeedName | packages.NeedTypes | packages.NeedTypesInfo | packages.NeedSyntax | packages.NeedFiles | packages.NeedCompiledGoFiles,
		Overlay: orchestrationServiceTypeGuardOverlay(root),
		Tests:   false,
	}
	...
	if errors := packageLoadErrors(pkgs); len(errors) > 0 {
...
func orchestrationServiceTypeGuardOverlay(root string) map[string][]byte {
...
func isOrchestrationServiceTypeGuardProductionPackage(pkg *packages.Package) bool {
	if pkg == nil || pkg.PkgPath == "" {
		return false
	}
	if pkg.PkgPath == superAgentModulePath+"/internal/contract" {
		return false
	}
	if strings.HasPrefix(pkg.PkgPath, superAgentModulePath+"/internal/archtest") {
		return false
	}
	return strings.HasPrefix(pkg.PkgPath, superAgentModulePath+"/cmd/") ||
		strings.HasPrefix(pkg.PkgPath, superAgentModulePath+"/internal/")
}
...
func (i orchestrationServiceFixtureImporter) Import(path string) (*types.Package, error) {
	if path == superAgentModulePath+"/internal/contract" {
...
func typeCheckOrchestrationServiceFixturePackage(
	...
	return &packages.Package{
		...
		Types:           checked,
		TypesInfo:       info,
		CompiledGoFiles: relPaths,
	}
}

func collectOrchestrationServiceTypeUses(pkg *packages.Package, target *types.TypeName) []orchestrationServiceTypeUse {
