...
func optionalDependencyBudgets() map[optionalDependencyBudgetKey]int {
	...
	maps.Copy(budgets, threadOptionalDependencyBudgets())
	return budgets
}
...
func registeredOptionalDependencyAppClassifications() map[string]optionalDependencyClassification {
	...
	return map[string]optionalDependencyClassification{
		"internal/app/storeadapter/turn/module.go:optional_tag:provideTurnDedupeStore":                  appAdjunct("internal/app/storeadapter/turn/module.go", "turn dedupe persistence is optional; provideTurnDedupeStore returns nil so turn continues with its process-local tracker"),
