...
const (
	...
	deleteCommandCardSQL        = `DELETE FROM command_cards WHERE card_key = $1;`
	...
	listCommandCardVersionsSQL  = `SELECT id, card_key, title, description, command_template, args_schema, risk_level, enabled, created_by, updated_by, source_updated_at, created_at, archived_at FROM command_card_versions WHERE card_key = $1 ORDER BY id DESC;`
...
func scanCommandCardVersion(row rowScanner) (CommandCardVersion, error) {
	var item CommandCardVersion
	err := row.Scan(&item.ID, &item.CardKey, &item.Title, &item.Description, &item.CommandTemplate, &item.ArgsSchema, &item.RiskLevel, &item.Enabled, &item.CreatedBy, &item.UpdatedBy, &item.SourceUpdatedAt, &item.CreatedAt, &item.ArchivedAt)
	return item, err
}
...
func (q *Queries) DeleteCommandCard(ctx context.Context, cardKey string) (int64, error) {
	return q.execRows(ctx, deleteCommandCardSQL, cardKey)
}
...
func (q *Queries) ListCommandCardVersions(ctx context.Context, cardKey string) ([]CommandCardVersion, error) {
	return queryMany(ctx, q, listCommandCardVersionsSQL, scanCommandCardVersion, cardKey)
}
