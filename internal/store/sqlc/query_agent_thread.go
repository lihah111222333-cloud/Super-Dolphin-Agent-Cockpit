...
const (
	getAgentThreadByIDSQL          = `SELECT thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at, finished_at, last_event_type, error_message, workspace_run_key, owner_thread_id FROM agent_threads WHERE thread_id = $1 LIMIT 1;`
	getAgentThreadByPortSQL        = `SELECT thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at, finished_at, last_event_type, error_message, workspace_run_key, owner_thread_id FROM agent_threads WHERE port = $1 AND status = 'running' ORDER BY updated_at DESC LIMIT 1;`
	listAgentThreadsSQL            = `SELECT thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at, finished_at, last_event_type, error_message, workspace_run_key, owner_thread_id FROM agent_threads ORDER BY created_at DESC;`
	listRunningAgentsSQL           = `SELECT thread_id, port, pid, status FROM agent_threads WHERE status = 'running' ORDER BY created_at DESC;`
	listRunningAgentThreadsSQL     = `SELECT thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at, finished_at, last_event_type, error_message, workspace_run_key, owner_thread_id FROM agent_threads WHERE status = 'running' ORDER BY created_at ASC;`
	listRecoverableAgentThreadsSQL = `SELECT thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at, finished_at, last_event_type, error_message, workspace_run_key, owner_thread_id FROM agent_threads WHERE status = 'created' ORDER BY created_at ASC;`
	upsertAgentThreadSQL           = `INSERT INTO agent_threads (thread_id, prompt, model, cwd, status, port, pid, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) ON CONFLICT (thread_id) DO UPDATE SET prompt = $2, model = $3, cwd = $4, status = $5, port = $6, pid = $7, updated_at = $9;`
...
func scanAgentThread(row rowScanner) (AgentThread, error) {
	...
	err := row.Scan(&item.ThreadID, &item.Prompt, &item.Model, &item.Cwd, &item.Status, &item.Port, &item.PID, &item.CreatedAt, &item.UpdatedAt, &item.FinishedAt, &item.LastEventType, &item.ErrorMessage, &item.WorkspaceRunKey, &item.OwnerThreadID)
	return item, err
}
...
func (q *Queries) UpsertAgentThread(ctx context.Context, arg UpsertAgentThreadParams) error {
	return q.exec(ctx, upsertAgentThreadSQL, arg.ThreadID, arg.Prompt, arg.Model, arg.Cwd, arg.Status, arg.Port, arg.PID, arg.CreatedAt, arg.UpdatedAt)
}
