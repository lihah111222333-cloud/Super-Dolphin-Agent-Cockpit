...
// Provider whitelist — P1b v1 only accepts codex because the dedupe_key
// handshake on StartTurn is only implemented in the codex driver.
const (
	ProviderCodex = "codex"
)
...
type Store interface {
	...
	// claimTokensByID maps from the job id the caller wants to claim to
	// the per-job UUID v4 token generated in the application layer; the
	// store passes the token through verbatim. If fewer ids are supplied
	// than the returned count, only rows with matching ids are claimed
	// and the rest are skipped. When claimTokensByID is empty, the store
	// generates nothing and lets PostgreSQL's SKIP LOCKED arbitrate.
	ClaimDueJobs(ctx context.Context, params ClaimDueJobsParams) ([]Job, error)
...
// ClaimDueJobsParams drives ClaimDueJobs. ClaimToken is generated by the
// caller (UUID v4) and stamped onto every row claimed this batch; every
// renew/finish/release then verifies the token to prevent a preempted
// worker from overwriting state.
