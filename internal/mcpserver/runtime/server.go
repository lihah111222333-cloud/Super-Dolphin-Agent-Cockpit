...
// CwdContextKey stores the current workspace root in MCP call contexts.
const CwdContextKey = contextKey("mcp_cwd")

// ErrMissingContextCWD reports a strict MCP request missing a trusted CWD.
var ErrMissingContextCWD = errors.New("strict context enforcement: missing tool scope CWD")

// ErrMissingWorkspaceRoots reports a strict MCP request missing workspace roots.
...
// ToolProvider supplies MCP tool schemas and handles tool invocations.
...
// ToolProvider and all mcpserver/runtime internal code can reference
// it by short name without breaking the public API surface.
type MCPTool = mcpdto.MCPTool

// Server serves MCP JSON-RPC requests over the configured transport.
type Server struct {
	name      string
	version   string
