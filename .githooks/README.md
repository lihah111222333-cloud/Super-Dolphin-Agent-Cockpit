...
底层做的事：在没有其他 `core.hooksPath` 时执行 `git config --local core.hooksPath .githooks`。安装器拒绝静默覆盖已有 hooks；先显式检查并迁移它们，再重新执行安装。
...
`pre-commit` 只运行一次 `git write-tree`，并保留 Git 传入的 `GIT_INDEX_FILE` 到 Git 完成最终 commit。这个 authoritative tree OID 显式交给 closure verifier、submit/source 与 wait；wait 成功后还必须对同一 index 重读 `git write-tree` 并与初始 OID 比对。任何一步不得清除 alternate index 或回退到默认 index 重算。verifier 只从 Git tree object 提取输入，不编译或执行工作树源码。CLI 缺失、tree 解析失败、tree 不匹配或检查失败都会在提交前 fail closed。
...
- 每个 CI invocation 都从已接受的 truth image fork 恰好三个 fresh、隔离的 Docker `PlanExecution` shard/container；owner 只为完整 `max-3` gang 预留容量，超出的 invocation 按 FIFO 排队。每个 shard 固定 4 CPU、8 GiB，Docker 总预算约 25 GiB。普通任务时限 10 分钟，release 任务 30 分钟。
