...
**Verificación de entrega de la imagen de verdad.** Los [Git hooks](.githooks/README.md) versionados, las ejecuciones manuales de `make ci-l0` y `make ci-l1`, y la validación de release separan dos límites. `pre-commit` solo ejecuta el code guard del repositorio sobre el árbol staged exacto y no inicia CI remota. `pre-push`, los comandos manuales L0-L1 y la validación de release ejecutan el gate ECI remoto con rechazo ante fallos sobre el árbol pushed o commit exacto. GitHub solo es el remoto de Git; no proporciona un runner de CI. La falta de configuración remota, procedencia, autoridad del resultado o evidencia de limpieza, así como cualquier gate de push/release fallido, rechaza la entrega. `commit-msg` sigue exigiendo texto de commit en chino y evidencia de fix-test.
