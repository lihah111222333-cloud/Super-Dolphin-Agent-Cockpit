...
if [[ "$goos" != "linux" ]]; then
  echo "prepare_lsp_bundle_linux.sh must run on Linux; current GOOS=$goos" >&2
  exit 1
fi
...
if [[ -n "$node_src" ]]; then
  ...
  cp "$(command -v node)" "$lsp_dir/node/bin/node"
else
  echo "missing node; set SUPER_DOLPHIN_NODE_DIST to a Node runtime" >&2
  exit 1
fi
...
if [[ "$lsp_profile" == "full" ]]; then
  ...
  if [[ -z "$jdtls_home" ]]; then
    if command -v jdtls >/dev/null 2>&1; then
      jdtls_home="$(cd "$(dirname "$(command -v jdtls)")/.." && pwd)"
    else
      echo "missing jdtls; set SUPER_DOLPHIN_JDTLS_HOME" >&2
      exit 1
    fi
  fi
  if [[ -z "$jdk_home" ]]; then
    if [[ -n "${JAVA_HOME:-}" && -x "$JAVA_HOME/bin/java" ]]; then
      ...
      echo "missing JDK; set SUPER_DOLPHIN_JDK_HOME or JAVA_HOME" >&2
      exit 1
    fi
  fi
...
cache_root="${HOME:-${TMPDIR:-/tmp}}/.cache/jdtls"
cwd_name="$(basename "$PWD")"
data_hash="$(printf '%s' "$cwd_name" | sha1sum | awk '{print $1}')"
...
echo "==> Linux LSP bundle ready: $lsp_dir"
du -sh "$lsp_dir"
