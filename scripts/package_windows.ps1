...
for %%E in (mcp-orch.exe mcp-lsp.exe mcp-ida.exe codex.exe gopls.exe go.cmd typescript-language-server.cmd vscode-css-language-server.cmd pyright-langserver.cmd rust-analyzer.exe bash-language-server.cmd shellcheck.exe sg.exe) do (
...
function Package-WindowsMain() {
    ...
    try {
        & go build -o (Join-Path $Root 'bin/mcp-orch.exe') './cmd/mcp-orch'
        if ($LASTEXITCODE -ne 0) { throw 'go build mcp-orch failed' }
        & go build -o (Join-Path $Root 'bin/mcp-lsp.exe') './cmd/mcp-lsp'
        if ($LASTEXITCODE -ne 0) { throw 'go build mcp-lsp failed' }
        & go build -ldflags '-H=windowsgui' -o (Join-Path $Root 'bin/agent-terminal.exe') './cmd/agent-terminal'
        if ($LASTEXITCODE -ne 0) { throw 'go build agent-terminal failed' }
        & go build -o (Join-Path $Root 'bin/mcp-ida.exe') './cmd/mcp-ida'
        if ($LASTEXITCODE -ne 0) { throw 'go build mcp-ida failed' }
    } finally {
        Pop-Location
    }
