...
func writeSQLiteReleaseGateUnsignedPackage(t *testing.T) sqliteReleaseGateUnsignedPackage {
	t.Helper()
	stageDir := t.TempDir()
	root := filepath.Join(stageDir, fmt.Sprintf("super-dolphin-0.1.0-%s-%s", runtime.GOOS, runtime.GOARCH))
	return writeSQLiteReleaseGateUnsignedPackageWithLayout(t, sqliteReleaseGateUnsignedPackage{
		root:        root,
		entrypoint:  filepath.Join(root, "bin", "agent-terminal"),
		launchers:   []string{filepath.Join(root, "run.sh")},
		binaryNames: []string{"agent-terminal", "mcp-orch", "mcp-lsp", "mcp-ida", "codex", "gopls"},
	}, false)
}

func sqlitePackageSmokeCommandTarget(stage sqliteReleaseGateUnsignedPackage) string {
	return stage.launchers[0]
}
