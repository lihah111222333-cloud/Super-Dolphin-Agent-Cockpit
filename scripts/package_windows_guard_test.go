...
func TestPackageWindowsScriptBuildsNativeWindowsPackage(t *testing.T) {
	...
	assertScriptContains(t, script, "go build -ldflags '-H=windowsgui' -o (Join-Path $Root 'bin/agent-terminal.exe') './cmd/agent-terminal'")
	assertScriptContains(t, script, "go build -o (Join-Path $Root 'bin/mcp-orch.exe') './cmd/mcp-orch'")
	assertScriptContains(t, script, "go build -o (Join-Path $Root 'bin/mcp-lsp.exe') './cmd/mcp-lsp'")
	assertScriptContains(t, script, "go build -o (Join-Path $Root 'bin/mcp-ida.exe') './cmd/mcp-ida'")
	...
	assertScriptOrder(t, script, "Build-CurrentFrontendApp", "go build -ldflags '-H=windowsgui' -o (Join-Path $Root 'bin/agent-terminal.exe') './cmd/agent-terminal'")
}
...
func TestPackageWindowsBuildsDesktopExeWithoutConsoleWindow(t *testing.T) {
	...
	assertScriptContains(t, script, "go build -ldflags '-H=windowsgui' -o (Join-Path $Root 'bin/agent-terminal.exe') './cmd/agent-terminal'")
	assertScriptDoesNotContain(t, script, "go build -o (Join-Path $Root 'bin/agent-terminal.exe') './cmd/agent-terminal'")
}
