...
func TestPackageWindowsScriptUsesIncrementalBuildPhaseCache(t *testing.T) {
	...
	assertScriptContains(t, script, "function Invoke-BuildPhaseCache")
	assertScriptContains(t, script, "function Test-BuildPhaseCache")
	assertScriptContains(t, script, "function Save-BuildPhaseCache")
	assertScriptContains(t, script, "'./scripts/build_phase_cache'")
	assertScriptContains(t, script, "shared artifact cache hit, restored")
	assertScriptContains(t, script, "foreach ($variableName in @('GOOS', 'GOARCH', 'CGO_ENABLED'))")
	assertScriptContains(t, script, "[Environment]::SetEnvironmentVariable($variableName, $null, 'Process')")

	assertScriptContains(t, script, "$frontendCacheOutputs = @((Join-Path $Root 'frontend-app/dist'))")
	assertScriptContains(t, script, "Test-BuildPhaseCache -Name 'frontend' -Paths $frontendCachePaths -Inputs $frontendCacheInputs -Outputs $frontendCacheOutputs")
	assertScriptContains(t, script, "Save-BuildPhaseCache -Name 'frontend' -Paths $frontendCachePaths -Inputs $frontendCacheInputs -Outputs $frontendCacheOutputs")
	...
	assertScriptContains(t, script, "Test-BuildPhaseCache -Name 'go-binaries' -Paths $goBinaryCachePaths -Inputs $goInputs -Outputs $goBinaryCacheOutputs")
	assertScriptContains(t, script, "$goBinaryCachePaths = @(")
	assertScriptContains(t, script, "$goBinaryCacheOutputs = @(")
	assertScriptContains(t, script, "(Join-Path $Root 'go.mod')")
	assertScriptContains(t, script, "(Join-Path $Root 'go.sum')")
	assertScriptContains(t, script, "'GOOS=windows',")
	assertScriptContains(t, script, "\"GOARCH=$WindowsPackageArch\",")
	assertScriptContains(t, script, "\"CGO_ENABLED=$env:CGO_ENABLED\",")
	assertScriptContains(t, script, "\"CGO_CPPFLAGS=$env:CGO_CPPFLAGS\",")
	assertScriptContains(t, script, "\"CGO_CFLAGS=$env:CGO_CFLAGS\",")
	assertScriptContains(t, script, "\"CGO_CXXFLAGS=$env:CGO_CXXFLAGS\",")
	assertScriptContains(t, script, "\"CGO_LDFLAGS=$env:CGO_LDFLAGS\",")
...
func TestPackageWindowsFrontendBuildCacheKeyIncludesAllViteInputs(t *testing.T) {
	...
	assertScriptContains(t, script, "Join-Path $Root 'frontend-app/recovery.html'")
	...
	assertScriptContains(t, script, "Join-Path $Root 'scripts/build_phase_cache/main.go'")
...
func TestPackageWindowsScriptFindsInnoCompiler(t *testing.T) {
	...
	assertScriptContains(t, script, "Windows installer requested but Inno Setup iscc.exe was not found")
	assertScriptDoesNotContain(t, script, "Windows installer skipped: Inno Setup iscc.exe not found")
