...
const DOMAIN_DESCRIPTIONS = {
  'app-ui': '桌面应用、Wails host、当前 React/Vite 前端、legacy Vue/Vite 前端与 UI 测试',
...
const PURPOSE_RULES = [
  ['frontend-app/src/pages/chat/components/', '当前 React 聊天页组件、composer、runtime panel 与 timeline 展示'],
  ['frontend-app/src/pages/chat/hooks/', '当前 React 聊天页交互 hooks、composer 本地交互和线程数据读取'],
  ['frontend-app/src/pages/chat/adapters/', '当前 React 聊天页 UI 状态适配器'],
  ['frontend-app/src/pages/chat/services/', '当前 React 聊天页后端 API 薄服务'],
  ['frontend-app/src/pages/chat/', '当前 React 聊天页与工作区布局'],
  ['frontend-app/src/entities/client/model/', '当前 React 客户端 Zustand 状态、thread/turn/composer 运行链'],
  ['frontend-app/src/shared/api/', '当前 React 前端 RPC facade、Wails bridge 与契约矩阵'],
  ['frontend-app/src/pages/', '当前 React 页面层'],
  ['frontend-app/src/services/', '当前 React 页面服务与后端 API 适配'],
  ['frontend-app/src/app/', '当前 React app shell 状态模型'],
  ['frontend-app/src/', '当前 React/Vite 前端源码'],
  ['frontend-app/', '当前 React/Vite 前端工程、脚本、测试和构建配置'],
  ...
  ['cmd/agent-terminal/frontend/', 'legacy Vue/Vite 前端包与 frontend-app dist 嵌入输出'],
  ...
  ['internal/module/thread/promptrouting/', 'thread prompt 模板纯路由规则与 sections 转换'],
...
function moduleDescription(module) {
  return {
    ...
    'frontend-app': '当前 React/Vite 前端工程',
