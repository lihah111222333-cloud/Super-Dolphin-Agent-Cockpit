...
1. 记录 `git status --short`、HEAD、对象和不审范围，不回退或混入用户 dirty 文件。
...
按改动面选择维度，每个适用维度至少取一个导航入口和一个源码、测试或门禁证据。

| 维度 | 关键风险 | 最小证据 |
|---|---|---|
| D01 架构边界 | cmd/app/contract/dto/module/platform/provider/mcpserver/store/pkg 越界 | codemap、调用方向、archtest |
...
| D08 Skill/Memory/Prompt/Thread | canonical `.agents/skills`、mirror、snapshot、fork/resume 漂移 | validator、module 测试、codemap 11/12 |
...
验证匹配改动面：Go 跑受影响包/guard，SQL 跑 `make sqlc-verify`，skill 跑 `python3 scripts/validate_super_agent_skills.py` 与 `git diff --check`，当前前端跑 lint/test/build。diagnostics 不是测试，旧 binary、空日志或单次 PASS 不是完成证据。

```text
severity | dimension | file:line_start-line_end | problem | evidence | fix
```

Findings 按 P0-P3 置顶。无 finding 时列覆盖维度、验证和残余风险；工具失败记录 action、work_dir、目标、错误与收窄重试，不得写成 PASS。
