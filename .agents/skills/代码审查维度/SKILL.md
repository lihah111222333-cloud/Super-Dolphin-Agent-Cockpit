...
- 边界治理 Task 的代码变动控制：收敛任务必须同步履行 D18（实现简化/DRY），优先清理冗余/废弃路径；单任务手写生产代码新增不得超过 500 行，超过必须二次裁决与拆分。
- 多维度治理拆分门禁：单次治理任务禁止跨 5 个以上维度打包实施；扫描出多维度缺口时，必须按“维度/模块”拆分为独立 Task 分批实施。
- 单任务改动文件数上限：单一收敛/治理 Task 修改的生产文件数不得超过 15 个；超过 15 个文件时必须阻断并拆分为小步原子 Task。
