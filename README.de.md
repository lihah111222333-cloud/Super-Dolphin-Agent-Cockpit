...
**Truth-Image-Lieferungsprüfung.** Versionierte [Git Hooks](.githooks/README.md), manuelle `make ci-l0`- und `make ci-l1`-Aufrufe sowie die Release-Prüfung trennen zwei Grenzen. `pre-commit` führt nur den Repository-Code-Guard für den exakt gestagten Baum aus und startet keine Remote-CI. `pre-push`, die manuellen L0-L1-Aufrufe und die Release-Prüfung führen das ausfallgeschlossene Remote-ECI-Gate für den exakten Push-Baum oder Commit aus. GitHub ist nur das Git-Remote und stellt keinen CI-Runner bereit. Fehlende Remote-Konfiguration, Provenienz, Ergebnisautorität oder Aufräumnachweise sowie ein fehlgeschlagenes Push-/Release-Gate weisen die Lieferung zurück. `commit-msg` verlangt weiterhin chinesischen Commit-Text und Fix-Test-Nachweise.
