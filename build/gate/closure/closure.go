...
func renderDockerfile(lock toolchainLock, runtimeDeps runtimeDepsLock, buildFiles, sourceSnapshotFiles []string) ([]byte, error) {
    ...
    test -n "$MAIN_COMMIT" && test -n "$BUILD_SOURCE_TREE" && test -n "$ACCEPTED_SNAPSHOT_ID" && test -n "$GATE_SOURCE_DIGEST" && test -n "$RUNTIME_DEPENDENCY_DIGEST" && test -n "$IMAGE_INPUT_DIGEST" && test -n "$POLICY_DIGEST" && test -n "$TOOLCHAIN_DIGEST" && test -n "$TARGET_PLATFORM"; \
    ...
    rm -rf /out/go-build-cache /out/go-cache-metrics /out/generation-one-receipts; \
    mkdir -p /out/go-build-cache /out/go-cache-metrics /out/generation-one-receipts; \
    ...
    record_provision_check() { check=$1; started_ms=$2; completed_ms=$3; duration_ms=$4; compile_ms=$5; compile_na=$6; printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$check" "$started_ms" "$completed_ms" "$duration_ms" "$compile_ms" "$compile_na" >> /out/generation-one-receipts/observations.tsv; }; \
    compile_phase() { check=$1; shift; printf "[gate-image] compile begin phase=%s\n" "$check"; started_ms=$(date +%s%3N); compile_go "$check" "$@"; completed_ms=$(date +%s%3N); log_go_cache "$check"; duration_ms=$((completed_ms-started_ms)); test "$duration_ms" -gt 0; record_provision_check "$check" "$started_ms" "$completed_ms" "$duration_ms" "$duration_ms" false; printf "[gate-image] compile complete phase=%s elapsed_ms=%s test_body=not_applicable\n" "$check" "$duration_ms"; if test "$duration_ms" -gt 100000; then printf "[gate-image] warning phase=%s exceeds_target_ms=100000 elapsed_ms=%s action=warn_and_continue\n" "$check" "$duration_ms"; fi; }; \
    compile_phase gate_build env CGO_ENABLED=0 go build -mod=mod -trimpath -buildvcs=false -o /out/super-dolphin-gate ./cmd/super-dolphin-gate; \
    printf "[gate-image] dependency begin\n"; dependency_started_ms=$(date +%s%3N); rm -rf frontend-app/node_modules frontend-app/dist; env npm_config_cache=/opt/super-dolphin-gate/runtime/frontend/npm-cache npm_config_offline=true npm --prefix frontend-app ci --ignore-scripts --no-audit --no-fund; dependency_completed_ms=$(date +%s%3N); dependency_duration_ms=$((dependency_completed_ms-dependency_started_ms)); test "$dependency_duration_ms" -gt 0; test -d frontend-app/node_modules; record_provision_check dependency "$dependency_started_ms" "$dependency_completed_ms" "$dependency_duration_ms" 0 true; printf "[gate-image] dependency complete elapsed_ms=%s test_body=not_applicable\n" "$dependency_duration_ms"; if test "$dependency_duration_ms" -gt 100000; then printf "[gate-image] warning phase=dependency exceeds_target_ms=100000 elapsed_ms=%s action=warn_and_continue\n" "$dependency_duration_ms"; fi; \
    printf "[gate-image] frontend_build begin\n"; frontend_started_ms=$(date +%s%3N); (cd frontend-app && ./node_modules/.bin/vite optimize --force); test -s frontend-app/node_modules/.vite/deps/_metadata.json; env npm_config_cache=/opt/super-dolphin-gate/runtime/frontend/npm-cache npm_config_offline=true npm --prefix frontend-app run build; test -s frontend-app/dist/index.html; test -s cmd/agent-terminal/web-dist/index.html; frontend_completed_ms=$(date +%s%3N); frontend_duration_ms=$((frontend_completed_ms-frontend_started_ms)); test "$frontend_duration_ms" -gt 0; record_provision_check frontend_build "$frontend_started_ms" "$frontend_completed_ms" "$frontend_duration_ms" "$frontend_duration_ms" false; printf "[gate-image] frontend_build complete elapsed_ms=%s test_body=not_applicable\n" "$frontend_duration_ms"; if test "$frontend_duration_ms" -gt 100000; then printf "[gate-image] warning phase=frontend_build exceeds_target_ms=100000 elapsed_ms=%s action=warn_and_continue\n" "$frontend_duration_ms"; fi; \
    mkdir -p /out/frontend-node-modules /out/vite-cache /out/frontend-embed; cp -a frontend-app/node_modules/. /out/frontend-node-modules/; cp -a frontend-app/node_modules/.vite/. /out/vite-cache/; rm -rf /out/frontend-node-modules/.vite /out/frontend-node-modules/.vite-temp; cp -a cmd/agent-terminal/web-dist/. /out/frontend-embed; \
    ...
    python3 -c 'import json,sys;commit,tree,gate,runtime_digest,runtime_image,image_input,policy,toolchain,platform=sys.argv[1:];checks=["gate_build","dependency","frontend_build","normal_compile","e2e_compile","race_compile"];manifest={"schema_version":"generation-one-compiled-seed/v1","main_commit":commit,"source_tree":tree,"gate_source_digest":gate,"runtime_dependency_digest":runtime_digest,"runtime_image":runtime_image,"image_input_digest":image_input,"policy_digest":policy,"toolchain_digest":toolchain,"platform":platform,"checks":checks};open("/out/compiled-seed-manifest.json","w",encoding="utf-8").write(json.dumps(manifest,separators=(",",":"),sort_keys=True))' "$MAIN_COMMIT" "$BUILD_SOURCE_TREE" "$GATE_SOURCE_DIGEST" "$RUNTIME_DEPENDENCY_DIGEST" "$RUNTIME_DEPS_IMAGE" "$IMAGE_INPUT_DIGEST" "$POLICY_DIGEST" "$TOOLCHAIN_DIGEST" "$TARGET_PLATFORM"; \
...
RUN --network=none set -eu; \
    ...
    verify_compiled_seed() { \
      python3 -c 'import json,sys;path,commit,tree,gate,runtime_digest,runtime_image,image_input,policy,toolchain,platform=sys.argv[1:];m=json.load(open(path,encoding="utf-8"));keys={"schema_version","main_commit","source_tree","gate_source_digest","runtime_dependency_digest","runtime_image","image_input_digest","policy_digest","toolchain_digest","platform","checks"};set(m)==keys or (_ for _ in ()).throw(SystemExit("compiled seed manifest fields mismatch"));expected=("generation-one-compiled-seed/v1",commit,tree,gate,runtime_digest,runtime_image,image_input,policy,toolchain,platform);actual=tuple(m[name] for name in ("schema_version","main_commit","source_tree","gate_source_digest","runtime_dependency_digest","runtime_image","image_input_digest","policy_digest","toolchain_digest","platform"));actual==expected or (_ for _ in ()).throw(SystemExit("compiled seed identity mismatch"));m["checks"]==["gate_build","dependency","frontend_build","normal_compile","e2e_compile","race_compile"] or (_ for _ in ()).throw(SystemExit("compiled seed check plan mismatch"))' /out/compiled-seed-manifest.json "$MAIN_COMMIT" "$BUILD_SOURCE_TREE" "$GATE_SOURCE_DIGEST" "$RUNTIME_DEPENDENCY_DIGEST" "$RUNTIME_DEPS_IMAGE" "$IMAGE_INPUT_DIGEST" "$POLICY_DIGEST" "$TOOLCHAIN_DIGEST" "$TARGET_PLATFORM"; \
    }; \
    ...
    postprocess_step generation-one-receipt write_generation_one_receipt; \
...
RUN --network=none set -eu; \
    ...
    chmod -R a-w /opt/super-dolphin-gate/frontend-embed /opt/super-dolphin-gate/source-snapshot /opt/super-dolphin-gate/source-baseline.git /opt/super-dolphin-gate/runtime/frontend/node_modules /opt/super-dolphin-gate/runtime/frontend/vite-cache; \
