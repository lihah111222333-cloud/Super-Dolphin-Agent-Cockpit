...
func renderDockerfile(lock toolchainLock, runtimeDeps runtimeDepsLock, buildFiles []string) ([]byte, error) {
	...
	output.WriteString("    record_refresh_check() { check=$1; started_ms=$2; completed_ms=$3; duration_ms=$4; compile_ms=$5; compile_na=$6; printf '%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n' \"$check\" \"$started_ms\" \"$completed_ms\" \"$duration_ms\" \"$compile_ms\" \"$compile_na\" >> /out/refresh-receipts/observations.tsv; printf \"" + cicontract.RefreshCheckLogPrefix + "%s started_at_unix_ms=%s completed_at_unix_ms=%s duration_ms=%s plan_digest=%s source_tree=%s refresh_only=true test_body=not_applicable\\n\" \"$check\" \"$started_ms\" \"$completed_ms\" \"$duration_ms\" \"$IMAGE_INPUT_DIGEST\" \"$BUILD_SOURCE_TREE\"; }; \\\n")
	output.WriteString("    compile_phase() { check=$1; shift; started_ms=$(date +%s%3N); compile_go \"$check\" \"$@\"; completed_ms=$(date +%s%3N); log_go_cache \"$check\"; duration_ms=$((completed_ms-started_ms)); test \"$duration_ms\" -gt 0; record_refresh_check \"$check\" \"$started_ms\" \"$completed_ms\" \"$duration_ms\" \"$duration_ms\" false; printf \"[gate-image] compile phase=%s elapsed_ms=%s test_body=not_applicable\\n\" \"$check\" \"$duration_ms\"; if test \"$duration_ms\" -gt 100000; then printf \"[gate-image] warning phase=%s exceeds_target_ms=100000 elapsed_ms=%s action=warn_and_continue\\n\" \"$check\" \"$duration_ms\"; fi; }; \\\n")
	...
	output.WriteString("    python3 -c 'import hashlib,json,sys;tree,snapshot,plan=sys.argv[1:];required=(\"gate_build\",\"normal_compile\",\"e2e_compile\",\"race_compile\",\"dependency\",\"frontend_build\");records=[];lines=open(\"/out/refresh-receipts/observations.tsv\",encoding=\"utf-8\").read().splitlines();len(lines)==len(required) or (_ for _ in ()).throw(SystemExit(\"refresh check observation count mismatch\"));[records.append((lambda check,started,completed,duration,compile_ms,compile_na:{\"check\":check,\"executed\":True,\"passed\":True,\"source_tree\":tree,\"accepted_snapshot_id\":snapshot,\"plan_digest\":plan,\"started_at_unix_ms\":started,\"completed_at_unix_ms\":completed,\"duration_ms\":duration,\"candidate_compile_ms\":compile_ms,\"candidate_compile_not_applicable\":compile_na,\"test_body_not_applicable\":True,\"receipt_sha256\":\"sha256:\"+hashlib.sha256((check+\"\\0true\\0true\\0\"+tree+\"\\0\"+snapshot+\"\\0\"+plan+\"\\0\"+str(started)+\"\\0\"+str(completed)+\"\\0\"+str(duration)+\"\\0\"+str(compile_ms)+\"\\0\"+str(compile_na).lower()+\"\\0true\").encode()).hexdigest()})(line.split(\"\\t\")[0],*map(int,line.split(\"\\t\")[1:5]),line.split(\"\\t\")[5]==\"true\")) for line in lines if len(line.split(\"\\t\"))==6];tuple(item[\"check\"] for item in records)==required or (_ for _ in ()).throw(SystemExit(\"refresh check order mismatch\"));[(item[\"started_at_unix_ms\"]>0 and item[\"completed_at_unix_ms\"]>item[\"started_at_unix_ms\"] and item[\"duration_ms\"]==item[\"completed_at_unix_ms\"]-item[\"started_at_unix_ms\"] and item[\"test_body_not_applicable\"]) or (_ for _ in ()).throw(SystemExit(\"refresh check timing mismatch\")) for item in records];open(\"/out/refresh-receipts/refresh-build-receipt.json\",\"w\",encoding=\"utf-8\").write(json.dumps({\"schema_version\":2,\"source_tree\":tree,\"accepted_snapshot_id\":snapshot,\"plan_digest\":plan,\"refresh_receipts\":records},separators=(\",\",\":\")))' \"$BUILD_SOURCE_TREE\" \"$ACCEPTED_SNAPSHOT_ID\" \"$IMAGE_INPUT_DIGEST\"; \\\n")
