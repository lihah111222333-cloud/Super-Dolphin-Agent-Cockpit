...
| 共享库位置（已拍板） | `internal/module/notify/shared/{webhook,resolver,render,dingtalk,feishu,slack}.go` [NEW] | 共享 webhook 客户端 / resolver / 模板 / 平台签名；放在 `internal/module` 子包会被 `internal/archtest/dependency_direction_mcp_orch_test.go` 的白名单放行，orch 侧 import 不撞护栏。**禁止** 新建 `internal/platform/notify` 顶层包（那会触发 archtest 护栏改动，本期不采）。 |
...
| 解析器 | `internal/module/notify/shared/resolver.go` [NEW] | 业务层只传 `channelAlias`；resolver 内部把 alias 解析成 `{Platform, URL, Secret}` |
...
- **双树同构原则**：core Fx 与 `cmd/mcp-orch` Fx 各自有一棹 bus / run.Group；两树同时 import 同一套共享库（放在 `internal/module/notify/shared/*` 以命中 archtest 白名单，见上文“archtest 白名单事实”），后在各自业务 module 里装一套 subscriber + runner。两树同构不要求“只留 core 或只留共享库”这类互斥裁剪，且互不用跨树订阅来盖对方事件。
...
> 三个模板实现时一律在 `internal/module/notify/shared/render.go` 走统一 `markdownEscape` / 长度截断 / mention 抓找。禁止在每平台实现里再写一遍。
...
- worker 通过 `Runner.Run(ctx)` 进入 `runner.actors`；shutdown 只依赖 `ctx cancel + bus 停派发`，不在 `fx` 里额外写 cancel worker 逻辑。v1 采用 **bounded drain 5s**：超时仍未发出的 webhook 直接 drop，并记指标；不承诺 durable retry/WAL。
