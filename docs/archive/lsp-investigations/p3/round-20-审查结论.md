...
- `cmd/mcp-lsp/tools/tool_coderun.go`（code_run/code_run_test 工具：sandbox 执行、snippet 准备、Go/JS/TS 语言支持、project_cmd）
- `cmd/mcp-lsp/tools/tool_edit.go`（edit 工具：patch 解析、replace_range、LSP sync、editEnvelope）
- `cmd/mcp-lsp/tools/tool_structure.go`（structure 工具：document_symbol/workspace_symbol/folding_range/semantic_tokens）
- `cmd/mcp-lsp/tools/tool_completion.go`（completion 工具：LSP 补全、compact list）
...
下一轮范围建议：
- `cmd/mcp-lsp/search/`（搜索引擎实现：text_search、ast_search、path resolution）
- `cmd/mcp-lsp/manager/`（LSP manager 生命周期、语言适配器）
