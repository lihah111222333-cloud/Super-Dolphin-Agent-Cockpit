...
- `cmd/mcp-lsp/middleware/budget.go`（output budget 限制、overflow envelope 构造）
- `cmd/mcp-lsp/middleware/budget_hints.go`（per-tool overflow hint 配置、summary 提取）
- `cmd/mcp-lsp/middleware/recovery.go`（panic recovery middleware）
- `cmd/mcp-lsp/middleware/timeout.go`（tool timeout middleware、callWithRecover、ClampTimeout）
- `cmd/mcp-lsp/middleware/logging.go`（Handler/Middleware 类型定义、Chain、Logging middleware）
...
- 或 `cmd/mcp-lsp/tools/tool_xref.go` + `tool_grep.go`（具体工具实现的入参校验）
