...
下一轮范围建议：
- `cmd/mcp-lsp/tools/tool_xref.go`（xref 工具实现：references/call_hierarchy/type_hierarchy）
- `cmd/mcp-lsp/tools/tool_grep.go`（grep 工具实现：text_search/ast_search）
- `cmd/mcp-lsp/tools/tool_file.go`（file 工具实现：open_file/read_file/diagnostics）
