# T08 local/standalone DAG agent launcher 契约：明确禁止或补齐兼容实现
...
建议实现 worktree：`.worktrees/mcp-orch-dag-t08`
...
`cmd/mcp-orch/fx.go, orchestration/launch_helpers.go, service_launcher_bridge.go, launcher.go, nodeexec/executor_agent.go, tests`

## 必须满足的验收标准

- local/standalone 模式下 DAG agent node 不能以缺 `Command` 的 LaunchRequest 走到晦涩 validation；要么启动期/launch 时 fail-fast 明确说明 DAG agent 需要 remote launcher，要么补齐本地 command/thread_id/spawn writeback 契约。
- 如果选择 fail-fast，错误要包含 dag_key/node_key 与修复建议，不要生成半成品 run。
- 如果选择实现 local 兼容，必须写回 thread_id/spawning_thread_id 并复用现有 lifecycle 语义。
- 测试覆盖 remote 与 local 分支，保证当前桌面 remote 路径不回退。

## 明确非目标

- 不要触碰 provider runtime 泛化预检，范围只限 DAG agent launcher contract。
