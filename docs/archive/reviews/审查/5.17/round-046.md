...
1. **[major] command_card_versions 表存在，但 Upsert/Delete 不自动归档当前版本**
   - 证据：store 暴露 `InsertVersion()` / `ListVersions()`（`cmd/mcp-orch/store/commandcard/contract.go:35-58`），SQL 也有 `InsertCommandCardVersion`（`cmd/mcp-orch/sql/queries/command_card.sql:10-21`）。但 `Upsert()` 只执行 `UpsertCommandCard()`（`cmd/mcp-orch/store/commandcard/store.go:27-44`），`Delete()` 只执行 `DeleteCommandCard()`（`cmd/mcp-orch/store/commandcard/store.go:58-60`），没有读取旧值并写 version。
...
2. **[major] automation DAG 只存 `command_ref`，运行时读取当前 command card，不固定版本**
   - 证据：`AutomationExecConfig` 只有 `CommandRef` 和 args，没有 version/id/hash（`cmd/mcp-orch/orchestration/nodeexec/config.go:91-99`）。executor 每次 `loadCommandCard()` 都按 trimmed command_ref 读取当前卡片（`cmd/mcp-orch/orchestration/nodeexec/executor_automation.go:261-272`）。
...
3. **[major] command_card_runs 统计表被 List 查询使用，但 executor 没有写入运行记录**
   - 证据：`ListCommandCards` 聚合 `command_card_runs` 计算 `last_run_at/run_count`（`cmd/mcp-orch/sql/queries/command_card.sql:39-54`）。全文只找到该表被查询，没有 insert 运行记录；executor 只 `RunCommandCard()` 后 finalize outcome（`cmd/mcp-orch/orchestration/nodeexec/executor_automation.go:144-148`）。
...
4. **[major] DeleteCommandCard 没有 DAG 引用保护**
   - 证据：`Delete()` 直接按 card_key 删除当前 command card（`cmd/mcp-orch/store/commandcard/store.go:58-60`；SQL 在 `cmd/mcp-orch/sql/queries/command_card.sql:6-8`）。automation node config 使用 `command_ref` 字符串引用卡片（`cmd/mcp-orch/orchestration/nodeexec/config.go:91-99`）。
...
5. **[moderate] command_list 暴露完整 command_template 给调用方**
   - 证据：`commandCardDTO` 包含 `CommandTemplate`（`cmd/mcp-orch/tools/command_tools.go:23-38`），`command_list` 映射也返回该字段（`cmd/mcp-orch/tools/command_tools.go:95-120`）。
...
- disabled command card 会在 executor 层 hard fail，不会继续执行（`cmd/mcp-orch/orchestration/nodeexec/executor_automation.go:268-271`）。
- command_get 对 not found 做了用户可读错误转换（`cmd/mcp-orch/tools/parity_v2_test.go:160-176`）。
- command_list 固定使用 `resourceListLimit=50`，不是用户可控无上限列表（`cmd/mcp-orch/tools/command_tools.go:13`、`cmd/mcp-orch/tools/command_tools.go:65-77`）。

## 验证

```bash
./scripts/test_with_guard.sh ./cmd/mcp-orch/store/commandcard ./cmd/mcp-orch/tools ./cmd/mcp-orch/orchestration/nodeexec ./internal/store/commandcard -count=1
```
