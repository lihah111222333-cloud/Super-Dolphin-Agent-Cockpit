...
1. **[major] `inputs.from_sharedfiles` 只有全局 path，没有 dag/run/node scope**
   - 证据：`RunContext.SharedFileReader` 的接口只有 `ReadSharedFile(ctx, path)`，没有 dag_key/run_id/node_key 参数（`cmd/mcp-orch/orchestration/nodeexec/types.go:133-149`）。router 只把 reader 原样注入 `RunContext`（`cmd/mcp-orch/orchestration/node_router.go:191-198`），agent 与 automation executor 都按配置里的 path 直接读取（`cmd/mcp-orch/orchestration/nodeexec/inputs.go:228-257`；`cmd/mcp-orch/orchestration/nodeexec/executor_automation.go:527-555`）。sharedfile store 本身也只以 `Path` 作为领域键（`cmd/mcp-orch/store/sharedfile/contract.go:18-35`）。
   - 风险：并发量化 run 若复用 `reports/out.log`、`plan.md` 等路径，下游节点可能读取另一个 run 或另一个 DAG 的共享文件。`from_nodes` 已通过 `ListTaskDagRunNodes(dag_key, run_id)` 做运行实例隔离（`cmd/mcp-orch/sql/queries/task_dag_node_read.sql:8-13`），但 sharedfile 输入没有同等级隔离。
...
2. **[major] `from_nodes` 上游未 done 会被诊断成 unknown node_key**
   - 证据：`prefetchPrevResults()` 在找到目标上游但 status 非 `done` 时选择跳过，不填 `PrevResults`（`cmd/mcp-orch/orchestration/node_router.go:263-281`）。随后 `loadFromNodes()` 对缺失 key 返回 `from_nodes references unknown node_key`（`cmd/mcp-orch/orchestration/nodeexec/inputs.go:187-221`）。测试也把非 done 上游描述为触发 unknown node_key validation（`cmd/mcp-orch/orchestration/node_router_shard18_test.go:159-190`）。
...
3. **[major] sharedfile 已存在时 agent 输出 materialization 复用旧内容**
   - 证据：subscriber 在写 `outputs.to_sharedfile` 前先 `configuredSharedfileAlreadyExists()` 读取同 path（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:300-324`）。如果 exists=true，只写 result 引用并保留旧内容，日志说明 “preserve existing content”（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:316-323`）。
...
4. **[moderate] sharedfile writer 审计身份固定为 `node-router`**
   - 证据：适配器写入时固定 `UpdatedBy: sharedFileWriterUpdatedBy`，常量值为 `"node-router"`（`cmd/mcp-orch/orchestration/sharedfile_adapter.go:70-95`）。注释也说明生产 `RunContext` 暂未带节点级身份（`cmd/mcp-orch/orchestration/sharedfile_adapter.go:20-24`）。
...
5. **[moderate] automation sharedfile 写入错误被归类为 validation**
   - 证据：automation executor 的 `writeAutomationSharedfile()` 对 `SharedFileWriter.WriteSharedFile` 返回的任何错误，都生成 `FailureClassValidation`（`cmd/mcp-orch/orchestration/nodeexec/executor_automation.go:561-581`）。而 sharedfile 适配器可能返回 DB、磁盘、路径校验等不同类型错误（`cmd/mcp-orch/orchestration/sharedfile_adapter.go:83-96`）。
...
- `inputs.from_nodes` 已有同 run 隔离测试，`runB` 不会读到 `runA` 的上游结果（`cmd/mcp-orch/orchestration/node_router_shard18_test.go:106-157`）。
- router 现在要求 wakeup 必须带 `run_id`，不会回退读模板节点（`cmd/mcp-orch/orchestration/node_router.go:284-325`）。
- sharedfile reader 对 not found 做了三态区分，不会把缺文件当基础设施错误（`cmd/mcp-orch/orchestration/sharedfile_adapter.go:44-63`）。

## 验证

```bash
./scripts/test_with_guard.sh ./cmd/mcp-orch/orchestration ./cmd/mcp-orch/orchestration/nodeexec ./cmd/mcp-orch/store/sharedfile ./internal/store/sharedfile -count=1
```
