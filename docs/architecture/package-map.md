...
Migration target: 非入口库包逐步迁入 `internal/sidecar/lsp/*` 或更具体的职责目录。迁移完成后，`cmd/mcp-lsp` 只保留 `main.go`、Fx 装配和命令层进程控制。

Notes: 边界守卫已把 `cmd/mcp-lsp/*` 非根包识别为 `sidecar` 过渡层；AST 守卫当前仍把 `cmd/**` 当作 command 层处理，因此后续迁移到 `internal/sidecar/lsp` 后必须同步收紧直接 `slog`、`panic`、`os.Exit` 和 `fmt.Print*`。

### `cmd/mcp-orch`

Responsibility: Orchestration sidecar 进程根入口，负责 DAG/MCP 编排服务启动、独立 store 装配、任务调度 runner 和 peer 通信。

Allowed imports: `cmd/mcp-orch/*` 过渡 sidecar 库包、`internal/platform/*`、`internal/contract`、`internal/dto`、`pkg/logger`。

Forbidden imports: `internal/provider/*` 的宿主实现细节、`internal/ui/*`、桌面主进程私有状态。

Public entry points: `main`、Fx module/provider、sidecar runtime wiring。根目录文件只做进程级装配，业务流程进入 `orchestration` 或后续 `internal/sidecar/orch/*`。

### `cmd/mcp-orch/*` 过渡 sidecar 库

Responsibility: 当前承载 orchestration、DAG、wakeup、notify、sidecar store、MCP tools、workspace 等编排服务内部能力。

Allowed imports: 同一 sidecar 内相邻包、`internal/platform/*`、`internal/contract`、`internal/dto`、`pkg/*` 中明确允许的稳定 API。

Forbidden imports: `cmd/mcp-lsp/*`、`internal/ui/*`、`internal/provider/*` 的宿主具体实现、桌面主进程装配对象。

Migration target: 非入口库包逐步迁入 `internal/sidecar/orch/*`。其中持久化实现应迁到 `internal/sidecar/orch/store/*` 或明确保留为 sidecar-local store，不能混入根 `internal/store/*` 的桌面主进程 store。
...
Allowed imports: `internal/contract`、`internal/dto`、必要的 `internal/platform` 叶子能力、同包子目录、标准库。

Forbidden imports: `cmd/*`、`internal/provider/*`、`internal/mcpserver/*`、`internal/ui/*`、store 具体 SQL 类型、数据库驱动、直接日志实现。

Public entry points: Service 接口、命令/查询对象、结果 DTO、Fx `Module`。具体 service、缓存、worker 状态默认不导出。

Notes: 当前 `internal/module/*` 混合了 domain 与 application 职责。新增或大规模迁移时，优先在子包中拆出纯规则、use case、port，而不是继续扩大单包文件数。
...
Allowed imports: `internal/contract`、`internal/dto`、`internal/platform/db`、生成的 sqlc 包、标准库和数据库驱动。
...
Allowed imports: 标准库、第三方基础设施库、`internal/contract` 或纯 DTO 中的稳定契约。
...
Allowed imports: 标准库、轻量无副作用 DTO。
...
- `pkg/logger`: 当前日志实现和兼容入口。长期目标是把核心实现迁入 `internal/platform/logging`，`pkg/logger` 保留薄兼容层；迁移前它是 AST 守卫允许直接接触 `slog` 的例外包。
