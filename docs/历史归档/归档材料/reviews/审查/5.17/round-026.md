...
- `internal/sidecar/orch/orchestration/nodeexec/executor_agent_inputs_test.go`
...
1. **[critical] `inputs.summarization.max_tokens` 只是字段位，agent 和 automation 输入路径都不执行裁剪或摘要**
   - 证据：`InputsConfig` 包含 `Summarization *SummarizationConfig`，注释称“输入摘要/裁剪策略字段位”（`internal/sidecar/orch/orchestration/nodeexec/config.go:24-39`）。但 agent 的 `collectInputSections()` 只读取 `FromNodes` 和 `FromSharedfiles`，随后把原始结果拼入 prompt（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:154-179`、`internal/sidecar/orch/orchestration/nodeexec/inputs.go:199-257`）；automation 的 `buildInputsPayload()` 同样只注入完整 prev/sharedfile 内容（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:479-496`）。`rg` 未发现任何 `Summarization` 执行逻辑，只有 round-trip 测试（`internal/sidecar/orch/orchestration/nodeexec/config_test.go:34-91`）。
...
2. **[major] agent 输入用裸文本 section 拼接，没有内容边界转义，sharedfile/prev result 可伪造后续 `[first_turn]` 或输入段落**
   - 证据：`loadFromNodes()` 直接写 `## node:<key>` 后追加 raw `json.RawMessage`（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:199-219`）；`loadFromSharedfiles()` 直接写 `## sharedfile:<path>` 后追加文件 content（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:237-255`）；`composePrompt()` 再用 `[first_turn]` 分隔真实指令（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:260-279`）。没有 fenced block、length prefix、JSON envelope 或 escaping。
...
3. **[major] `from_nodes` 依赖上游 `task_dag_nodes.result`，但上游 sharedfile-only 输出会注入 `(empty)` 而不是自动解引用 sharedfile**
   - 证据：router 预取只复制 done 节点的 `Result` 列；若 `Result` 长度为 0，就填 nil（`internal/sidecar/orch/orchestration/node_router.go:259-280`）。agent 输入装载遇到 nil result 会写 `(empty)` 并继续 launch（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:211-219`），测试明确锁定“上游可能未配 outputs.to_node_result”时继续执行（`internal/sidecar/orch/orchestration/nodeexec/executor_agent_inputs_test.go:317-343`、`internal/sidecar/orch/orchestration/node_router_shard18_test.go:192-225`）。
...
4. **[major] automation 与 agent 的输入语义不一致：同一 `from_nodes` 在 automation 中变成 JSON 对象，在 agent 中变成原文 prompt**
   - 证据：automation 的 `collectPrevResults()` 会尝试 `json.Unmarshal`，成功后注入 `__inputs.from_nodes.<key>` 对象；解析失败才退回原始字符串（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:499-524`）。agent 的 `loadFromNodes()` 不解析 JSON，直接把 raw bytes 写进 prompt（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:199-219`）。测试分别锁定 automation 看到 object（`internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go:229-277`）和 agent prompt 包含原文 JSON（`internal/sidecar/orch/orchestration/nodeexec/executor_agent_inputs_test.go:37-77`）。
...
5. **[moderate] 空字符串路径和空 node_key 被静默跳过，配置错误可能退化成空输入而非失败**
   - 证据：`loadFromNodes()` 对 trim 后空 key 直接 `continue`（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:201-205`）；`loadFromSharedfiles()` 对空 path 也 `continue`（`internal/sidecar/orch/orchestration/nodeexec/inputs.go:239-243`）；automation 同样跳过空 key/path（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:503-507`、`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:537-541`）。如果数组只含空字符串，`inputsConfigured()` 仍为 true，但最终 sections/injected 为空，可能继续执行。
...
- `from_nodes` 已按 run 隔离：router 按当前 `runID` 列节点并只填目标 run 的结果，测试覆盖 run A/run B 不串线（`internal/sidecar/orch/orchestration/node_router_shard18_test.go:106-157`）。
- 未 done 的上游节点不会被注入：router 过滤非 done，nodeexec 随后 validation fail，测试覆盖 running 上游被拒绝（`internal/sidecar/orch/orchestration/node_router.go:263-280`、`internal/sidecar/orch/orchestration/node_router_shard18_test.go:159-190`）。
- sharedfile reader 未注入或文件不存在会 fail-loud，测试覆盖 nil reader 与 missing path（`internal/sidecar/orch/orchestration/nodeexec/executor_agent_inputs_test.go:244-315`、`internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go:329-372`）。
...
结果：Go guard、`internal/archtest`、`internal/sidecar/orch/orchestration` 与 `internal/sidecar/orch/orchestration/nodeexec` 通过。
