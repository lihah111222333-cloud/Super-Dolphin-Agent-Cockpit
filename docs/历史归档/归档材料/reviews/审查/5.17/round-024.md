...
- `internal/sidecar/orch/store/taskdag/store_node_spawn_test.go`
...
1. **[critical] `task_dag_runs.events` 只记录“重试覆盖 spawning_thread_id”，不是完整 DAG 审计事件流**
   - 证据：`RecordNodeSpawn()` 注释和实现只在旧 `spawning_thread_id` 非空且与新 thread 不同时 append `node_spawn`（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:20-31`、`internal/sidecar/orch/store/taskdag/store_node_spawn.go:90-98`）。测试明确首次 spawn 不写 events（`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:32-63`）。状态变化、dispatch、turn bound、retry、replan planner 启动、finalize 等路径没有统一 append run events。
...
2. **[major] 首次 spawn 不写 `node_spawn` 事件，run events 无法列出每个节点实际启动的第一个 child thread**
   - 证据：`recordNodeSpawnTx()` 在 `PreviousThreadID == ""` 时直接返回，不 append event（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:90-96`）。测试 `TestRecordNodeSpawn_FirstSpawn_WritesFieldNoEvents` 锁定 run.events 为空（`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:32-63`）。
...
3. **[major] run events 固定只保留最近 50 条，高频重试会丢失早期审计证据**
   - 证据：`AppendTaskDagRunEvent` SQL 在 append 后只保留最近 50 条事件（`internal/sidecar/orch/sql/queries/task_dag_run.sql:208-224`）。测试 `TestRecordNodeSpawn_EventsRingTrim_KeepsLastFifty` 明确 60 次 spawn 后只保留 50 条，最早保留的是 thread-10→thread-11（`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:256-300`）。
...
4. **[major] spawn 事件 append 对“run 不存在/非 running”是软失败，会覆盖节点 thread 但不留下 run 级审计**
   - 证据：`appendNodeSpawnEvent()` 对 `AppendTaskDagRunEvent` 返回 `pgx.ErrNoRows` 直接 nil，不报错（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:116-128`）。测试 `TestRecordNodeSpawn_RetryWithoutRunningRun_SoftMiss` 锁定覆盖成功、`AppendedEvent=false`、`RunKey=""`（`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:123-157`）。SQL 只更新 `status='running' AND id=$3` 的 run（`internal/sidecar/orch/sql/queries/task_dag_run.sql:214-230`）。
...
- `RecordNodeSpawn` 已经是 run-scoped：SQL 要求 `run_id=$4 AND $4>0`，测试覆盖同一 DAG 两个 run 的 thread 和 event 不串线（`internal/sidecar/orch/sql/queries/task_dag_node_spawning_thread.sql:16-43`、`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:159-206`）。
- `AppendTaskDagRunEvent` 已修复 JSON object concat 被误当 append 的问题，使用 `jsonb_build_array($2::jsonb)` 强制数组 append（`internal/sidecar/orch/sql/queries/task_dag_run.sql:198-206`）。本轮不报告 object merge 覆盖。
- `FinalOutputFileFromRunMetadata` 兼容 legacy `sharedfile.path` envelope（`internal/contract/dag_final_output_test.go:31-41`）。

## 验证

```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/store/taskdag ./internal/sidecar/orch/orchestration ./internal/sidecar/orch/orchestration/nodeexec ./internal/contract ./internal/module/dashboard ./internal/module/memory -count=1
```

结果：Go guard、`internal/archtest`、`internal/sidecar/orch/store/taskdag`、`internal/sidecar/orch/orchestration`、`internal/sidecar/orch/orchestration/nodeexec`、`internal/contract`、`internal/module/dashboard` 与 `internal/module/memory` 通过。
