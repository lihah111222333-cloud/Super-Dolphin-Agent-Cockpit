...
- `internal/sidecar/orch/store/taskdag/store_update_running_status_test.go`
- `internal/sidecar/orch/store/taskdag/store_lookup_by_spawning_thread_test.go`

## Findings

1. **[critical] `BindRunningNodeTurn` 有持久化实现但未接生产事件流，active turn 链路基本悬空**
   - 证据：store 提供 `BindRunningNodeTurn()`，在一个事务中绑定 wakeup 和 node active turn（`internal/sidecar/orch/store/taskdag/store.go:181-210`），SQL 要求 node 处于 `running`、`active_turn_id IS NULL` 且 `active_wakeup_id` 匹配 wakeup（`internal/sidecar/orch/sql/queries/task_dag_node_runtime.sql:1-13`）。但全仓 `rg` 只发现定义和测试，没有生产调用点；turn lifecycle 只更新 in-memory agent state（`internal/sidecar/orch/orchestration/turn_lifecycle.go:18-42`），DAG subscriber 反查也只靠 `spawning_thread_id`（`internal/sidecar/orch/orchestration/dag_turn_completed_subscriber.go:171-202`）。
...
2. **[major] agent launch、`spawning_thread_id`、running 状态、wakeup sent 是分散写入，任何一步失败都会产生半绑定节点**
   - 证据：AgentExecutor launch 后通过 `RecordNodeSpawn()` 写 `spawning_thread_id`（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:46-70`），router 随后 `UpdateRunningNodeStatus()` 写 running 和 `active_wakeup_id`（`internal/sidecar/orch/orchestration/node_router.go:379-440`），dispatcher 再 `MarkWakeupSent()`（`internal/sidecar/orch/orchestration/wakeup_dispatcher.go:278-293`）。这些写入不在同一事务，且 running 写失败只 warn/metric 后仍返回成功 outcome（`internal/sidecar/orch/orchestration/node_router.go:398-439`）。
...
3. **[major] turn.completed 仅按 `spawning_thread_id` 反查，无法用 active wakeup/run 做强校验**
   - 证据：DAG subscriber 对 `ev.ThreadID` 调 `LookupNodesBySpawningThread()`，然后遍历所有返回节点推进终态（`internal/sidecar/orch/orchestration/dag_turn_completed_subscriber.go:171-202`）。store 注释说明 N>1 是“normal on retry / recovery chains”，调用方会逐个推进（`internal/sidecar/orch/store/taskdag/store.go:149-157`）；测试覆盖同一 thread id 可返回多行（`internal/sidecar/orch/store/taskdag/store_lookup_by_spawning_thread_test.go:94-142`）。
...
4. **[major] `UpdateRunningNodeStatus` 不校验 assigned_to 或 wakeup 目标 agent，错误 dispatcher 可把节点置 running**
   - 证据：SQL 只按 dag_key、node_key、run_id、status pending/ready 更新 running，并写 `active_wakeup_id`（`internal/sidecar/orch/sql/queries/task_dag_node_runtime.sql:28-36`）；store 调用也只传这些字段（`internal/sidecar/orch/store/taskdag/store.go:228-242`）。没有校验 wakeup.target_agent_id 与 node.assigned_to。
...
5. **[moderate] thread.stopped fallback 对所有非终态/非 awaiting_verify 节点都 fail，可能误杀仍在绑定间隙的 ready 节点**
   - 证据：fallback 通过 `LookupNodesBySpawningThread()` 找到节点后，除 done/failed/cancelled/skipped/awaiting_verify 外都会 `FailNodeAndCancelDownstream()`（`internal/sidecar/orch/orchestration/hook_consumer.go:410-470`）。`CompleteTaskDagNode` 又允许 ready→done 以覆盖 race window（`internal/sidecar/orch/sql/queries/task_dag_node_runtime.sql:47-61`），说明 ready 状态下也可能收到 completion。
...
- running 状态更新的 SQL fence 已允许 pending/ready，避免 ready 根节点无法进入 running 的历史问题，测试覆盖该点（`internal/sidecar/orch/store/taskdag/store_update_running_status_test.go:11-90`）。
- `RecordNodeSpawn()` 拒绝空 thread id，避免覆盖已有有效 thread id（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:46-60`）。
- subscriber 在节点已终态时会跳过，重复 turn.completed 不会直接覆盖终态（`internal/sidecar/orch/orchestration/dag_turn_completed_subscriber.go:28-41`、`internal/sidecar/orch/orchestration/dag_turn_completed_subscriber.go:215-230`）。
