...
- `internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go`
...
- `internal/sidecar/orch/store/sqlc/command_card.sql.go`
...
2. **[critical] automation 节点执行时只按 `command_ref` 拉当前 command card，没有绑定创建/启动时的版本快照**
   - 证据：`AutomationExecutor.loadCommandCard()` 每次执行只 `GetCommandCard(ctx, commandRef)`（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:261-272`）。生产 getter 也是通过 `command_get` 工具读取当前 store 记录（`cmd/mcp-orch/fx.go:225-250`、`internal/sidecar/orch/tools/command_tools.go:79-93`）。store 虽然有 `InsertVersion/ListVersions`，但 executor 和 DAG node config 只保存 `exec.command_ref`，没有 version id/hash（`internal/sidecar/orch/store/commandcard/contract.go:35-58`、`internal/sidecar/orch/sql/queries/command_card.sql:10-21`）。
...
3. **[major] `args_schema` 只暴露给 UI/工具，不参与 automation 执行校验**
   - 证据：command card DTO/store 包含 `ArgsSchema`（`internal/sidecar/orch/tools/command_tools.go:23-38`、`internal/sidecar/orch/store/commandcard/contract.go:18-26`），但 `renderCommandTemplate()` 只把 `cfg.Exec.Args` 解成 `map[string]any` 并执行模板（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:339-367`）。`AutomationCommandCard.ArgsSchema` 在 runner 路径中没有被读取，相关测试只覆盖缺 key template error，不覆盖 schema validation（`internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go:55-89`）。
...
4. **[major] `risk_level` 不参与调度或审批，只有 disabled 会阻断执行**
   - 证据：`AutomationCommandCard` 有 `RiskLevel` 字段（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:36-43`），store/list 也返回 risk_level（`internal/sidecar/orch/sql/queries/command_card.sql:39-54`）。但 `loadCommandCard()` 只检查 `card.Enabled`，没有根据 `RiskLevel` 做人工确认、策略降级或工具限制（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:261-272`）。
...
5. **[major] 非零退出统一归类 hard，可能跳过 transient/retry 策略**
   - 证据：shell 命令非零退出被包装为 `CommandExitError`（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:82-100`），`classifyAutomationError()` 对任何 `CommandExitError` 直接返回 `FailureClassHard`，不再检查 stderr/stdout 或 exit code（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:290-313`）。测试也锁定 nonzero hard（`internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go:552-572`）。
   - 风险：curl/git/npm/回测脚本因为网络、锁等待、临时限流或外部服务 503 退出非零时，会被标成 hard permanent。dispatcher 对 unmapped hard failure 不走 retry，直接 fail-fast（`internal/sidecar/orch/orchestration/wakeup_dispatcher_shard17_smart_retry_test.go:207-238`），导致可恢复失败中断整条 DAG。
...
- `missingkey=error` 已避免模板缺字段静默渲染为空（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:354-360`）。
- disabled command card 会被 hard failure 阻断，不会继续执行 runner（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:261-272`）。
- unsupported automation kind 已 fail-loud，当前只允许 command_card（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:226-247`、`internal/sidecar/orch/orchestration/nodeexec/executor_automation_test.go:91-114`）。

## 验证

```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/orchestration ./internal/sidecar/orch/orchestration/nodeexec ./internal/sidecar/orch/store/commandcard ./internal/sidecar/orch/tools -count=1
```

结果：Go guard、`internal/archtest`、`internal/sidecar/orch/orchestration`、`internal/sidecar/orch/orchestration/nodeexec`、`internal/sidecar/orch/store/commandcard` 与 `internal/sidecar/orch/tools` 通过。
