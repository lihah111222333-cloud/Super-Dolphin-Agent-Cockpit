...
- `internal/sidecar/orch/orchestration/dag_query_test.go`
...
1. **[critical] `task_get_dag` 的 DAG DTO 不含 `version`，调用方无法从权威读模型取得 `ApplyOps.base_version`**
   - 证据：`DAGSummary` 字段只有 id/key/title/description/status/created_by/metadata/time，没有 version（`internal/contract/orchestration.go:387-399`）。`loadDAGDetail()` 返回 `dagSummaryDTO(*dag)` 和模板节点（`internal/sidecar/orch/orchestration/dag.go:305-315`），`dagSummaryDTO()` 也没有 version 映射（`internal/sidecar/orch/orchestration/dag.go:391-405`）。同时 `ApplyOpsRequest` 要求 `BaseVersion`（`internal/contract/orchestration.go:281-285`），工具 schema 也将 base_version 设为必填（`internal/sidecar/orch/tools/task_tools.go:397-401`）。
...
2. **[major] `task_get_run` 返回 `dag_version_snapshot`，但当前 StartDAG 写入恒为 0，读模型会传播错误审计值**
   - 证据：`Run` DTO 暴露 `DagVersionSnapshot`（`internal/contract/orchestration.go:319-334`），`dagRunDTO()` 原样映射该字段（`internal/sidecar/orch/orchestration/dag_query.go:144-160`），测试也断言该字段会被返回（`internal/sidecar/orch/orchestration/dag_query_test.go:109-120`）。但 Round 020 已确认 `dagVersionFor()` 当前固定返回 0（`internal/sidecar/orch/orchestration/scheduler.go:220-225`）。
...
3. **[major] `task_list_runs` 只返回 run header，不带节点进度/失败摘要，监控面无法直接区分卡住类型**
   - 证据：`ListRuns()` 仅调用 `runStore.ListRuns` 并返回 `[]Run`（`internal/sidecar/orch/orchestration/dag_query.go:104-122`）。SQL 只查 `task_dag_runs` 行，不聚合 runtime nodes（`internal/sidecar/orch/sql/queries/task_dag_run.sql:18-27`）。测试只覆盖 run slice、status filter 和 limit（`internal/sidecar/orch/orchestration/dag_query_test.go:341-490`）。
...
4. **[major] `task_get_run` 返回 runtime nodes，但没有包含 DAG 模板当前 version，调用方无法判断 run 快照与当前模板是否漂移**
   - 证据：`GetRunResponse` 只有 `Run` 和 `Nodes`（`internal/contract/orchestration.go:300-309`），`getRunResponse()` 只读 run row 和 `ListRunNodes`（`internal/sidecar/orch/orchestration/dag.go:360-373`）。它不会同时读取当前 DAG template 或 version。
...
- `task_get_run` 已经按 run_id 返回 runtime nodes，而不是模板节点；`getRunResponse()` 要求 `RunNodeReadStore` 并调用 `ListRunNodes(dagKey, run.ID)`（`internal/sidecar/orch/orchestration/dag.go:360-373`），测试覆盖了 runtime node 返回（`internal/sidecar/orch/orchestration/dag_query_test.go:136-180`）。
- `task_list_runs` 有默认 limit=50 和上限 200，避免无界扫描（`internal/sidecar/orch/orchestration/dag_query.go:85-116`、`internal/sidecar/orch/orchestration/dag_query_test.go:411-490`）。本轮不报告列表无分页上限。
- `Run` DTO 对 `budget_limit` 指针做防御拷贝，测试已覆盖（`internal/sidecar/orch/orchestration/dag_query_test.go:122-134`）。

## 验证

```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/orchestration ./internal/sidecar/orch/store/taskdag ./internal/sidecar/orch/tools -count=1
```

结果：Go guard、`internal/archtest`、`internal/sidecar/orch/orchestration`、`internal/sidecar/orch/store/taskdag` 与 `internal/sidecar/orch/tools` 通过。
