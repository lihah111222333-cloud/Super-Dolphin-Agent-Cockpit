...
1. **[critical] `command_card_runs` 表有完整执行历史 schema，但当前执行路径没有任何写入**
   - 证据：schema 定义了 `command_card_runs(card_key, requested_by, params, rendered_command, risk_level, status, requires_review, output, error, exit_code, executed_at...)`（`migrations/0001_initial_schema.sql:171-187`、`migrations/001_baseline.sql:88-96`）。但 `rg` 只找到 `ListCommandCards` 聚合读取该表，没有 insert/update query 或 store 方法；`AutomationExecutor` 执行命令后只返回 `AutomationCommandResult`，不写 run 表（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:59-89`、`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:144-148`）。
...
2. **[major] dashboard 的 command card `run_count/last_run_at` 来自永远不写的聚合，容易显示为 0 / nil**
   - 证据：`ListCommandCards` 通过 `command_card_runs` 子查询聚合 `MAX(created_at)` 与 `COUNT(*)`（`internal/sidecar/orch/sql/queries/command_card.sql:39-54`）。`internal/sidecar/orch/store/commandcard` 和 `internal/store/commandcard` 都只是映射 `LastRunAt/RunCount`（`internal/sidecar/orch/store/commandcard/store.go:107-123`、`internal/store/commandcard/store.go:41-57`）。dashboard commands 页面直接调用 reader list（`internal/module/dashboard/ui_page.go:154-201`、`internal/module/dashboard/rpc.go:182-188`）。
...
3. **[major] `command_card_runs.status/requires_review` 暗示审批流，但 DAG automation 完全绕过该状态机**
   - 证据：run 表默认 `status='pending_review'` 且 `requires_review=true`（`migrations/0001_initial_schema.sql:171-187`）。实际 `AutomationExecutor.loadCommandCard()` 只检查 `card.Enabled`，没有创建 pending_review run、等待批准或推进 executed/failed 状态（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:261-272`）。
...
4. **[major] 执行结果只可能落到 DAG node.result 或 sharedfile，无法按 command card 聚合失败率和输出**
   - 证据：`AutomationCommandResult` 包含 `CardKey/ExitCode/Stdout/Stderr/Command/Args`（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:46-53`），但只有 `finalizeAutomationOutcome()` 决定写 node.result 或 sharedfile（`internal/sidecar/orch/orchestration/nodeexec/executor_automation.go:151-172`）。command card store 没有 run 级 CRUD（`internal/sidecar/orch/store/commandcard/contract.go:35-42`）。
...
```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/store/commandcard ./internal/store/commandcard ./internal/module/dashboard ./internal/sidecar/orch/orchestration/nodeexec -count=1
```

结果：Go guard、`internal/archtest`、`internal/sidecar/orch/store/commandcard`、`internal/store/commandcard`、`internal/module/dashboard` 与 `internal/sidecar/orch/orchestration/nodeexec` 通过。
