...
- `internal/sidecar/orch/orchestration/archive.go`
- `internal/sidecar/orch/orchestration/archive_test.go`
- `internal/sidecar/orch/orchestration/stop_helper_test.go`
...
1. **[major] DAG 完成后的 spawned agent stop 是 best-effort，失败不重试也不阻塞 DAG 终态**
   - 证据：subscriber 在 DB advance 后调用 `stopSpawnedAgentForSubscriber()`；该函数调用 `StopSpawnedAgent()` 后仅 warn，不向上返回错误（`internal/sidecar/orch/orchestration/dag_turn_completed_subscriber.go:661-682`）。`StopSpawnedAgent()` 注释也明确调用方不得传播错误（`internal/sidecar/orch/orchestration/stop_helper.go:96-114`）。
...
2. **[major] lookup 失败会跳过 stop，且没有补偿任务**
   - 证据：`resolveAgentIDForStop()` 在 `AgentThreadLookup.GetByThreadID` 报非 not-found 错误时返回 `StopResultSkippedLookupFailed`，不调用 StopAgent（`internal/sidecar/orch/orchestration/stop_helper.go:161-172`）。测试锁定 lookup failed 不调用 StopAgent（`internal/sidecar/orch/orchestration/stop_helper_test.go:237-253`）。
...
3. **[major] ArchiveAgent 远端 archive 成功后跳过本地 thread/binding archived 标记**
   - 证据：`ArchiveAgent()` 若 `stopArchiveTarget()` 返回 `remoteArchived=true`，则不会调用 `archivePersistedArchiveTarget()`（`internal/sidecar/orch/orchestration/archive.go:30-47`）。测试 `TestArchiveAgentArchivesOwningRuntimeWhenCalledWithProviderThreadID` 和 `TestArchiveAgentInvokesLauncherArchiveNotStop` 断言 remote archive 后 `UpdateStatus`/`SetArchived` 调用为 0（`internal/sidecar/orch/orchestration/archive_test.go:140-207`）。
...
4. **[major] ArchiveAgent 先停 runtime 再写 DB，DB 归档失败会留下“运行时已停、本地未归档”的半状态**
   - 证据：ArchiveAgent 先 `stopArchiveTarget()`，随后才 `archivePersistedArchiveTarget()`（`internal/sidecar/orch/orchestration/archive.go:30-47`）。`archivePersistedArchiveTarget()` 中 thread UpdateStatus 或 binding SetArchived 任一失败会返回错误（`internal/sidecar/orch/orchestration/archive.go:80-117`）。
...
5. **[moderate] StopSpawnedAgent metrics 仍是本地 snapshot，未进入 Prometheus**
   - 证据：`StopSpawnedAgentCounters()` 只是包内 atomic snapshot（`internal/sidecar/orch/orchestration/stop_metric.go:95-105`）；Prometheus DAG collector 只导出 dispatch retry 指标（`internal/platform/metrics/dag.go:9-62`）。
...
- stop helper 对 already stopped / already archived 做了幂等分类，避免把正常重复清理记为失败（`internal/sidecar/orch/orchestration/stop_helper.go:240-263`）。
- ArchiveAgent 已经优先走 `ArchiveAgent` recycle path，工具层在服务支持时不会退回裸 StopAgent（`internal/sidecar/orch/tools/orchestration_tools.go:221-244`）。
- StopSpawnedAgent 的七类结果有单元测试覆盖，分类本身不是本轮问题；问题在补偿和监控出口不足。

## 验证

```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/orchestration ./internal/sidecar/orch/tools ./internal/platform/metrics -count=1
```
