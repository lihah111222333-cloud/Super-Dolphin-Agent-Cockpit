...
1. **[critical] agent launch 成功后 ready→running 写失败被吞，wakeup 仍会标记 sent，导致 DAG 节点与真实子线程脱钩**
   - 证据：`dispatchAgent()` 在 child agent launch 成功后调用 `advanceAgentNodeToRunning()`，但该函数对通用 DB 错误只记录 warn/metric 并返回 false，不把错误传回 dispatcher（`internal/sidecar/orch/orchestration/node_router.go:372-440`）。测试明确锁定通用 DB 错误“不传播回 RouteByWakeup”（`internal/sidecar/orch/orchestration/dispatch_agent_running_test.go:102-139`）。dispatcher 对非 failed outcome 默认调用 `markLaunched()`，把 wakeup 标记为 `sent`（`internal/sidecar/orch/orchestration/node_executor_dispatch.go:302-341`、`internal/sidecar/orch/orchestration/wakeup_dispatcher.go:278-293`）。
...
2. **[major] `RecordNodeSpawn` 不限制节点状态，迟到的 spawn 写回可以覆盖终态节点的 `spawning_thread_id`**
   - 证据：`UpdateTaskDagNodeSpawningThread` 的注释明确 “WHERE 不限定 status”，SQL 仅按 `dag_key/node_key/run_id` 更新（`internal/sidecar/orch/sql/queries/task_dag_node_spawning_thread.sql:16-43`）。store 入口只校验 dag/node/thread/run 必填，不校验当前 status（`internal/sidecar/orch/store/taskdag/store_node_spawn.go:46-70`）。现有测试只覆盖输入必填、重试覆盖和事件环形截断，没有终态拒绝用例（`internal/sidecar/orch/store/taskdag/store_node_spawn_test.go:219-330`）。
...
3. **[major] `task_dispatch_node` 的 assign 和 enqueue 非事务化，失败会留下“已指派但无 wakeup”的 ready 节点**
   - 证据：`DispatchNode()` 先 list 检查状态，再 `AssignNode()`，再 `EnqueueWakeup()`；代码注释明确“不在事务里跑 list + assign + enqueue”（`internal/sidecar/orch/orchestration/dag_dispatch.go:37-56`）。`AssignTaskDagNode` 会在 `pending/ready` 状态写 `assigned_to`（`internal/sidecar/orch/sql/queries/task_dag_node_write.sql:36-46`），`enqueueManualDispatchWakeup()` 失败时直接返回错误，不回滚前面的 assigned_to（`internal/sidecar/orch/orchestration/dag_dispatch.go:129-164`）。
...
4. **[moderate] worker lease acquire 不校验空 owner/target，可能出现空 owner 抢占或续租**
   - 证据：`AcquireWorkerLease()` / `RenewWorkerLease()` 直接把 `TargetAgentID`、`OwnerID` 传给 SQL，没有 trim/required 校验（`internal/sidecar/orch/store/taskdag/store_lease.go:9-35`）。SQL 以 `target_agent_id` 为唯一冲突键，允许同 owner 续租或过期抢占（`internal/sidecar/orch/sql/queries/task_dag_worker_lease.sql:1-15`）。
...
- `ClaimDueWakeups` 使用 `FOR UPDATE SKIP LOCKED` 且 Mark/Retry/Fail 都带完整 claim fence，已有测试覆盖 stale claim 不能提交（`internal/sidecar/orch/sql/queries/task_dag_wakeup_dispatch.sql:6-40`、`internal/sidecar/orch/store/taskdag/store_fencing_test.go:18-130`）。
- downstream 自动调度的 idempotency key 已包含 run_id，避免多 run 下同 dag/node 去重互相污染（`internal/sidecar/orch/store/taskdag/store_complete_downstream.go:298-305`）。
- `UpdateRunningNodeStatus` 已允许 pending/ready -> running 且拒绝 running/done 重写，这是有测试保护的（`internal/sidecar/orch/sql/queries/task_dag_node_runtime.sql:28-36`、`internal/sidecar/orch/store/taskdag/store_update_running_status_test.go:11-90`）。
