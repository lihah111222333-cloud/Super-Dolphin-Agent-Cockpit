...
1. **[critical] `sent` 但未绑定 turn 的 wakeup 没有回收机制，会让 DAG 节点永久等待**
   - 证据：成功 launch 后 dispatcher 只调用 `MarkWakeupSent()` 把 wakeup 改成 `sent`（`internal/sidecar/orch/orchestration/wakeup_dispatcher.go:278-293`）；`BindTaskDagWakeupTurn` 只在 status=`sent` 且 `bound_turn_id IS NULL` 时绑定（`internal/sidecar/orch/sql/queries/task_dag_wakeup_dispatch.sql:42-45`）。reclaimer 只回收 `status='dispatching'` 且 lease 过期的行（`internal/sidecar/orch/sql/queries/task_dag_wakeup_query.sql:1-4`），不会处理 sent-unbound。recovery 只 replay `status='sent'` 且 `bound_turn_id` 等于 active turn 的 wakeup（`internal/sidecar/orch/orchestration/recover.go:171-183`）。
...
3. **[major] reclaimer 只重置 claim 信息，不记录 lease 过期原因或次数**
   - 证据：`ReclaimStaleDispatchingTaskDagWakeups` 只把 status 改回 pending 并清空 claimed/lease 字段（`internal/sidecar/orch/sql/queries/task_dag_wakeup_query.sql:1-4`），没有更新 `last_error`，也不增加独立的 reclaim/lease timeout 计数。测试只断言 rows 数和 loop 存活（`internal/sidecar/orch/orchestration/wakeup_reclaim_test.go:59-127`）。
...
4. **[major] `lease_expires_at < NOW()` 与代码注释的 `<= NOW()` 不一致，边界时刻会多卡一个 reclaim 周期**
   - 证据：reclaimer 注释写“`lease_expires_at <= NOW()` 的 wakeup 还原 pending”（`internal/sidecar/orch/orchestration/wakeup_reclaim.go:18-21`），SQL 实际使用 `lease_expires_at < NOW()`（`internal/sidecar/orch/sql/queries/task_dag_wakeup_query.sql:1-4`）。
   - 风险：边界很小，但默认 reclaim 周期与 lease 都是 30s（`internal/sidecar/orch/orchestration/wakeup_reclaim.go:31-45`、`internal/sidecar/orch/orchestration/wakeup_dispatcher.go:28-39`），精确等于边界的 wakeup 会等到下一轮。高频量化任务会放大延迟抖动。
   - 建议：统一注释和 SQL，优先改为 `<= NOW()`；同时测试覆盖等于过期时间的回收。

5. **[moderate] `BindTaskDagWakeupTurn` 不校验 target agent 或 run/node，错误 turn 可能绑定到同一 wakeup**
   - 证据：SQL 只按 `id`、`status='sent'`、`sent_at IS NOT NULL`、`bound_turn_id IS NULL` 更新（`internal/sidecar/orch/sql/queries/task_dag_wakeup_dispatch.sql:42-45`）；store 层 `bindWakeupTurnTx()` 也只传 TurnID 和 ID（`internal/sidecar/orch/store/taskdag/factory.go:72-89`）。
...
- `dispatching` lease 过期的基本回收存在，并且 runner 接入为独立 ticker，dispatcher 失败不会让 reclaim loop 退出（`internal/sidecar/orch/orchestration/wakeup_reclaim.go:74-117`）。
- 单条 wakeup 处理失败不会中断同 batch 其余项，`handleClaimed()` 的子步骤只记录日志并继续（`internal/sidecar/orch/orchestration/wakeup_dispatcher.go:237-255`）。
- recovery 会跳过已经被 reclaim 回 pending 的 wakeup，测试覆盖该语义（`internal/sidecar/orch/orchestration/recover_test.go:260-293`）。
