...
- `internal/sidecar/orch/orchestration/dag_ops_running_invariants_test.go`
- `internal/sidecar/orch/orchestration/dag_ops_remove_node_test.go`

## Findings

1. **[critical] 运行中 `add_node` 被允许，但持久化只写模板行，当前 run 不会得到新增 runtime node**
   - 证据：`enforceRunningDAGInvariants()` 在 DAG running 或存在 active run 时，只拒绝 `update_dag/update_node/remove_node`，允许 `add_node`，且无依赖的 `add_node` 也被测试明确允许（`internal/sidecar/orch/orchestration/dag_query.go:358-427`、`internal/sidecar/orch/orchestration/dag_ops_running_invariants_test.go:136-155`）。但 `persistAddNodeSpecs()` 调用 `tx.UpsertNode()`（`internal/sidecar/orch/orchestration/dag_query.go:731-746`），底层 `UpsertTaskDagNode` 只插入 `run_id IS NULL` 的模板节点并在模板唯一约束上冲突更新（`internal/sidecar/orch/sql/queries/task_dag_node_write.sql:1-12`）。运行实例节点是在 StartDAG 时由模板复制到 `run_id=$2`（`internal/sidecar/orch/sql/queries/task_dag_run.sql:35-48`），本轮未发现 `ApplyOps` 在 active run 中插入 `run_id>0` runtime node 或 wakeup 的逻辑。
...
2. **[major] 运行中 `add_node.depends_on` 的 done 判定使用模板节点状态，不是当前 run 的 runtime 状态**
   - 证据：`preflightOpsBatch()` 在事务内调用 `tx.ListNodes(ctx, dagKey)` 得到 `existing`（`internal/sidecar/orch/orchestration/dag_query.go:307-324`），`DAGOpsStore` 注释说明它复用 `DAGDetailStore` 的 `ListNodes`（`internal/sidecar/orch/store/taskdag/contract.go:93-119`）。SQL `ListTaskDagNodes` 明确只查 `run_id IS NULL` 的模板节点（`internal/sidecar/orch/sql/queries/task_dag_node_read.sql:1-6`）。随后 `enforceRunningAddNodeDeps()` 用 `doneNodeKeys(existing)` 要求依赖指向 `status == "done"` 的节点（`internal/sidecar/orch/orchestration/dag_query.go:412-437`）。但 F6.5 后 StartDAG 会把模板节点复制为 runtime node，真实执行状态在 `run_id>0` 行上推进。
...
3. **[major] `ApplyOps` 的 OCC version 已接通 raw SQL，但 DAG DTO/StartDAG 仍不暴露 version，调用方很难拿到可靠 `base_version`**
   - 证据：`ApplyOps` 入口要求 `BaseVersion` 非负并在事务内用 `GetDAGVersionForUpdate` 比对（`internal/sidecar/orch/orchestration/dag.go:608-643`、`internal/sidecar/orch/orchestration/dag_query.go:297-306`），store 用 raw SQL 读取 `task_dags.version`（`internal/sidecar/orch/store/taskdag/store_dag_ops.go:23-47`）。但常规 `GetTaskDag` / `GetTaskDagForUpdate` SQL 返回列不包含 `version`（`internal/sidecar/orch/sql/queries/task_dag_dag.sql:24-32`），`fromDAG` 使用的 sqlc 模型也不含 version；Round 020 已确认 StartDAG snapshot 用的 `dagVersionFor()` 仍固定返回 0（`internal/sidecar/orch/orchestration/scheduler.go:220-225`）。
...
4. **[major] active run 保护只锁 DAG 行，不锁/冻结 active run 集合，StartDAG 与 ApplyOps 可在检查后交错**
   - 证据：`ApplyOps` 在事务内先 `GetDAGVersionForUpdate` 锁 `task_dags` 行，再 `CountRunningRunsByDagKey()`（`internal/sidecar/orch/orchestration/dag_query.go:297-318`）。StartDAG 也在事务内 `GetDAGForUpdate` 锁同一 DAG 行后创建 run 和复制节点（`internal/sidecar/orch/orchestration/scheduler.go:112-128`）。这能让两条写路径串行，但 `ApplyOps` 如果先拿锁并看到 activeRuns=0，就会继续修改模板、bump version；随后排队的 StartDAG 拿锁并复制修改后的模板。此行为对 draft 编辑合理，但对“active run 保护”来说，保护对象不是一个稳定的“无 run 即将启动”条件。
...
5. **[moderate] `update_dag` 每个 patch 单独计算 `next_run_at`，同批多 patch 时使用同一旧 schedule 校验，顺序语义不明确**
   - 证据：`planDAGUpdates()` 对每个 `update_dag` op 调 `validateDAGPatch(patch, current)`，其中 `current` 是同一个 `GetDAGSchedule()` 结果（`internal/sidecar/orch/orchestration/dag_query.go:491-509`）。`persistDAGPatches()` 又按 patch 顺序逐条 `UpdateDAGPatch`（`internal/sidecar/orch/orchestration/dag_query.go:682-692`）。同批 duplicate `update_dag` 有测试拒绝（`internal/sidecar/orch/orchestration/dag_ops_update_dag_test.go:381-400`），因此当前不会出现多 patch，但这个约束由 `partitionOps` 侧的 duplicate 规则间接保护，并没有在 `planDAGUpdates` 自身表达。
...
- `ApplyOps` 的基本 OCC 不是空的：事务内 `GetDAGVersionForUpdate` + `BumpDAGVersion(expected)` 已接通，且 bump lost-lock 会翻译为 `ErrVersionConflict`（`internal/sidecar/orch/orchestration/dag_query.go:346-356`、`internal/sidecar/orch/store/taskdag/store_dag_ops.go:123-143`）。
- `remove_node` 已有状态 fence：service 规划时拒绝被依赖和 running DAG，SQL 删除也只允许模板节点 `status IN ('pending','ready')`（`internal/sidecar/orch/orchestration/dag_ops_remove_node_test.go:157-223`、`internal/sidecar/orch/sql/queries/task_dag_node_write.sql:29-34`）。本轮不报告 remove 无保护。
- 空 ops 短路会读取当前 version 并校验 base_version，不会推进 version，这是测试覆盖的设计行为（`internal/sidecar/orch/orchestration/dag.go:693-713`）。

## 验证

```bash
./scripts/test_with_guard.sh ./internal/sidecar/orch/orchestration ./internal/sidecar/orch/store/taskdag ./internal/sidecar/orch/orchestration/nodeexec ./internal/sidecar/orch/tools -count=1
```

结果：Go guard、`internal/archtest`、`internal/sidecar/orch/orchestration`、`internal/sidecar/orch/store/taskdag`、`internal/sidecar/orch/orchestration/nodeexec` 与 `internal/sidecar/orch/tools` 通过。
