...
P2 不迁移 v2 的 cache key。v3 当前 cache key 已包含 `ScopeKey`、`WorkspaceKey`、`LanguageID`、`URI`、`LanguageSpecificHash`，见 `internal/sidecar/lsp/multilsp/cache.go:25-36` 与 `internal/sidecar/lsp/multilsp/cache.go:433-453`。
...
带 `lsp_integration` / `e2e` 的测试也必须使用仓库 wrapper（例如 `./scripts/test_with_guard.sh ./internal/sidecar/lsp/manager -tags lsp_integration ...`），除非文档明确标注为“附加 smoke，不作为完成门禁”。
