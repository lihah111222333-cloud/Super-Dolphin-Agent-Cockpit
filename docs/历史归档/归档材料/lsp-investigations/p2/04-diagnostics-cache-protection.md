...
v3 显式 URI diagnostics 已会在返回前 refresh：`internal/sidecar/lsp/multilsp/manager_diagnostics.go:70-108`。deleted cleanup 会清 diagnostics、cache tombstone、bootstrap state，见 `internal/sidecar/lsp/multilsp/manager_diagnostics.go:296-330`。

v3 cache key 已 scoped：`internal/sidecar/lsp/multilsp/cache.go:25-36`，string key 包含 scope/workspace/language/URI/hash：`internal/sidecar/lsp/multilsp/cache.go:433-453`。
...
v3 persistent cache 当前是单 JSON disk state，整体 `os.WriteFile`，见 `internal/sidecar/lsp/multilsp/cache.go:398-412`。v2 是 per-record file store，并有删除坏 record、tmp rename、过期 cleanup 的经验。P2 不迁移 v2 per-record store，只迁移保护策略。

### P2：tombstone 重启防复活语义不明确

v3 tombstone 是内存 map，见 `internal/sidecar/lsp/multilsp/cache.go:183-194`。若 persistent cache 文件中仍有旧 record，重启后是否复活需要明确测试和设计。
...
set -euo pipefail
required_tests=(
  './internal/sidecar/lsp/multilsp:TestDiagnosticsAllRefreshesStaleScopedDiagnosticBeforeReturn'
  './internal/sidecar/lsp/multilsp:TestDiagnosticsAllBootstrapsUntrackedExistingDiagnosticURI'
  './internal/sidecar/lsp/multilsp:TestDiagnosticsAllDeletedFileClearsDiagnosticsAndTombstones'
  './internal/sidecar/lsp/multilsp:TestDidChangeAdvancesBootstrapCacheVersionForFullDiskBackedText'
  './internal/sidecar/lsp/multilsp:TestIncrementalDidChangeDoesNotWritePersistentCache'
  './internal/sidecar/lsp/multilsp:TestMemoryOnlyDidChangeDoesNotWritePersistentCache'
  './internal/sidecar/lsp/multilsp:TestDidChangeFailureFallsBackToReopenThenRestart'
  './internal/sidecar/lsp/multilsp:TestDidCloseClearsBootstrapReadyAndNextOpenReopens'
  './internal/sidecar/lsp/multilsp:TestDidCloseDoesNotTombstoneExistingFile'
  './internal/sidecar/lsp/multilsp:TestBootstrapBootstrappingTimeoutAllowsRetry'
  './internal/sidecar/lsp/multilsp:TestPersistentCacheCorruptFileQuarantinedAndRewritten'
  './internal/sidecar/lsp/multilsp:TestPersistentCacheWritesAtomically'
  './internal/sidecar/lsp/multilsp:TestPersistentCacheExpiredEntryFilteredAndPersisted'
  './internal/sidecar/lsp/multilsp:TestDeletedPersistentCacheRecordDoesNotResurrectAfterRestart'
  './internal/sidecar/lsp/multilsp:TestDiagnosticsAfterServerExitDoesNotReturnOldGeneration'
  './internal/sidecar/lsp/multilsp:TestCacheKeySeparatesLanguageIDAcrossSameURI'
  './internal/sidecar/lsp/multilsp:TestDiagnosticsAllDoesNotReturnCrossLanguageSameURI'
  './internal/sidecar/lsp/multilsp:TestDeletedPersistentCacheRecordDoesNotResurrectAcrossLanguages'
  './internal/sidecar/lsp/multilsp:TestBootstrapCacheMatrixForRegisteredLanguageIDs'
)
run_required_test() {
  local pkg="$1"
  local test_name="$2"
