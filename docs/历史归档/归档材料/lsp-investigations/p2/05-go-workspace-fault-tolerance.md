...
v3 `resolveGoRootFromGOWORK` 对非空 GOWORK 当显式路径处理，见 `internal/sidecar/lsp/multilsp/go_root_resolver.go:101-123`。Go 工具链中 `GOWORK=auto` 表示自动发现；P2 应把 `auto` 等同 unset。

### P1：破损 go.work 阻断 gopls

v3 `resolveGoWorkRoot` 解析 go.work use list 失败会返回错误，见 `internal/sidecar/lsp/multilsp/go_root_resolver.go:181-190`。v2 只根据 root 是否存在 go.work/go.mod 做轻量判断，见 `/Users/mima0000/Desktop/wj/go-agent-v2/pkg/toolsdk/lsp/gomod_root.go:34-36`。P2 应 soft fallback 到 go.work 所在 root，让 gopls 自己报告 workspace 诊断，而不是 manager 无法启动。

### P1：外部 `GOWORK=/other/go.work` root 污染风险

v3 多入口会使用 ambient `os.Environ()`，见 `internal/sidecar/lsp/multilsp/go_root_resolver.go:95-99`。如果外部 GOWORK 不包含当前 target/projectRoot，可能把 gopls root 导向无关 workspace。

### P2：一级子模块扫描未过滤隐藏/下划线目录

v3 `findFirstLevelGoModRoots` 会扫描所有一级目录，见 `internal/sidecar/lsp/multilsp/go_root_resolver.go:237-260`。v2 跳过 `.` / `_` 前缀目录，见 `/Users/mima0000/Desktop/wj/go-agent-v2/pkg/toolsdk/lsp/gomod_root.go:45-55`。
...
- workspace key 中的 `goModPath/goWorkPath/goworkMode/moduleRoot/moduleRootsHash/workspaceFoldersHash`，见 `internal/sidecar/lsp/multilsp/go_root_resolver.go:430-438`。
...
set -euo pipefail
required_tests=(
  './internal/sidecar/lsp/multilsp:TestGoRootResolverGOWORKAutoUsesAutoDiscovery'
  './internal/sidecar/lsp/multilsp:TestGoRootResolverBrokenGoWorkFallsBackToWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGoRootResolverExplicitGoWorkOutsideProjectConflicts'
  './internal/sidecar/lsp/multilsp:TestGoRootResolverSingleSubmoduleSkipsHiddenAndUnderscoreDirs'
  './internal/sidecar/lsp/multilsp:TestGoRootResolverLinkedWorktreeSymlinkAliasCanonicalKey'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectJSTSWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectJavaWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectPythonWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectRustWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectCSSWorkspaceRoot'
  './internal/sidecar/lsp/multilsp:TestGOWORKDoesNotAffectJSONYAMLMarkdownFallback'
  './internal/sidecar/lsp/multilsp:TestEmptyLanguageIDDoesNotDefaultToGoAdapter'
  './internal/sidecar/lsp/multilsp:TestGoLanguageSpecificHashNotAddedToNonGoCacheKey'
)
run_required_test() {
  local pkg="$1"
  local test_name="$2"
...
optional_smoke ./internal/sidecar/lsp/manager TestGoWorkRealGoplsSmoke lsp_integration gopls
```
