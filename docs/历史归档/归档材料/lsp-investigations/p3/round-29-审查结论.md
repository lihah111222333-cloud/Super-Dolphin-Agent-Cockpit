...
- `internal/sidecar/orch/notify/turn.go`（TurnNotifier.OnTurnCompleted/Interrupted、OnThreadStopped、lookupAlias、enqueue）
- `internal/sidecar/orch/notify/resolve.go`（resolveNodeAlias、extractNotifyChannel、isTerminalNodeStatus）
- `internal/sidecar/orch/notify/module.go`（provideOrchResolver、provideOrchWebhookClient、provideOrchNotifier、provideOrchFlusher、registerDAGSubscriberLifecycle）
- `internal/sidecar/orch/orchestration/sharedfile_adapter.go`（ReadSharedFile、WriteSharedFile）
- `internal/sidecar/orch/orchestration/nodeexec/stubs.go`（HybridExecutor stub）
