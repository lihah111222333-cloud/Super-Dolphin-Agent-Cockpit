...
- `internal/sidecar/lsp/manager/manager.go`（Manager 接口定义：LifecycleManager、NavigationManager、XRefManager、StructureManager、CompletionManager、EditManager、DocumentLifecycleManager、DiagnosticsManager）
- `internal/sidecar/lsp/manager/registry.go`（dynamicRegistry：Register、GetManagerForFile/Language、ensureInstalled、scopedManagerForConfig、Close、Diagnostics、DetectLanguageID）
- `internal/sidecar/lsp/manager/scope.go`（ToolScope、ResolvedToolScope、ScopedManager、resolvedScopeManager 代理、WithResolvedToolScope/FromContext）
