...
Git hook 的职责边界固定为：`pre-commit` 只在 exact staged tree 上运行仓库代码守卫，不进入本契约的 remote CI 执行路径，也不消费 remote config、duration ledger 或 agent token；`pre-push` 是 Git 交付路径唯一的完整 remote ECI 门禁，并继续绑定 exact pushed ref update。手动 remote run 与 release 入口不受该 hook 分工影响。
