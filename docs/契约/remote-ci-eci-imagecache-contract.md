...
- Go/module/build cache、Node/npm、前端依赖/构建 cache、Playwright 浏览器及系统依赖必须直接读取 accepted ImageCache 的不可变镜像层。禁止为这些内容创建 `expanded-data`、DataCache、缓存卷、subPath 挂载、解压目录或逐分片复制展开；每个 shard 只允许为候选源码、私有工作目录和临时目录创建 EmptyDir。Go 增量编译必须使用镜像内只读 build-cache seed 与分片私有的小型写层，不得把 seed 复制到外挂卷。
...
| `3.4` | §3 | 工具链、依赖、浏览器与缓存只直读 accepted ImageCache 镜像层；禁止 expanded-data、DataCache、缓存卷、subPath 或逐 shard 复制展开，只有 source/work/temp 可使用私有 EmptyDir | `ECI request shape + image closure + archtest` |
