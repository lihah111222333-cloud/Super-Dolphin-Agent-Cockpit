...
- `cmd/mcp-orch/store/sqlc/task_dag_node_runtime.sql.go`
...
1. **[critical] DAG completion 只按 spawning_thread_id 反查节点，不校验事件 turn_id 与 active_turn_id**
   - 证据：subscriber 入口只取 `ev.ThreadID`，调用 `LookupNodesBySpawningThread()` 后逐条推进节点（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:152-198`）。`CompleteTaskDagNode` 的 SQL fence 只要求 status in `ready/running/awaiting_verify`，没有 active_turn_id 或 active_wakeup_id 条件（`cmd/mcp-orch/store/sqlc/task_dag_node_runtime.sql.go:71-80`）。
...
2. **[major] completion 早于 spawning_thread_id 落库时只执行 stop_helper，不会补偿 DAG 节点**
   - 证据：反查空结果时只记录 LookupNoNode，并调用 `stopSpawnedAgentForSubscriber()` 后返回（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:171-185`）。注释承认该分支可能是 dispatchAgent 尚未来得及记录 `spawning_thread_id`（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:182-183`），测试也固定了 no-node 分支只 stop、不推进 flow（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber_test.go:377-399`）。
...
3. **[major] 同一 spawning_thread_id 命中多节点时逐条推进，dirty data 会扩大成多节点终态**
   - 证据：`len(nodes)>1` 只增加 `LookupDirtyData` 并继续 iterate every row（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:187-198`）。测试要求同一 completion 推进两条 running row（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber_test.go:401-425`）。
...
4. **[major] 成功输出写入 sharedfile 后，CompleteNode 失败不会进入 durable retry，但 stop_helper 仍会结束 agent**
   - 证据：`materializeSharedfileAfterClaim()` 先 claim 节点为 `awaiting_verify`，再写 sharedfile，随后 `advanceNodeDone()` 可能因 DB 错误返回 false（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:300-337`、`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:339-370`）。测试里的 replay 场景依赖再次收到同一 completion 事件才从 `awaiting_verify` 完成（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber_shard16_test.go:122-174`）。subscriber 循环后无论 advance 成功与否都会 stop spawned agent（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:199-203`）。
...
5. **[major] 失败 completion 永远以 FailFast=false 推进，DAG 策略无法决定是否级联取消**
   - 证据：subscriber 的 `advanceNodeFailedWithReason()` 固定传 `FailFast: false`（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:404-417`）。store 层只有 `input.FailFast` 为 true 时才级联 cancel downstream（`cmd/mcp-orch/store/taskdag/store_fail_downstream.go:28-60`）。
...
- 输出体积超 4KB 时，如果没有 sharedfile 配置，agent 节点会 fail 而不是把超大结果写入 node result（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:544-559`、`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber_shard16_test.go:423-451`）。
- sharedfile 已存在时会保留 agent 自写内容，避免 subscriber 覆盖报告文件（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:311-323`、`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber_shard16_test.go:268-309`）。
- duplicate terminal node 会被 skip，并计入 `IdempotentSkipped`（`cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go:215-220`）。
