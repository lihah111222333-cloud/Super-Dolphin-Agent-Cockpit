...
- `cmd/mcp-orch/orchestration/wakeup_dispatcher_shard17_dag_test.go`
...
3. **[major] SQL `attempt_count < 8` 硬上限会截断显式配置的大于 8 次重试**
   - 证据：注释说明 SQL 层仍保留 `attempt_count<8` 硬上限（`cmd/mcp-orch/orchestration/retry_strategy.go:34-36`）。实际 `RetryTaskDagWakeup` 在 WHERE 中写死 `AND attempt_count < 8`（`cmd/mcp-orch/sql/queries/task_dag_wakeup_dispatch.sql:47-57`）。store 的 `RetryWakeup()` 和 smart retry patch 路径都调用同一个 SQL（`cmd/mcp-orch/store/taskdag/store_wakeup.go:57-72`、`cmd/mcp-orch/store/taskdag/store_wakeup.go:75-105`）。
...
4. **[major] `skip` 和 `ask_human` 在枚举中可配置，但 dispatcher fail-closed**
   - 证据：`OnFailureStrategy` 枚举包含 `skip` 与 `ask_human`（`cmd/mcp-orch/orchestration/nodeexec/types.go:52-64`）。但 `smartRetryStrategyImplemented()` 只接受 retry、escalate_model、append_error、replan、fail_fast（`cmd/mcp-orch/orchestration/retry_strategy.go:367-378`）。测试明确期望 `skip` / `ask_human` 被 unsupported 并 fail closed（`cmd/mcp-orch/orchestration/wakeup_dispatcher_shard17_smart_retry_test.go:323-372`）。
...
5. **[moderate] 配置 JSON 解析失败静默退回默认重试策略**
   - 证据：`ResolveRetryPolicy()` 注释要求解析失败安静走默认值（`cmd/mcp-orch/orchestration/retry_strategy.go:79-82`）。`decodeDAGSchedulePolicy()` 和 `decodeNodeExecutionPolicy()` 都在 JSON 解析失败时返回空策略（`cmd/mcp-orch/orchestration/retry_strategy.go:97-120`），测试把 malformed JSON 期望为默认单次（`cmd/mcp-orch/orchestration/retry_strategy_test.go:39-47`）。
...
- smart retry 的 config patch 与 retry wakeup 在同一事务中执行，patch miss 会回滚 retry，不会出现 wakeup 已重试但 node config 未改的半更新（`cmd/mcp-orch/store/taskdag/store_node_config_patch_test.go:86-160`）。
- permanent `hard/needs_human` 若没有 by_class 显式策略，不会被 default replan 误重试（`cmd/mcp-orch/orchestration/wakeup_dispatcher_shard17_smart_retry_test.go:207-284`）。
- `escalate_model` 与 `append_error` 明确只支持 agent 节点，automation 节点会 fail closed；本轮不把这个作为缺陷，只记录策略不适用边界（`cmd/mcp-orch/orchestration/retry_strategy.go:469-519`）。
