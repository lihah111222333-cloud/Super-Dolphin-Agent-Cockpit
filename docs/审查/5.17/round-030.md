...
1. **[critical] `FailFast=false` 的失败节点不会终止依赖它的 pending 下游，run 可永久停在 `running`**
   - 证据：`FailNodeAndCancelDownstream()` 只有在 `input.FailFast` 为 true 时才调用 `cancelDownstreamTx()`（`cmd/mcp-orch/store/taskdag/store_fail_downstream.go:43-49`），随后 `maybeFinalizeRunTx()` 只在所有 runtime nodes 都进入终态时才推进 run（`cmd/mcp-orch/sql/queries/task_dag_run.sql:90-188`）。测试 `TestCompleteNode_NotAllTerminal_RunUnchanged` 明确锁定“仍有 pending 节点则 run 保持 running”（`cmd/mcp-orch/store/taskdag/store_finalize_run_test.go:563-587`）。
...
2. **[major] run finalization 只挂在少数状态写路径上，绕过 NodeFlowStore 的终态更新可能不推进 run**
   - 证据：`maybeFinalizeRunTx()` 只在 `CompleteNodeAndScheduleDownstream()` 和 `FailNodeAndCancelDownstream()` 内调用（`cmd/mcp-orch/store/taskdag/store_complete_downstream.go:56-63`、`cmd/mcp-orch/store/taskdag/store_fail_downstream.go:50-54`）。`service.UpdateNodeStatus()` 对 `status="done"` 走 flow store，但非 done 状态仍保留旧 `UpdateNodeStatus` 路径（`cmd/mcp-orch/orchestration/dag.go:186-242`）；对应测试也说明 `status != "done"` 保持 legacy update（`cmd/mcp-orch/orchestration/dag_complete_downstream_test.go:117-139`）。
...
3. **[major] failed/cancelled run 不保存 final output 或失败摘要，终态可观测性不足**
   - 证据：finalization SQL 只有 `final_status = 'succeeded'` 且 `final_output` 非空时才写 `metadata.final_output`（`cmd/mcp-orch/sql/queries/task_dag_run.sql:171-184`）。测试 `TestCompleteNode_FailedRunDoesNotPromoteFinalOutput` 明确期望 failed run metadata 只保留原始 request_id（`cmd/mcp-orch/store/taskdag/store_finalize_run_test.go:442-470`）。
...
4. **[major] final output 选择读取当前 DAG 模板 metadata，而不是 run 创建时的快照**
   - 证据：`FinalizeTaskDagRunIfAllNodesTerminal` 的 `dag_meta` CTE 直接从 `task_dags` 当前行读取 `metadata->>'final_node_key'`（`cmd/mcp-orch/sql/queries/task_dag_run.sql:110-126`）。虽然 `CreateTaskDagRun` 注释称 `dag_version_snapshot` 用于保证模板改动不影响本次 run（`cmd/mcp-orch/sql/queries/task_dag_run.sql:8-15`），final output 实际没有使用 run snapshot metadata。
...
5. **[moderate] finalized run 的 pending wakeup 会变成不可 claim 的残留记录**
   - 证据：`ClaimDueTaskDagWakeups` 只有当 wakeup 关联的 run 仍为 `running` 时才允许 claim（`cmd/mcp-orch/sql/queries/task_dag_wakeup_dispatch.sql:13-24`）。测试 `TestClaimDueWakeupsSkipsFinalizedRunWakeup` 验证 finalized run 的 pending wakeup 不会被 claim（`cmd/mcp-orch/store/taskdag/store_wakeup_test.go:26-45`）。
...
- finalize SQL 的终态优先级有测试覆盖：failed 高于 cancelled，done/skipped 聚合为 succeeded（`cmd/mcp-orch/store/taskdag/store_finalize_run_test.go:473-560`）。
- `CompleteNodeAndScheduleDownstream()` 在 done 分支会同事务调度下游并尝试 finalize，成功路径的链式调度闭环是存在的（`cmd/mcp-orch/store/taskdag/store_complete_downstream.go:44-64`）。
