...
| 索引文件 | 文件数 | 大小 | 覆盖范围 |
|---|---:|---:|---|
| `docs/doc/codemap/project-map/index/app-ui.tsv` | 563 | 114.9 KB | 桌面应用、Wails host、React/Vite 前端与 UI 测试 |
...
| `docs/doc/codemap/project-map/index/modules.tsv` | 738 | 143.6 KB | 业务模块层：dashboard、memory、prompt、skill、thread、turn、uistate 等 |
| `docs/doc/codemap/project-map/index/platform-provider.tsv` | 1103 | 203.2 KB | 基础设施与 provider 集成：RPC、hooks、toolbridge、Claude/Codex/统一 provider |
| `docs/doc/codemap/project-map/index/store-sql.tsv` | 204 | 29.8 KB | 持久化层：store、sqlc、SQL queries、migrations |
| `docs/doc/codemap/project-map/index/docs-agent.tsv` | 955 | 172.0 KB | 代码地图、ADR/决策、计划与 docs 项目知识 |
| `docs/doc/codemap/project-map/index/other.tsv` | 683 | 133.7 KB | 公共库、脚本、测试、配置与其他根级资源 |
...
| 模块 | 文件数 | 职责 |
|---|---:|---|
| `internal` | 2237 | 应用内部模块、平台、provider、store 与守卫 |
| `docs` | 947 | 代码地图、ADR、计划、迁移和内部说明 |
| `cmd` | 666 | 可执行入口与 MCP peer |
| `frontend-app` | 557 | 当前 React/Vite 新 UI |
| `scripts` | 140 | 工程自动化脚本 |
| `pkg` | 30 | 可复用公共库 |
...
| `internal/module/uistate` | 54 | UI 事件投影与 timeline/sidebar 状态 |
...
| `internal/platform/toolbridge` | 103 | provider 与 MCP tools 桥接 |
...
| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `internal/provider/codexapp` | 125 | Codex app/server provider 集成 |
