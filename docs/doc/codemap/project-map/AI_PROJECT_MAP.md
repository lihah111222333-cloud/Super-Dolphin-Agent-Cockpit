...
| 索引文件 | 文件数 | 大小 | 覆盖范围 |
|---|---:|---:|---|
| `docs/doc/codemap/project-map/index/app-ui.tsv` | 766 | 156.3 KB | 桌面应用、Wails host、React/Vite 前端与 UI 测试 |
...
| `docs/doc/codemap/project-map/index/platform-provider.tsv` | 1285 | 239.0 KB | 基础设施与 provider 集成：RPC、hooks、toolbridge、Claude/Codex/统一 provider |
| `docs/doc/codemap/project-map/index/store-sql.tsv` | 231 | 36.1 KB | 持久化层：store、sqlc、SQL queries、migrations |
| `docs/doc/codemap/project-map/index/remote-ci.tsv` | 220 | 59.6 KB | 远程 CI：Git hooks、strict SQLite authority、阿里云 ECI/OSS、ImageCache 与 shard worker |
| `docs/doc/codemap/project-map/index/docs-agent.tsv` | 96 | 14.3 KB | 代码地图、ADR、契约与 docs 项目知识 |
| `docs/doc/codemap/project-map/index/other.tsv` | 1090 | 216.8 KB | 公共库、脚本、测试、配置与其他根级资源 |
...
| 模块 | 文件数 | 职责 |
|---|---:|---|
| `internal` | 2849 | 应用内部模块、平台、provider、store 与守卫 |
| `cmd` | 878 | 可执行入口与 MCP peer |
| `frontend-app` | 758 | 当前 React/Vite 新 UI |
| `scripts` | 190 | 工程自动化脚本 |
...
| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `internal/module/thread` | 115 | thread start/resume/fork/stop 生命周期与绑定真相源 |
...
| `cmd/mcp-lsp/multilsp` | 108 | 多语言 LSP manager、transport 与缓存 |

### remote CI

| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `cmd/super-dolphin-gate` | 62 | remote run/hook/materializer/manifest installer/worker CLI |
| `internal/devtools/remoteci` | 115 | coordinator、source transport、workload identity 与 timing |
| `internal/devtools/cicontract` | 13 | ECI/ImageCache/SQLite canonical contract owner |
