...
| 索引文件 | 文件数 | 大小 | 覆盖范围 |
|---|---:|---:|---|
| `docs/doc/codemap/project-map/index/app-ui.tsv` | 767 | 156.6 KB | 桌面应用、Wails host、React/Vite 前端与 UI 测试 |
...
| `docs/doc/codemap/project-map/index/platform-provider.tsv` | 1710 | 325.1 KB | 基础设施与 provider 集成：RPC、hooks、toolbridge、Claude/Codex/统一 provider |
| `docs/doc/codemap/project-map/index/store-sql.tsv` | 231 | 36.1 KB | 持久化层：store、sqlc、SQL queries、migrations |
| `docs/doc/codemap/project-map/index/remote-ci.tsv` | 270 | 73.7 KB | 远程 CI：Git hooks、strict SQLite authority、阿里云 ECI/OSS、ImageCache 与 shard worker |
| `docs/doc/codemap/project-map/index/docs-agent.tsv` | 99 | 14.8 KB | 代码地图、ADR、契约与 docs 项目知识 |
| `docs/doc/codemap/project-map/index/other.tsv` | 1202 | 241.0 KB | 公共库、脚本、测试、配置与其他根级资源 |
...
| 模块 | 文件数 | 职责 |
|---|---:|---|
| `internal` | 3033 | 应用内部模块、平台、provider、store 与守卫 |
| `cmd` | 1267 | 可执行入口与 MCP peer |
| `frontend-app` | 759 | 当前 React/Vite 新 UI |
| `scripts` | 206 | 工程自动化脚本 |
| `docs` | 92 | 当前文档、生成索引、开发中材料与历史证据 |
| `pkg` | 30 | 可复用公共库 |
...
| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `internal/app/storeadapter` | 51 | 业务 Store 到 module 窄端口的适配器 |
| `internal/app/runtimeadapter` | 10 | runtime consumer 的 Store/module 窄端口适配器 |
| `internal/app/internal/storeguard` | 2 | adapter 共享的 typed-nil fail-fast 检查 helper |
| `internal/app` | 100 | 全域汇总（root + adapter packages） |

### internal/module

| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `internal/module/thread` | 117 | thread start/resume/fork/stop 生命周期与绑定真相源 |
| `internal/module/turn` | 68 | turn 启动、执行、审批与 provider 调度 |
| `internal/module/prompt` | 91 | prompt 模板、启用条件与 system prompt 组装 |
| `internal/module/memory` | 156 | memory canonical 管理、检索与持久化接线 |
| `internal/module/skill` | 99 | skill canonical 管理与 provider-native mirror |
| `internal/module/uistate` | 59 | UI 事件投影与 timeline/sidebar 状态 |
...
| `internal/platform/mcpcontrol` | 46 | MCP 控制平面与 peer 注册 |
| `internal/platform/toolbridge` | 126 | provider 与 MCP tools 桥接 |
| `internal/platform/hooks` | 33 | hook 配置、执行与三阶段拦截 |
| `internal/platform/config` | 18 | 运行配置、env、provider 与超时策略 |

### internal/provider

| 子系统 | 文件数 | 职责 |
|---|---:|---|
| `internal/provider/codexapp` | 133 | Codex app/server provider 集成 |
| `internal/provider/claudecli` | 90 | Claude CLI provider 集成 |
| `internal/provider/shared` | 21 | provider home、配置和共享 helpers |
| `internal/provider/unified` | 32 | 统一 provider 会话解析与 manifest |
...
| `cmd/mcp-lsp/tools` | 57 | LSP MCP tools 实现 |
| `cmd/mcp-lsp/multilsp` | 162 | 多语言 LSP manager、transport 与缓存 |
...
| `internal/devtools/remoteci` | 146 | coordinator、source transport、workload identity 与 timing |
...
默认刷新增量合并当前 Git 工作树；`--full` 全量重建同一工作树，`--check` 也按同一文件与规范化大小检查。首次生成、生成器或规则指纹变化时必须显式传入 `--full`。现有手写代码地图仍以 `docs/doc/codemap/README.md` 和 `make codemap-check` / `make codemap-refresh` 为准；本目录提供低 token 的全仓文件级索引补充。
