...
| 索引文件 | 文件数 | 大小 | 覆盖范围 |
|---|---:|---:|---|
| `docs/doc/codemap/project-map/index/app-ui.tsv` | 548 | 112.0 KB | 桌面应用、Wails host、React/Vite 前端与 UI 测试 |
...
| `docs/doc/codemap/project-map/index/modules.tsv` | 730 | 142.3 KB | 业务模块层：dashboard、memory、prompt、skill、thread、turn、uistate 等 |
| `docs/doc/codemap/project-map/index/platform-provider.tsv` | 1026 | 188.8 KB | 基础设施与 provider 集成：RPC、hooks、toolbridge、Claude/Codex/统一 provider |
...
| `docs/doc/codemap/project-map/index/other.tsv` | 658 | 128.3 KB | 公共库、脚本、测试、配置与其他根级资源 |
...
| 模块 | 文件数 | 职责 |
|---|---:|---|
| `internal` | 2152 | 应用内部模块、平台、provider、store 与守卫 |
| `docs` | 936 | 代码地图、ADR、计划、迁移和内部说明 |
| `cmd` | 648 | 可执行入口与 MCP peer |
| `frontend-app` | 546 | 当前 React/Vite 新 UI |
| `scripts` | 129 | 工程自动化脚本 |
| `pkg` | 30 | 可复用公共库 |
...
| `internal/provider/claudecli` | 87 | Claude CLI provider 集成 |
| `internal/provider/shared` | 19 | provider home、配置和共享 helpers |
