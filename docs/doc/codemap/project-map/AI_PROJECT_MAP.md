...
| 索引文件 | 文件数 | 大小 | 覆盖范围 |
|---|---:|---:|---|
| `docs/doc/codemap/project-map/index/app-ui.tsv` | 542 | 110.8 KB | 桌面应用、Wails host、React/Vite 前端与 UI 测试 |
...
| `docs/doc/codemap/project-map/index/platform-provider.tsv` | 1024 | 188.4 KB | 基础设施与 provider 集成：RPC、hooks、toolbridge、Claude/Codex/统一 provider |
| `docs/doc/codemap/project-map/index/store-sql.tsv` | 204 | 29.8 KB | 持久化层：store、sqlc、SQL queries、migrations |
| `docs/doc/codemap/project-map/index/docs-agent.tsv` | 943 | 169.0 KB | 代码地图、ADR/决策、计划与 docs 项目知识 |
...
| 模块 | 文件数 | 职责 |
|---|---:|---|
| `internal` | 2141 | 应用内部模块、平台、provider、store 与守卫 |
| `docs` | 935 | 代码地图、ADR、计划、迁移和内部说明 |
| `cmd` | 648 | 可执行入口与 MCP peer |
| `frontend-app` | 540 | 当前 React/Vite 新 UI |
| `scripts` | 128 | 工程自动化脚本 |
| `pkg` | 30 | 可复用公共库 |
