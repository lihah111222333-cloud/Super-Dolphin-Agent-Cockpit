...
| `command_boundary` | `cmd/acp-node/**/*.go`<br>`cmd/agent-runtime/**/*.go`<br>`cmd/agent-terminal/**/*.go`<br>`cmd/build-trusted-gate-launcher/**/*.go`<br>`cmd/codex-worktree-setup/**/*.go`<br>`cmd/mcp-schema-compiler-helper/**/*.go`<br>`cmd/super-dolphin-gate/**/*.go`<br>`cmd/super-dolphin-guard/**/*.go`<br>`cmd/super-dolphin-release-manifest/**/*.go`<br>`cmd/super-dolphin-updater/**/*.go` | standalone commands import only their registered host or runtime seams |
...
| `provider_runtime` | `internal/provider/**/*.go`<br>`internal/provider/**/module.go`<br>`internal/provider/unified/**/*.go` | provider adapters own transport/runtime integration and must not reach into persistence internals |
...
| `command_narrow_import_surface` | `command_boundary` | `allow_internal_imports` | `cmd/acp-node/**/*.go`<br>`cmd/agent-runtime/**/*.go`<br>`cmd/agent-terminal/**/*.go`<br>`cmd/build-trusted-gate-launcher/**/*.go`<br>`cmd/codex-worktree-setup/**/*.go`<br>`cmd/mcp-schema-compiler-helper/**/*.go`<br>`cmd/super-dolphin-gate/**/*.go`<br>`cmd/super-dolphin-guard/**/*.go`<br>`cmd/super-dolphin-release-manifest/**/*.go`<br>`cmd/super-dolphin-updater/**/*.go` | 39 policies across 10 file patterns | — | — | — | standalone commands may import only their registered application or runtime seams |
...
| `module_no_outer_implementation_imports` | `module_boundary` | `deny_imports` | `internal/module/**/*.go` | — | `internal/module/**/*.go` → `cmd`<br>`internal/module/**/*.go` → `internal/mcpserver`<br>`internal/module/**/*.go` → `internal/provider` | — | — | business modules must depend on contracts and injected ports instead of provider, MCP server, or command implementations |
...
| `provider_external_import_surface` | `provider_runtime` | `allow_external_imports` | `internal/provider/**/*.go`<br>`internal/provider/**/module.go` | `internal/provider/**/*.go` → `github.com/BurntSushi/toml`<br>`internal/provider/**/*.go` → `github.com/gorilla/websocket`<br>`internal/provider/**/*.go` → `github.com/kelindar/event`<br>`internal/provider/**/*.go` → `golang.org/x/net`<br>`internal/provider/**/*.go` → `golang.org/x/sync`<br>`internal/provider/**/*.go` → `golang.org/x/sys`<br>`internal/provider/**/module.go` → `go.uber.org/fx` | — | — | — | provider production code keeps a closed external module surface, with Fx limited to module assembly files |
...
| `provider_unified_no_concrete_imports` | `provider_runtime` | `allow_internal_imports` | `internal/provider/unified/**/*.go` | `internal/provider/unified/**/*.go` → `internal/contract`<br>`internal/provider/unified/**/*.go` → `internal/dto`<br>`internal/provider/unified/**/*.go` → `internal/platform`<br>`internal/provider/unified/**/*.go` → `internal/provider/shared`<br>`internal/provider/unified/**/*.go` → `internal/util` | — | — | — | the unified provider dispatch layer must depend on shared contracts instead of concrete provider implementations |
...
|---|---|---|---|---|---|
| `acp_node_boundary` | `internal/archtest/acp_node_boundary_guard_test.go` | — | `TestACPNodeBoundary`<br>`TestACPNodeNarrowImportBoundary` | `cmd/acp-node`<br>`internal/devtools` | experimental ACP development code must remain isolated from production provider, UI, RPC, and persistence surfaces |
...
| `dependency_direction` | `internal/archtest/dependency_direction_test.go` | — | `TestDependencyDirection` | `internal/contract`<br>`internal/dto`<br>`internal/mcpserver`<br>`internal/module`<br>`internal/platform`<br>`internal/provider`<br>`internal/store` | dependency direction tests protect typed backend layer relationships |
...
|---|---|---|---|
| `cmd/acp-node` | `command_narrow_import_surface` | `acp_node_boundary` | experimental ACP development node |
...
| `cmd/build-trusted-gate-launcher` | `command_narrow_import_surface`<br>`fx_assembly_scope` | — | trusted gate launcher build command |
...
| `internal/contract` | `contract_dto_no_framework_imports`<br>`contract_reverse_pollution` | `dependency_direction` | stable DTO and port contracts |
| `internal/devtools` | `fx_assembly_scope`<br>`internal_support_narrow_import_surface` | `acp_node_boundary` | backend developer tooling |
| `internal/dto` | `contract_dto_no_framework_imports`<br>`fx_assembly_scope`<br>`internal_support_narrow_import_surface` | `dependency_direction` | transport-neutral data transfer objects |
| `internal/e2e` | — | `rpc_runtime_e2e` | backend end-to-end test surface |
| `internal/guards` | — | `ignored_go_tests`<br>`rollback_skip_markers` | repository-level test guard surface |
| `internal/mcpserver` | `fx_assembly_scope` | `dependency_direction` | shared MCP server implementations |
| `internal/module` | `fx_assembly_scope`<br>`module_horizontal_deep_import`<br>`module_no_direct_db_imports`<br>`module_no_outer_implementation_imports`<br>`module_no_store_imports` | `dependency_direction` | business module ownership |
| `internal/platform` | `fx_assembly_scope`<br>`hooks_no_mcpcontrol`<br>`hooks_no_platform_db`<br>`mcpcontrol_no_hooks`<br>`platform_no_module`<br>`platform_no_store` | `dependency_direction` | infrastructure runtime layer |
| `internal/provider` | `fx_assembly_scope`<br>`provider_external_import_surface`<br>`provider_no_platform_db`<br>`provider_no_store`<br>`provider_unified_no_concrete_imports` | `dependency_direction` | provider adapter runtime |
| `internal/store` | `fx_assembly_scope`<br>`store_dependency_surface`<br>`store_sqlc_store_platform_only` | `dependency_direction` | persistence anti-corruption layer |
...
| `pkg/cronmetrics` | `pkg_no_internal_imports` | `pkg_public_boundary` | public cron recovery metrics library |
