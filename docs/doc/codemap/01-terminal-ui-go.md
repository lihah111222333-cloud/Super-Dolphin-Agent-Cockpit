...
- **明确不展开**：`cmd/agent-terminal/frontend/vue-app/**` 的页面、store、快捷键注入；A01b 负责。
...
graph TD
    ...
    B --> C[internal/app/app.go:101 RunDesktop]
    C --> D[internal/ui/wails/module.go:19 uiwails.Module]
    D --> E[internal/ui/wails/assets.go:34 AssetHandlerFrom]
    D --> F[internal/ui/wails/module.go:99 NewWailsApplication]
    D --> G[internal/ui/wails/rpc.go:77 NewRPCHandlers]
    D --> H[internal/ui/wails/lifecycle.go:56 NewWailsLifecycle]
    D --> I[internal/ui/wails/bridge.go:24 NewEventBridge]
    D --> J[internal/ui/wails/http_server.go:27 NewHTTPAssetServer]
    F --> K[internal/ui/wails/binding.go:20 App binding]
    F --> L[internal/ui/wails/window.go:14 CreateMainWindow]
    G --> M[internal/ui/wails/scope_catalog.go:23 requestScopeRoots]
...
sequenceDiagram
    ...
    participant V as Vue bootstrap
...
- `cmd/agent-terminal/frontend.go:15`：从 `frontend/dist` embed 子目录导出 `fs.FS`。
- `internal/app/app.go:101`：桌面模式总入口；向 Fx 注入 `FrontendFS` 并 `Populate(&wailsApp, &lifecycle)`。
- `internal/ui/wails/assets.go:34`：资源服务选择器；dev proxy / 注入 FS / placeholder 三路回退。

#### 关键类型
- `internal/ui/wails/assets.go:18` `FrontendFS`：入口层传给 Wails 资产服务的 `fs.FS` 包装。
- `internal/ui/wails/module.go:90` `httpAssetServerParams`：HTTP 调试模式复用同一份前端资源输入。

#### 关键流程
1. `cmd/agent-terminal/main.go:11` 进入 `app.RunDesktop(frontendDistFS())`；入口本身不持有业务状态。
2. `cmd/agent-terminal/frontend.go:15` 用 `fs.Sub(frontendDist, "frontend/dist")` 把 embed 根裁成前端产物目录。
3. `internal/app/app.go:101` 通过 `fx.Supply(uiwails.FrontendFS{FS: frontendFS})` 把资源文件系统注入 `uiwails.Module`，并在启动后直接调用 `wailsApp.Run()`。
4. `internal/ui/wails/assets.go:34` 按顺序选择：`VITE_DEV_URL` 反代 → 注入的 dist FS → `frontend/index.html` placeholder。
5. `AssetHandlerFrom` 的生产 consumer 有两处：`internal/ui/wails/module.go:114`（桌面 Wails 壳）和 `internal/ui/wails/http_server.go:28`（浏览器调试 HTTP 服务）。

### 4.2 Wails App 装配与主窗口创建

#### 文件地图
- `internal/ui/wails/module.go:19`：`uiwails.Module` 的 Provide / Invoke 装配面。
- `internal/ui/wails/module.go:99`：构造 `application.App`，绑定退出钩子、事件 emitter、主窗口。
...
- `internal/ui/wails/module.go:74` `applicationParams`：Wails 壳构造参数集合。

#### 关键流程
1. `internal/ui/wails/module.go:19` 统一提供 `NewApp`、`NewRPCHandlers`、`NewService`、`NewWailsLifecycle`、`NewEventBridge`、`NewWailsApplication`、`NewHTTPAssetServer`。
2. `internal/ui/wails/module.go:34` `NewApp` 只注入 `rpc.Server.Dispatch`、窗口标题和 debug 标志；业务能力不在这里拼装。
3. `internal/ui/wails/module.go:99` `NewWailsApplication` 创建 `application.App`，把 `ShouldQuit` / `OnShutdown` 接到 `WailsLifecycle`，再用 `internal/ui/wails/binding.go:121` `bindRuntime` 回填 `wailsApp` 和事件 emitter。
4. `internal/ui/wails/module.go:127` 监听 `events.Common.ApplicationStarted`，触发 `internal/ui/wails/lifecycle.go:90` `MarkFrontendReady()`；这决定了退出 overlay 是否可以真正落地到前端。
5. `internal/ui/wails/window.go:14` `CreateMainWindow` 只创建主窗口；后续子窗口走 `internal/ui/wails/binding.go:93` `openNewWindow()`。
6. `internal/ui/wails/window.go:95` `windowURL` 会把 `ao_ui_bootstrap` / `ao_window_cwd` 注入 query string；Go 侧已写出，前端消费留给 A01b。

### 4.3 生命周期、Runner 与退出收口

#### 文件地图
- `internal/ui/wails/lifecycle.go:56`：退出状态机、overlay、backend shutdown、硬超时。
- `internal/ui/wails/module.go:53`：把 orchestration service 包装成 `ActiveAgentCounter`。
- `internal/app/runner.go:34`：统一启动 grouped runners，并在 runtime 异常退出时请求 Fx shutdown。
- `internal/app/app.go:171`：桌面路径单独 watch `app.Done()`，兜底调用 `NotifyBackendFailed()`。
...
- `internal/app/runner.go:22` `runtimeParams`：runner group + lifecycle + shutdowner 的运行时依赖集合。

#### 关键流程
1. `internal/ui/wails/module.go:53` `NewActiveAgentCounter` 通过 `contract.OrchestrationService.ListAgents` 统计非 stopped/failed agent；退出是否弹 overlay 由它决定。
...
4. `internal/app/runner.go:34` `BindRuntime()` 启动 grouped runners；若 runner 异常退出，会在 `internal/app/runner.go:61` 调 `lifecycle.NotifyBackendFailed()`，再触发 Fx shutdown。
5. `internal/app/app.go:171` `watchFXShutdown()` 监听 `app.Done()`；一旦 Fx 提前结束，也会在 `internal/app/app.go:176` 调 `NotifyBackendFailed()`。
...
- `internal/ui/wails/module.go:154`：用 Fx lifecycle 驱动 bridge 的 Start/Stop。

#### 关键类型
- `internal/ui/wails/bridge.go:15` `EventBridge`：持有 `event.Dispatcher`、`WailsLifecycle` 和 cancel 列表。

#### 关键流程
1. `internal/ui/wails/module.go:154` `bindEventBridge()` 在 `OnStart` 调 `bridge.Start()`，在 `OnStop` 调 `bridge.Stop()`；`bridge.Start()` 的生产 caller 为 `internal/ui/wails/module.go:160`。
...
- `internal/ui/wails/rpc.go:77`：注册桌面专属 UI RPC。
...
3. **UI RPC 注册**：`internal/ui/wails/rpc.go:77` `NewRPCHandlers()` 注册 `ui/code/*`、`ui/copyText`、`ui/log`、`ui/selectProjectDir(s)`、`ui/selectFiles`、`ui/windowBootstrap/get`、`ui/openNewWindow`。
...
5. **日志回传**：`internal/ui/wails/rpc.go:177` `handleUILog()` 把前端批量日志重新映射到 Go logger，并补 thread/agent/client 元信息。

### 4.6 项目作用域与代码预览 RPC

#### 文件地图
- `internal/ui/wails/scope_catalog.go:23`：把 config root + UI 注册项目汇总成 scope catalog。
...
- `internal/ui/wails/rpc.go:124`：三类 `ui/code/*` handler 的统一入口。

#### 关键类型
- `internal/ui/wails/scope_catalog.go:18` `scopeCatalog`：默认 root + known roots 的目录册。
...
#### 关键流程
1. `internal/ui/wails/rpc.go:124` `handleCodeSave()`、`internal/ui/wails/rpc.go:137` `handleCodeLocate()`、`internal/ui/wails/rpc.go:150` `handleCodeOpen()` 都先调用 `internal/ui/wails/scope_catalog.go:23` `requestScopeRoots()`；其生产 callers 可在 `internal/ui/wails/rpc.go:130/143/156` 看到。
2. `internal/ui/wails/scope_catalog.go:37` `loadScopeCatalog()` 会把 `config.ProjectRoot`、`ProjectsState.Active`、`ProjectsState.Projects` 全部折叠进 `knownRoots`，只允许已注册项目作为 scope root。
...
#### 文件地图
- `internal/ui/wails/rpc.go:310`：`ui/openNewWindow` RPC 入口。
...
#### 关键流程
1. `internal/ui/wails/rpc.go:310` `handleUIOpenNewWindow()` 先校验 `n`，再调用 `internal/ui/wails/rpc.go:331` `resolveUIBootstrap()` 把 `snapshot/raw` 统一转为编码字符串。
...
4. `internal/ui/wails/rpc.go:304` `handleUIWindowBootstrapGet()` 直接消费 `internal/ui/wails/window_state.go:90` `consumeWindowBootstrapSnapshot()`；这是一次性读取，不是长期共享状态。
...
6. Go 侧已把 `ao_ui_bootstrap` / `ao_window_cwd` 注入 query（`internal/ui/wails/binding.go:110`、`internal/ui/wails/window.go:37` 的 TODO 均有说明），但前端消费仍留在 A01b。

### 4.8 浏览器调试 HTTP runner 与 dormant runner adapter

#### 文件地图
- `internal/ui/wails/http_server.go:27`：输出 grouped runner，提供 `127.0.0.1:4511` 浏览器调试入口。
- `internal/ui/wails/http_server.go:39`：同时挂 HTTP 资产服务与 `/wails/ws` JRPC 桥。
- `internal/app/runner.go:34`：统一启动 runners group。
- `internal/ui/wails/runner.go:18`：`application.App -> platformrunner.Runner` 适配器，但当前未接入 Module。

#### 关键类型
- `internal/ui/wails/http_server.go:17` `httpAssetServer`：浏览器调试模式的 HTTP server 容器。
- `internal/ui/wails/runner.go:14` `runner`：把 `application.App.Run()` 包成 `platformrunner.Runner` 的适配器。

#### 关键流程
1. `internal/ui/wails/http_server.go:27` `NewHTTPAssetServer()` 产出 `group:"runners"` 的 runner；`internal/app/runner.go:34` `BindRuntime()` 会把它拉进统一运行组。
2. `internal/ui/wails/http_server.go:39` `Run()` 在 `/wails/ws` 上挂 `rpc.WSHandler(s.server, nil)`，在 `/` 上挂前端资产服务，因此浏览器调试与桌面壳共用同一套 RPC surface。
3. `internal/app/runner.go:34` `BindRuntime()` 负责 runner group 的生命周期；它与 `WailsLifecycle` 的失败收口共用一个 shutdown 路径。
4. `internal/ui/wails/runner.go:18` `NewRunner()` 当前仍是 dormant adapter：`internal/ui/wails/module.go:19-29` 只提供 `NewHTTPAssetServer`，没有把 `NewRunner` 接进 Fx；桌面主路径仍由 `internal/app/app.go:123` 直接 `wailsApp.Run()`。
...
```mermaid
graph TD
    RPC[internal/ui/wails/rpc.go:77 NewRPCHandlers] --> SAVE[handleCodeSave]
    RPC --> LOCATE[handleCodeLocate]
    RPC --> OPEN[handleCodeOpen]
    SAVE --> ROOTS[internal/ui/wails/scope_catalog.go:23 requestScopeRoots]
    LOCATE --> ROOTS
    OPEN --> ROOTS
...
1. **`ui/code/save` 的 `createNew` 还没真正打通**：`internal/ui/wails/rpc.go:29` 暴露了 `CreateNew`，`internal/ui/wails/rpc.go:124` 也向下传参，但 `internal/ui/wails/code_scope.go:67` 的 `resolveSaveTarget(raw, roots, _ bool)` 明确忽略该布尔值，仍要求文件已存在。
...
3. **桌面主路径仍是“直接 Run Wails App”**：`internal/app/app.go:123` 直接执行 `wailsApp.Run()`；`internal/ui/wails/runner.go:18` 的 adapter 目前未进入 `uiwails.Module`。
4. **EventBridge 是生产路径，不是测试专用 helper**：`internal/ui/wails/bridge.go:35` `Start()` 由 `internal/ui/wails/module.go:160` 的 Fx hook 启动，不是孤儿代码。
5. **UI 侧 dashboard/prompts 查询不归 Wails 专属 handler 管**：桌面层只负责 `internal/ui/wails/binding.go:38` `App.CallAPI()` 与 `internal/ui/wails/rpc.go:77` 这组 transport/helper；真正带 `cwd` 语义的 `ui/dashboard/get`、`dashboard/prompts`、`dashboard/skills` 仍在 `internal/module/dashboard/rpc.go:85-115`，并先走 `withDashboardPromptScopeCWD(ctx, p.Cwd)`。
...
|---|---|---|---|
| `wails` | `internal/ui/wails/code_preview_test.go`（另见 `rpc_test.go` / `bridge_test.go` / `binding_test.go`） | `internal/ui/wails/code_preview_test.go:13` `TestSaveScopedFileWritesWithinScope` | —（`internal/archtest/freeze_registry.go:19-35` 当前只列 `internal/module/memory` 与 `internal/module/prompt`） |

## 8. Go / Wails 变更 how-to

| 场景 | 触发 | 步骤 | 锚点 | 验证 |
|---|---|---|---|---|
| UI RPC / 原生能力 | 前端需要新的桌面 helper、原生 dialog、剪贴板、代码预览或窗口动作 | 1. 在 `internal/ui/wails/rpc.go` 增 typed params / handler；2. 若要走 Wails binding 兼容层，则接 `internal/ui/wails/binding.go` 或 `internal/ui/wails/window.go`；3. 保持 `_ao*` 元字段只在 transport 层剥离，不污染 `rpc.StrictHandler` | `internal/ui/wails/rpc.go:77`、`internal/ui/wails/binding.go:38`、`internal/ui/wails/window.go:95` | `internal/ui/wails/rpc_test.go:12`、`internal/ui/wails/binding_test.go:11` |
| 事件入 UI | 新后端事件要进桌面前端 runtime | 1. 事件继续发布到 bus / eventsurface；2. `internal/ui/wails/module.go:154` 启动 bridge；3. `internal/ui/wails/bridge.go:35` 订阅事件面；4. `internal/ui/wails/bridge.go:66` 标准化成 `bridge-event`，必要时由 `internal/ui/wails/bridge.go:81` 补 `agent-event` 兼容面 | `internal/ui/wails/module.go:154`、`internal/ui/wails/bridge.go:35`、`internal/ui/wails/bridge.go:66` | `internal/ui/wails/bridge_test.go:5`、`internal/ui/wails/bridge_test.go:32` |
