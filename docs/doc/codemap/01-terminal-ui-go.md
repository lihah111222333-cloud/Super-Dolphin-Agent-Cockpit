...
- **明确不展开**：`cmd/agent-terminal/frontend/vue-app/**` 的页面、store、快捷键注入；Vue 分卷负责。
...
3. `internal/app/app.go:105` 通过 `fx.Supply(uiwails.FrontendFS{FS: frontendFS})` 把资源文件系统注入 `uiwails.Module`，并在启动后直接调用 `wailsApp.Run()`。
4. `internal/ui/wails/assets.go:34` 按顺序选择：`VITE_DEV_URL` 反代 → 注入的 dist FS → `frontend/index.html` placeholder。
5. `AssetHandlerFrom` 的生产 consumer 有两处：`internal/ui/wails/module.go:116`（桌面 Wails 壳）和 `internal/ui/wails/http_server.go:36`（浏览器调试 HTTP 服务）。
