...
> 当前 `cmd/mcp-lsp/` 下共有 138 个生产 Go 文件：根目录 20 个，后代目录 118 个（测试文件不计入统计）。具体目录分布以同页代码地图索引和项目地图为准，避免手写分布与自动索引产生第二真相源。
>
> <!-- codemap-count path="cmd/mcp-lsp" kind="go-files" expected="20" -->
> <!-- codemap-count path="cmd/mcp-lsp" kind="go-files-recursive" expected="138" -->
>
