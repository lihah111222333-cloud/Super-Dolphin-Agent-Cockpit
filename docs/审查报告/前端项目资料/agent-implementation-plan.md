...
1. `internal/sidecar/orch/tools/data_analysis_tools.go`：`data_analysis` 工具 — 读 CSV/JSON（路径限 cwd + shared 白名单）→ 统计摘要（行数/列类型/数值分布/缺失值）→ 结果写 shared `reports/` 前缀。纯 Go 实现，不引入 Python。
