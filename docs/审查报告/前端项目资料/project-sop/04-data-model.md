...
| `task_dags` | DAG 模板或定义 | dag id、spec、metadata | `internal/sidecar/orch/orchestration` |
| `task_dag_nodes` | DAG 节点定义和运行节点 | dag id、run id、node id、status | `internal/sidecar/orch/orchestration` |
| `task_dag_runs` | DAG 运行记录 | run id、dag id、status、started_at | `internal/sidecar/orch/orchestration` |
...
| `workspace_runs` | workspace 运行记录 | run id、cwd、status | `internal/sidecar/orch/workspace` |
