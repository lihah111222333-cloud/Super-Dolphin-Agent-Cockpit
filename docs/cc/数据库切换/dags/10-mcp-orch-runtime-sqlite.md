...
- Modify SQLC:
  ...
  - `cmd/mcp-orch/sql/queries/**` 中阻止 SQLite sqlc generate 的 PostgreSQL-only 语法。
  - `cmd/mcp-orch/store/sqlctx/db.go`
  - `cmd/mcp-orch/store/sqlc/**` 中带 `Code generated` 标记的文件，必须由 sqlc 生成后提交，禁止手改。
  - `cmd/mcp-orch/store/sqlc/types_ext.go` 是当前手写扩展文件，不是 generated 文件；本任务必须迁移、删除或移动它，确保不再依赖 `pgtype`。
- Mechanical compile adapters:
  - 允许为 mcp-orch SQLC cutover 做“只为编译闭合”的机械类型适配，范围包括直接消费 generated nullable/time/interval 类型的生产 store callsite 与测试 helper，例如 `cmd/mcp-orch/store/taskdag/**`、`cmd/mcp-orch/store/agent/**`、`cmd/mcp-orch/store/prompt/**`、`cmd/mcp-orch/store/commandcard/**`、`cmd/mcp-orch/store/sharedfile/**`、`cmd/mcp-orch/store/workspace/**`、`cmd/mcp-orch/orchestration/**`。
...
- Any remaining `pgtype` aliases in `cmd/mcp-orch/store/sqlc/types_ext.go` or store callsites are P1 blockers for this task, because they make later runtime startup and `go test` depend on pgx despite SQLite SQLC generation.
...
```bash
./scripts/test_with_guard.sh ./cmd/mcp-orch ./cmd/mcp-orch/store/sqlctx ./cmd/mcp-orch/store/sqlc ./cmd/mcp-orch/fxadapter -count=1
make sqlc-verify
```
...
```bash
rg -n "github.com/jackc/pgx|pgxpool|pgconn|pgtype|DATABASE_URL|NewPool|pg_advisory|pg_try_advisory" cmd/mcp-orch/runtime.go cmd/mcp-orch/fx.go cmd/mcp-orch/dag_cron_runner.go cmd/mcp-orch/fxadapter cmd/mcp-orch/store/sqlctx cmd/mcp-orch/store/agent cmd/mcp-orch/store/prompt cmd/mcp-orch/store/commandcard cmd/mcp-orch/store/sharedfile cmd/mcp-orch/store/workspace cmd/mcp-orch/store/taskdag cmd/mcp-orch/orchestration cmd/mcp-orch/sqlc.yaml cmd/mcp-orch/store/sqlc
```
