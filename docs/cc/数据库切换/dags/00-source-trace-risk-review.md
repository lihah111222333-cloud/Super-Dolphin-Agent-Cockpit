...
| 10 mcp-orch runtime SQLite | `cmd/mcp-orch/runtime.go` 直接创建 `*pgxpool.Pool`，调用 `platformdb.NewPool`、`VerifyMinSchemaVersion`，并把 pool 注入 sqlc queries；`cmd/mcp-orch/sqlc.yaml`、`cmd/mcp-orch/store/sqlctx/db.go`、`cmd/mcp-orch/dag_cron_runner.go`、`cmd/mcp-orch/fxadapter/dag_cron_store.go` 仍依赖 pgx/pgtype/PG advisory lock。 | 无。mcp-orch 是独立进程/sidecar，不能依赖主进程 store 已经安全；如果 generated DBTX 仍是 pgx，`newQueries(*sql.DB)` 不可能闭合。 | Task 10 必须完成 mcp-orch SQLC 机械 cutover、runtime SQLite 注入、schema gate、最小 `runtime_locks` locker，让 mcp-orch 无 `DATABASE_URL`/pgx pool 启动。 |
