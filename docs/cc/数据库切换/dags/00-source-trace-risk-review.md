...
| 02 SQLite baseline schema | `internal/platform/db/module.go` 的 `requiredBaselineTables` 是当前 baseline 下限；根 `sqlc.yaml`、`internal/sidecar/orch/sqlc.yaml`、`sql/queries/**`、`internal/sidecar/orch/sql/queries/**` 还引用更多持久化表。 | 无。`VerifyMinSchemaVersion` 只看版本；PG baseline partial check 不能证明 SQLite 表、索引、约束完整。 | Task 02 必须从源码反推表全集，`requiredBaselineTables` 是最低集合，不允许按手写清单静默遗漏。 |
| 03 root sqlc 与 DB 边界 | `internal/store/module.go`、`internal/platform/db/tx.go`、根 `sqlc.yaml` 与 `internal/store/sqlc/**` 仍以 pgx/pgxpool/pgtype 为主接口；root generated sqlc 文件包含 PG bind/cast 类型。 | 无。store 调用层没有通用 SQLite adapter；绕过 sqlc 或手写 string SQL 会破坏现有 guard。 | Task 03 必须完成主应用 root SQLC cutover、提交生成器产物、定义唯一 SQLite DB/Tx/sqlc 边界、`BEGIN IMMEDIATE`、busy/locked bounded retry、JSON/timestamp helper。`cmd/mcp-orch` 边界由 Task 10 处理。 |
...
| 10 mcp-orch runtime SQLite | `cmd/mcp-orch/runtime.go` 直接创建 `*pgxpool.Pool`，调用 `platformdb.NewPool`、`VerifyMinSchemaVersion`，并把 pool 注入 sqlc queries；`internal/sidecar/orch/sqlc.yaml`、`internal/sidecar/orch/store/sqlctx/db.go`、`cmd/mcp-orch/dag_cron_runner.go`、`internal/sidecar/orch/fxadapter/dag_cron_store.go` 仍依赖 pgx/pgtype/PG advisory lock。 | 无。mcp-orch 是独立进程/sidecar，不能依赖主进程 store 已经安全；如果 generated DBTX 仍是 pgx，`newQueries(*sql.DB)` 不可能闭合。 | Task 10 必须完成 mcp-orch SQLC 机械 cutover、runtime SQLite 注入、schema gate、最小 `runtime_locks` locker，让 mcp-orch 无 `DATABASE_URL`/pgx pool 启动。 |
| 11 mcp-orch DAG 核心 store | `internal/sidecar/orch/sql/queries/task_dag_*.sql`、`workspace_run.sql`、`prompt_template.sql`、`command_card.sql`、`shared_file.sql`、`task_ack.sql` 使用 jsonb、`ILIKE`、`NOW()`、PG casts；store 层直接消费 generated sqlc。 | 无。工具/RPC 输出 shape 不是 DB 方言防护；若 query 未迁移，DAG 核心读写会直接失败或语义漂移。 | Task 11 必须覆盖 DAG/node/workspace/resource/task_ack 全部 query，并保持 root/mcp-orch 同步契约。 |
