...
Defaults should be safe for packaged app users.
...
1. Should trace JSONL be enabled by default in packaged production, or default to slow/error only?
...
1. Enable tracing by default, but default sampler keeps only lifecycle/RPC done/slow/error and summaries.
