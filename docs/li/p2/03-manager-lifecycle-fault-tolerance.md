...
v3 `Client` 接口没有 health 方法，见 `cmd/mcp-lsp/multilsp/client.go:27-36`。`Request` 只检查 `ensureOpen()`，见 `cmd/mcp-lsp/multilsp/client.go:158-167`；`ensureOpen()` 只看 shutdown flag，见 `cmd/mcp-lsp/multilsp/client.go:229-236`。`lookupExistingClient()` 会直接返回 workspace client，见 `cmd/mcp-lsp/multilsp/manager_lifecycle.go:304-314`。
...
v3 clone 按 `ManagerKey` 存入 shard map，见 `cmd/mcp-lsp/multilsp/pool.go:284-320`。当前测试只能证明 resolver/clone 按 scope 隔离；尚不能证明真实 agent/thread stop/archive lifecycle 会释放 manager clone、diagnostics、bootstrap state、cache index、leases。
...
- v3 cache key 已包含 scope/workspace/language/URI/hash：`cmd/mcp-lsp/multilsp/cache.go:25-36`。
- v3 active lease recycler race 已有保护：`cmd/mcp-lsp/multilsp/recycler.go` 与 P1 测试 `TestRecyclerDoesNotRecycleActiveLease`。
...
set -euo pipefail
required_tests=(
  './cmd/mcp-lsp/multilsp:TestTransportClosedDetachesWorkspaceClientAndRebuilds'
  './cmd/mcp-lsp/multilsp:TestRequestFailureAdvancesGenerationAndRebootstrap'
  './cmd/mcp-lsp/tools:TestEditFailureAfterDeadClientReturnsRetryableWithoutAutoReplay'
  './cmd/mcp-lsp/multilsp:TestInitializeFailureDoesNotLeaveStaleWorkspaceClient'
  './cmd/mcp-lsp/multilsp:TestReleaseScopeClosesOnlyMatchingAgentThreadClone'
  './cmd/mcp-lsp/multilsp:TestReleaseScopeRespectsActiveLeaseBusyOrDrain'
  './cmd/mcp-lsp/multilsp:TestReleaseScopeClearsDiagnosticsBootstrapAndCache'
  ...
  './cmd/mcp-lsp/multilsp:TestReleaseScopeRejectsEmptyIdentityWithoutExplicitScopeKind'
  ...
  './cmd/mcp-lsp/multilsp:TestManagerPoolEvictsOldIdleCloneAtCap'
  './cmd/mcp-lsp/multilsp:TestManagerPoolDoesNotEvictActiveLeaseClone'
  './cmd/mcp-lsp/multilsp:TestDeadClientRebuildPreservesTypeScriptWorkspace'
  './cmd/mcp-lsp/multilsp:TestRecyclerRebuildDoesNotDefaultNonGoLanguageToGo'
  './cmd/mcp-lsp/multilsp:TestEvictionKeepsJavaWorkspaceKeyAndLanguageSpecificHash'
)
run_required_test() {
  local pkg="$1"
  local test_name="$2"
