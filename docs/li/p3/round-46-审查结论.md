...
- `cmd/mcp-lsp/protocol/methods.go`（28 个 LSP 方法常量定义）
...
- 累计 P0：**44 个**（本轮无新增）
- 覆盖文件：~60+ 个生产代码文件
- 未覆盖重点区域：`internal/contract/`（接口定义）、`internal/dto/`（数据传输对象）、`internal/module/`（业务模块）、`cmd/mcp-orch/orchestration/runtime*.go`（运行时调度核心）
