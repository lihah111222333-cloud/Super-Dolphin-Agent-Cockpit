...
- `cmd/mcp-lsp/tools/tool_xref.go`（xref 工具：references/call_hierarchy/type_hierarchy、方向校验、limit 处理）
- `cmd/mcp-lsp/tools/tool_grep.go`（grep 工具：text_search/ast_search、结果截断、payload cap、func range 附加）
- `cmd/mcp-lsp/tools/tool_file.go`（file 工具：open_file/read_file/diagnostics、pos 解析、batch read）
...
下一轮范围建议：
- `cmd/mcp-lsp/tools/tool_structure.go` + `tool_completion.go`（structure/completion 工具）
- `cmd/mcp-lsp/tools/tool_edit.go` + `tool_edit_replace.go`（edit 工具）
- `cmd/mcp-lsp/tools/tool_coderun.go`（code_run 工具）
- 或 `cmd/mcp-lsp/search/`（搜索引擎实现）
