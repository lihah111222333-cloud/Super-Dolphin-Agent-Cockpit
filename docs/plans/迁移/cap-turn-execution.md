...
- 范围：V3 `internal/module/turn`、`internal/module/orchestration`、`internal/provider/*`，以及对照 V2 `go-agent-v2/internal/apiserver`、`go-agent-v2/pkg/agentsdk/service/*`。
...
- orchestration 只订阅 `TurnStarted` 和 `TurnCompleted`，没有消费 `TurnInterrupted`：`internal/module/orchestration/module.go:33-44`。
...
- orchestration 模块订阅 `TurnCompleted` 后调用 `svc.CompleteTurn(...)`：`internal/module/orchestration/module.go:38-44`。
- `CompleteTurn` 会校验 `activeTurnID`，触发状态机转换，并在成功后清空 `activeTurnID`：`internal/module/orchestration/service.go:326-352`。
- 但 `activeTurnID` 只会在 orchestration queue 路径里预先设入：`internal/module/orchestration/service.go:291-324`。
- `TurnStarted` 订阅里的 `BindActiveTurnID(...)` 只在 `agent.activeTurnID` 已经非空时才会绑定 provider turn id，否则直接返回 `errTurnNotActive`：`internal/module/orchestration/helpers.go:77-98`。
- module 层把 `errTurnNotActive` 当作可忽略错误：`internal/module/orchestration/module.go:34-43`。
...
- orchestration 的启动失败路径会清掉 `activeTurnID`，并用 `TriggerTurnCompleted` 把状态拉回去：`internal/module/orchestration/helpers.go:176-192`。
...
- 但 orchestration 只消费 `TurnCompleted`，没有消费 `TurnInterrupted/TurnStalled`；因此卡死、断连、interrupt-only 场景不会统一回收 agent 状态：`internal/module/orchestration/module.go:33-44`。
...
证据：
- orchestration 提交是排队的：`internal/module/orchestration/service.go:166-187`。
- `claimTurnWork(...)` 只会在 `StateTurnQueued` 的 agent 上 dequeue 一条 submission，并设置唯一 `activeTurnID`：`internal/module/orchestration/service.go:291-324`。
...
- 推导风险：`CompleteTurn(success=false)` 在 `StateTurnStarting` 上会走 `TriggerTurnAborted`，而状态机只允许 `TriggerTurnCompleted` 从 `turn_starting -> idle`，不允许 `turn_aborted` 从 `turn_starting` 触发：`internal/dto/agent/state.go:89-95`、`internal/module/orchestration/service.go:341-348`。如果 provider 很快发出失败完成事件，可能触发非法状态迁移。
