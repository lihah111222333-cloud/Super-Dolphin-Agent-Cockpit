...
- 通过 `Store` 接口引用反查，当前只有 5 个被应用层消费：
  ...
  - `taskdag.Store`：`internal/module/orchestration/service.go:34,85`、`internal/module/orchestration/dag.go:20,81,88,103,112`
...
- 它先 `cancel()` 运行时 context，触发 `runnerActor.Run()` 进入 `ctx.Done()` 分支，然后 `stopAll()`：`internal/module/orchestration/runner_actor.go:37-40,79-81`。
- `StopAllAgents()` 内部顺序是：
  - 先 `stopAgentLocked()`，实际是 `cmd.Process.Kill()`：`internal/module/orchestration/service.go:143-164`、`internal/module/orchestration/helpers.go:303-308`
  - 再 `removeSession(agent.id)`：`internal/module/orchestration/service.go:147-150`
...
- 证据：
  ...
  - hook 注册点：`internal/platform/db/module.go:28-40`、`internal/platform/bus/module.go:25-35`、`internal/platform/rpc/module.go:51-69`、`internal/module/orchestration/module.go:25-53`、`internal/provider/unified/module.go:33-43`
...
- 已有 recover：
  ...
  - orchestration / rpc push / wails bridge 都用它：`internal/module/orchestration/module.go:33-44`、`internal/platform/rpc/push.go:82-90`、`internal/ui/wails/bridge.go:53-63`
- 但没有全局保护：
  - 没有 `fx.RecoverFromPanics`
  - `BindRuntime` goroutine 无 recover：`internal/app/runner.go:37-53`
  - `runnerActor.Run()` / `waitForExit()` 无 recover：`internal/module/orchestration/runner_actor.go:26-60`
...
- grouped provider 都有消费者：
  ...
  - `group:"runners"`：`internal/app/modules.go:40-48` 的 `AsRPCRunner` + `internal/module/orchestration/module.go:16-23` 的 `NewRunnerActor` -> `internal/app/runner.go:23-24,38-40`
...
- 已看到的主要 goroutine 与退出条件：
  ...
  - `runnerActor` waiter：`cmd.Wait()` 返回；shutdown 时由 `StopAllAgents` kill 进程，见 `internal/module/orchestration/runner_actor.go:48-60`
