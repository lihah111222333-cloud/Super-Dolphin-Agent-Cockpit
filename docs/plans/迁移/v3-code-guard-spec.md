...
- **V3 2026-04-17 更新**：`TestCodeSizeGuard` 在运行时**自动调用 `archtest.AutoRepairFreezeRegistry`**，即时把 freeze 条目收缩到当前观测值（`observed < Limit` 时 shrink）或删除（`observed <= default` 时 delete）并**直接回写 `internal/archtest/freeze_registry.go`**。守卫动作与 “only-shrink” 基线一致，用余量立即落盘，避免过期 freeze 垃圾积累。本地运行 `go test ./internal/archtest/...` 后若工作树内 `freeze_registry.go` 有改动，必须随业务 commit 提交。

## 第 1 章：V3 守卫常量

| 守卫维度 | V2 上限 | V3 上限 | 收紧理由 |
|---|---:|---:|---|
| 单文件有效行数 | 500 | **600**（2026-04-17 放宽） | V3 能力密度上升，400 对 provider / mcp / memory 等模块偏紧；600 与核心包一致，减少硬拆引入的人为所有权碎片。 |
| 单函数有效行数 | 150 | 80（不变） | V3 已把装配、协议、状态机、SQL 样板拆给框架，函数长度必须明显下降。 |
| 嵌套深度 | 5 | 4（不变） | 进一步压低认知复杂度，迫使早返回和子函数提取。 |
| 圈复杂度 | 13 | 10（不变） | V3 的分支密度必须低于 V2，复杂分支应改成表驱动或状态机。 |
| 标识符下划线数 | 3 | 3（不变） | 该规则已足够严格，继续收紧只会制造无意义重命名。 |
| 包内文件数 | 仅对少数目录冻结上限 | **默认 25**（2026-04-17 放宽） | 15 在模块内多文件拆分时偏紧，25 为默认、核心包保留 30。 |
| 包有效行数 | 冻结基线，无统一硬限 | **默认 10000**（2026-04-17 放宽） | 与核心包一致；普通包和核心包在包行维度合并口径，唯一差异留在包文件数上。 |
...
- **2026-04-17 新默认**：`10000` 是普通包有效行数上限（旧值 4500）；`25` 是普通包文件数上限（旧值 15）；`600` 是单文件有效行数上限（旧值 400）。更高预算仍需显式冻结值或核心包例外声明。
