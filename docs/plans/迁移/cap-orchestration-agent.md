...
- 主审对象：V3 `internal/module/orchestration`、`internal/module/thread`、`internal/module/turn`、`internal/provider/unified`、`internal/store/taskdag`
...
- RPC 入口存在：`internal/module/orchestration/rpc.go:17-19` 把 `agent.launch` 直接绑到 `svc.LaunchAgent(...)`，参数映射在 `internal/module/orchestration/rpc.go:79-88`
- service 链路真实存在：`internal/module/orchestration/service.go:110-125` 负责校验、取/建 runtime、清队列、置 launch 状态并启动进程
- 进程启动与状态变更真实存在：`internal/module/orchestration/service.go:234-263` 调 `exec.Command(...).Start()`，成功后触发 `launch_succeeded`，状态机会从 `provisioning -> idle`，随后发 `AgentLaunched`
...
- RPC 入口存在：`internal/module/orchestration/rpc.go:40-42`
- stop 主路径在 `internal/module/orchestration/service.go:127-141`
- `stopAgentLocked` 会先走 `stop_requested`，然后 `queue.Clear()`、清空 `activeTurnID/threadID`，最后 `Kill` 进程，见 `internal/module/orchestration/service.go:155-164`
- `RemoveSession` 确实包含 `Close`：`internal/provider/unified/session_adapter.go:34-39` -> `internal/provider/unified/session.go:59-82`
- 真正 `stopped` 状态要等 `cmd.Wait()` 回来：`internal/module/orchestration/runner_actor.go:48-59` -> `internal/module/orchestration/service.go:355-394`

判断：

- 这条链不是同步的“stop 完即 stopped”
- `StopAgent()` 返回时，状态通常只是 `stopping`
- `AgentStopped` 事件在 `service.StopAgent` 返回路径里提前发出，见 `internal/module/orchestration/service.go:138-139`；真正 `StateStopped` 则依赖稍后的 `handleProcessExitTransition`
...
- RPC 入口存在：`internal/module/orchestration/rpc.go:20-29,30-39`
- `SubmitTurn` 会把 submission 入队，并在 `idle` 时转到 `turn_queued`，见 `internal/module/orchestration/service.go:166-187`
- runner actor 每 200ms 处理一次队列：`internal/module/orchestration/runner_actor.go:33-44,62-66`
- `claimTurnWork` 会 dequeue、设 `ExpectedTurnID`、绑定 `activeTurnID`，并把状态从 `turn_queued -> turn_starting`，见 `internal/module/orchestration/service.go:291-324`
- `startTurnExecution` 会调用 `TurnStarter.StartTurn`，见 `internal/module/orchestration/helpers.go:140-151`
...
- orchestration `agent.launch` 自己并不会做这一步，见 `internal/module/orchestration/service.go:110-125`
...
- 监控等待器真实存在：`internal/module/orchestration/runner_actor.go:48-59`
- 退出统一收口在 `internal/module/orchestration/service.go:355-394`
- unexpected exit 会：
  - 清 `cmd`
...
- 但 recovery 只有低层 relaunch：`internal/module/orchestration/recover.go:27-58`
...
- actor 在 `Run()` 里固定创建 `StallDetector{threshold: 30 * time.Second}` 并周期检查，见 `internal/module/orchestration/runner_actor.go:27-44`
- `CheckStall` 只认 `state == turn_running` 且 `time.Since(updatedAt) > threshold`，见 `internal/module/orchestration/recover.go:16-25`
- `updatedAt` 只在这些地方刷新：
  - `prepareLaunchStateLocked` / `BindActiveTurnID`，见 `internal/module/orchestration/helpers.go:52-60,77-98`
  - `fireAndPublishLocked`，见 `internal/module/orchestration/service.go:281-289`
  - report 相关写操作，见 `internal/module/orchestration/report.go:135-169`
- 没有任何 turn output delta / input / provider 活动会刷新 orchestration 的 `updatedAt`
- 检测到 stall 后的动作只是 `service.Recover(...)`，见 `internal/module/orchestration/runner_actor.go:68-77`
...
- 实现是互斥锁 + slice：`internal/module/orchestration/submission.go:9-50`
- FIFO 成立：`Enqueue` append，`Dequeue` 总是取 `items[0]`
- 空队列边界存在：`Dequeue/Peek` 都会返回零值和 `false`
- 测试覆盖了顺序、清空、并发访问：`internal/module/orchestration/submission_test.go:17-111`
...
- service 用一个全局 `sync.RWMutex` 保护全部 agents 运行态，见 `internal/module/orchestration/service.go:29-39`
...
- `fireOrForceLocked` 实际上不会 force；它只是 `FireCtx(...)`，失败就返回 `illegal state transition` 并附 allowed trigger，见 `internal/module/orchestration/service.go:266-279`
- 但 `prepareLaunchStateLocked` 直接写 `agent.state = provisioning`，见 `internal/module/orchestration/helpers.go:52-60`
...
- `CompleteTurn(success=false)` 却会在任何当前 active turn 上选择 `TriggerTurnAborted`，见 `internal/module/orchestration/service.go:341-348`
...
- RPC 已注册：
  ...
  - 见 `internal/module/orchestration/rpc.go:61-72`
- service 层都真实落到 store：
  - `CreateDAG`：`internal/module/orchestration/dag.go:14-39`
  - `GetDAG`：`41-47`
  - `ListDAGs`：`49-63`
...
- `agent.getReport`、`agent.rememberReportRequest`、`agent.reportEvent`、`orchestration/report` 全都注册了，见 `internal/module/orchestration/rpc.go:52-75`
- `GetReport` 返回 `agent_id/report/state/metadata.requester_ids`，见 `internal/module/orchestration/report.go:62-70,140-151`
- `RememberReportRequest` 会把 requester 记到 `agent.reportRequesters`，见 `internal/module/orchestration/report.go:73-95,154-163`
- `HandleReportEvent` 会解析 report、必要时 drain requester，并返回 `notified_requester_ids`，见 `internal/module/orchestration/report.go:98-133`
...
- 合约字段与 JSON tag 在 `internal/module/orchestration/contract.go:49-59`，全部是 snake_case
- `snapshotLocked` 的填充逻辑在 `internal/module/orchestration/service.go:220-231`
- `Port` / `Provider` 来自 launch 参数推导，不是 runtime 真值：
  - `launchPort` / `launchProvider` 在 `internal/module/orchestration/helpers.go:233-253`
- `threadID` 只有两类写路径：
...
V3 当前方法面，见 `internal/module/orchestration/rpc.go:15-76`：
...
- V3 `agent.launch` 参数明显缩水，`internal/module/orchestration/rpc_types.go:8-17` 只保留 `agentId/name/cwd/command/parentId/env`
...
2. 这份报告把 orchestration submit 链讲成了“真实可执行”，但漏掉了当前最硬的前提条件：session 必须预先存在。`orchestrationTurnStarter.StartTurn` 第一件事就是 `sessions.GetSession(agentID)`，见 `internal/module/turn/orchestration_starter.go:29-37`；而 session 只会在 `StartSession/ResumeSession` 后被 `SessionManager.Register(...)`，见 `internal/provider/unified/client.go:47-67`。`agent.launch` 自己只起进程，不建 session，见 `internal/module/orchestration/service.go:110-125`。所以“代码链接上了”不等于“`agent.launch -> agent.submit` 当前就一定能跑通”。
...
3. 这份报告强调“状态机成功迁移后必发 `StateChanged`”，但漏掉了 orchestration 里有一个关键生命周期阶段根本不走状态机：`prepareLaunchStateLocked(...)` 直接写 `agent.state = provisioning`，见 `internal/module/orchestration/helpers.go:52-60`。真正会统一 `publishStateChanged(...)` 的只有 `fireAndPublishLocked(...)`，见 `internal/module/orchestration/service.go:281-289`。这意味着 bus/push/Wails 并不能观察到所有重要生命周期变化，至少 `provisioning` 进入点现在就是静默的。报告如果只强调 `StateChanged` 的稳定性，会高估当前生命周期事件面的完整度。
