...
- V3：
  ...
  - `internal/module/orchestration/report.go`
...
- `internal/module/orchestration/rpc.go:15-76` 注册了 `agent.list`
- `internal/module/orchestration/service.go:189-207` `ListAgents()` 返回 `[]AgentSnapshot`，并按 `ID -> Name -> Port` 做 `sort.SliceStable`
- `internal/module/orchestration/contract.go:49-59` `AgentSnapshot` 使用 snake_case json tag
...
- 字段完整性：
  ...
  - `port/provider`：
    ...
    - V3 来自 launch 请求推断：`launchPort(req)` / `launchProvider(req)`，见 `internal/module/orchestration/helpers.go:34-50,233-253`
...
- `internal/module/orchestration/rpc.go:46-48` 注册了 `agent.snapshot`
- `internal/module/orchestration/service.go:209-232` `Snapshot(ctx, agentID)` 返回单 agent `AgentSnapshot`
- `internal/module/orchestration/contract.go:49-59` `AgentSnapshot` 是 snake_case DTO
...
- `internal/module/orchestration/rpc.go:49-51` 注册并返回 `svc.GetState(...)`
- `internal/module/orchestration/contract.go:61-64` `AgentStateResult` 使用 snake_case tag
- `internal/module/orchestration/report.go:51-60` `GetState()` 直接返回 `{agent_id,state}`；缺失 agent 时返回错误
...
- `internal/module/orchestration/rpc.go:52-54` 注册并返回 `svc.GetReport(...)`
- `internal/module/orchestration/contract.go:70-75` `AgentReportResult` 使用 snake_case tag，新增可选 `metadata`
- `internal/module/orchestration/report.go:62-71,140-151` `GetReport()` 只返回 `lastReport`，不回退别的文本
