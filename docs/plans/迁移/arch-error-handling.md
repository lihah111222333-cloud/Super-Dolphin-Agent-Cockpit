...
| `internal/module/orchestration/service.go` | `:257` `_ = stopProcess(cmd)` |
...
| `internal/module/orchestration/service.go:234-264` + `internal/module/orchestration/helpers.go:303-308` | agent 进程启动 `exec.Command` 无 ctx；停止路径直接 `Process.Kill()`，没有 ctx 控制的优雅退出窗口。 |
