...
- orchestration 有 `StopAgent`，但不暴露成 thread 模块的公共 RPC：`internal/module/orchestration/service.go:127-141`
...
- orchestration 自己维护独立运行时状态 `agentRuntime.state/threadID`：`internal/module/orchestration/service.go:41-65,220-231`
...
- orchestration 的 `LaunchAgent` 也是按 `agentID` 加锁和判重，不是按 `threadID`：`internal/module/orchestration/service.go:110-125`
...
- `internal/module/thread` 和 `internal/module/orchestration` 中都没有 `WithTimeout` 使用
...
- orchestration 会发布 agent 级事件：`internal/module/orchestration/events.go:13-64`
