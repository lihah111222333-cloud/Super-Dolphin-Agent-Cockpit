...
- 已统一的消费侧：
  - `internal/module/orchestration/event_time.go:10-32` 定义 `withEventTime/resolveEventTime`。
  - `internal/module/orchestration/module.go:33-41` 与 `internal/module/orchestration/turn_lifecycle.go:20-25` 会把 turn 事件时间写入上下文。
...
- 未统一的内部生产侧：
  ...
  - `internal/module/orchestration/helpers.go:52-60` launch 准备阶段直接 `agent.updatedAt = time.Now()`。
  - `internal/module/orchestration/service.go:240-249`、`internal/module/orchestration/service.go:358-361` 进程启动/退出直接写 `now := time.Now()`。
...
- 正向变化：`internal/module/orchestration/service.go:261-274` 的 `fireOrForceLocked()` 现在只做严格 `FireCtx`，非法转换直接报错。
- 但仍存在状态机外直接写状态：
  - `internal/module/orchestration/helpers.go:52-60` 的 `prepareLaunchStateLocked()` 直接把状态写成 `provisioning`。
  - `internal/module/orchestration/turn_lifecycle.go:14`、`internal/module/orchestration/turn_lifecycle.go:29-59` 的 `forceIdleAfterCompletionError()` 直接把状态写回 `idle`。
- `internal/dto/agent/state.go:21-32` 的 trigger 定义里没有 `turn_completion_recovered`，但 `internal/module/orchestration/turn_lifecycle.go:14`、`internal/module/orchestration/turn_lifecycle.go:58` 仍发布这个未声明 trigger。
...
- 但 `internal/module/orchestration/module.go:33-41` 只订阅 `TurnStarted` 和 `TurnCompleted`，没有消费 approval 事件。
- LSP 对 `internal/module/orchestration` 搜索 `TriggerUserInputRequested` / `TriggerUserInputResolved` 无命中；当前没有找到任何 fire 点。
- 判定：链路仍停在 approval 层，没有反向驱动 agent 状态机进入/退出 `awaiting_user_input`。

## 6. recover 路径：⚠️

- `internal/module/orchestration/recover.go:27-58` 有显式 recover；流程是 `publishAgentRecovering("manual") -> stopProcess -> 清空 activeTurnID -> fire recover_requested -> startProcessLocked`。
- `internal/module/orchestration/recover.go:16-25` 与 `internal/module/orchestration/runner_actor.go:68-77` 仍保留 stall detector 自动 recover。
...
- 但 `internal/module/orchestration/recover.go:47-53` 明确会清空 `activeTurnID` 并直接重启；LSP 对 `internal/module/orchestration` 搜索 `replay` 无命中。
