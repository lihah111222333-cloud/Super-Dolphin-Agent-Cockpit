...
| `GetAgentDetail` | `orchestration.Service.Snapshot` + `orchestration.Service.GetReport` | 读取 agent 快照和最后报告；`TurnHistory` 目前固定空切片 | `internal/module/dashboard/service.go:68-90`、`internal/module/orchestration/contract.go:11-29` |
...
- `orchestration.AgentSnapshot` 的 tag 也是 snake_case：`parent_id`、`thread_id`、`last_report` 等。证据：`internal/module/orchestration/contract.go:52-62`
...
- `service.go` 依赖 `internal/module/orchestration`、`internal/store/ailog`、`internal/store/systemlog`，没有 `provider/`。证据：`internal/module/dashboard/service.go:3-14`
- `types.go` 依赖 `internal/module/orchestration`，没有 `provider/`。证据：`internal/module/dashboard/types.go:3-8`
