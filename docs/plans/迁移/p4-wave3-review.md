...
- `provider/unified` 做 driver registry 的方向可行。
  - 现仓已有 `fx` value-group 使用先例：`internal/app/runner.go` 用 `group:"runners"` 收集 runner，`internal/module/orchestration/module.go` 用 `fx.Annotate(..., fx.ResultTags(\`group:"runners"\`))` 输出 runner。
  - 因此可采用：
...
- Blocker：turn ID 相关职责未统一。
  - `module/orchestration` 在 provider 接受前就为 turn 分配 ID：`internal/module/orchestration/helpers.go:147-153`。
...
- tracker/stall 检测不能直接复用 orchestration 现有 `StallDetector`。
  - 当前 `internal/module/orchestration/recover.go` 的 `StallDetector` 只按 agent `updatedAt` 判定整进程卡死。
...
- I3 已落地：
  - `internal/module/orchestration/events.go` 已移除 turn 级事件发布。
  - `internal/module/orchestration/runner_actor.go` 已不再发布 `TurnStarted`。
  - `internal/module/orchestration/service.go` 已不再发布 `TurnCompleted`。
...
- I3：通过。`internal/module/orchestration` 下 `TurnStarted{` / `TurnCompleted{` 搜索结果为 0；`internal/provider/*/event_map.go` 仍保留 turn 级 typed event 发布。
...
- 其余关键文件：
  ...
  - `internal/module/orchestration/service.go`：308
