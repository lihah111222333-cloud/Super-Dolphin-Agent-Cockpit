...
- Warning。`agent` 进程崩溃不等于 `runner` 退出。当前 `orchestration.runnerActor` 在子进程 `Wait()` 返回后只是把结果送回 `handleProcessExit(...)` 并继续循环；真正的 runner 只在 `ctx.Done()` 时返回。P6 如果把“任一 runner 退出，例如进程崩溃”当成统一语义，会把 agent 级故障和 runtime 级故障混为一谈。证据：`cmd/mcp-orch/orchestration/runner_actor.go:26-45`，`cmd/mcp-orch/orchestration/service.go:342-381`。
