...
- `session-summary.md:48` 仍写“DAG 方法 TODO 骨架”。当前 `internal/module/orchestration/dag.go:14-79` 已实现 `CreateDAG/GetDAG/ListDAGs/UpdateNodeStatus`，`internal/module/orchestration/rpc.go:61-75` 已接入对应 handler，`orchestration/report` 也已改成 `GetReport` 读路径。
...
- 但 `p8-execution-plan.md:21-24` 的 V3 可复用资产行数已经不是当前精确值：
  - 文档写 `orchestration/rpc.go = 218`，当前末行是 `internal/module/orchestration/rpc.go:218`，即 217 行正文。
