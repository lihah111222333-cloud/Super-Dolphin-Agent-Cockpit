...
| `StopAgent` | 有，`app.go:173` | 走 `CallAPI("agent.stop")`；见 `internal/module/orchestration/rpc.go:40` | ⚠️ | 同名绑定删除，功能收敛到 RPC。 |
| `ListAgents` | 有，`app.go:188` | 走 `CallAPI("agent.list")`；见 `internal/module/orchestration/rpc.go:43` | ⚠️ | 同名绑定删除，功能收敛到 RPC。 |
| `SubmitInput` | 有，`app.go:149` | 走 `CallAPI("agent.submitPrompt")` 或 `CallAPI("agent.submit")`；见 `internal/module/orchestration/rpc.go:20`、`30` | ⚠️ | 同名绑定删除，功能收敛到 RPC。 |
