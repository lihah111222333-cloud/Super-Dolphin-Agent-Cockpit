...
- 核对范围覆盖：
  ...
  - 发布入口：`internal/module/orchestration/events.go`、`internal/provider/{unified,claudecli,codexapp}/event_map.go`、`internal/platform/rpc/approval_events.go`、`internal/module/workspace/{service.go,service_helpers.go}`
  - 订阅入口：`internal/module/orchestration/module.go`、`internal/platform/rpc/{module.go,push.go}`、`internal/ui/wails/{module.go,bridge.go}`。
...
1. `orchestration` 主动发布 agent 族事件  
   证据：`internal/module/orchestration/events.go:13-64`
...
2. `orchestration` 在 lifecycle 中订阅 `TurnStarted` / `TurnCompleted`  
   证据：`internal/module/orchestration/module.go:25-50`
...
- `orchestration`  
  `internal/module/orchestration/module.go:31-49`
