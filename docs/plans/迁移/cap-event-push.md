...
审查时间：2026-03-21  
审查范围：V3 `internal/platform/bus` / `internal/platform/rpc` / `internal/ui/wails` / `internal/module/orchestration` / `internal/provider/*`，以及 V2 `go-agent-v2` 对照实现。  
...
| 事件 | 发布点 | 订阅点 | 判定 |
| --- | --- | --- | --- |
| `agentdto.StateChanged` | orchestration `internal/module/orchestration/events.go:13-23`；codex translator `internal/provider/codexapp/event_map.go:39-74` | `LogSink` `internal/platform/bus/sink.go:43-49`；jrpc2 push `internal/platform/rpc/push.go:81-90`；Wails bridge `internal/ui/wails/bridge.go:53-63` | 有业务闭环，但存在双源 |
| `turndto.TurnStarted` | claude translator `internal/provider/claudecli/event_map.go:55-85`；codex translator `internal/provider/codexapp/event_map.go:76-112` | `LogSink` `internal/platform/bus/sink.go:51-59`；orchestration turn lifecycle `internal/module/orchestration/module.go:25-53`；jrpc2 push；Wails bridge | 有业务闭环 |
...
| `agentdto.AgentLaunched` | orchestration `internal/module/orchestration/events.go:25-33`；claude/codex translator | 仅 `LogSink` | 软孤儿 |
| `agentdto.AgentStopped` | orchestration `internal/module/orchestration/events.go:35-43`；claude/codex translator | 仅 `LogSink` | 软孤儿 |
| `agentdto.AgentRecovering` | orchestration `internal/module/orchestration/events.go:45-53`；codex translator | 仅 `LogSink` | 软孤儿，且双源 |
| `agentdto.AgentFailed` | orchestration `internal/module/orchestration/events.go:55-64`；claude/codex translator | 仅 `LogSink` | 软孤儿 |
...
- launch 成功时，orchestration 先 `fireOrForceLocked(...TriggerLaunchSucceeded)` 产出 `StateChanged(idle)`，再 `publishAgentLaunched()`，见 `internal/module/orchestration/service.go:255-263`
...
- orchestration turn lifecycle 订阅，见 `internal/module/orchestration/module.go:33-44`
...
见 `internal/module/orchestration/events.go:13-64`。
...
- orchestration 的 `SessionID` 用本地 `launchSeq`，见 `internal/module/orchestration/events.go:66-88`
...
- orchestration 状态机每次成功 transition 都发 `StateChanged`，见 `internal/module/orchestration/service.go:281-289`
...
- 所有成功的状态迁移都经 `fireAndPublishLocked()`，见 `internal/module/orchestration/service.go:281-289`
...
- orchestration 只是订阅这些 turn 事件来反推状态机，见 `internal/module/orchestration/module.go:25-53`
...
- 本地 `turn_queued -> turn_starting` 发生时，只会发 `StateChanged`，不会发 `TurnStarted`，见 `internal/module/orchestration/service.go:291-324`
- `finishTurnStartSuccess()` 把 `turn_starting -> turn_running` 时，同样只是状态迁移，不产出 `TurnStarted`，见 `internal/module/orchestration/helpers.go:153-174`
- `finishTurnStartFailure()` / `reconcileReadyStateLocked()` 能把 turn 状态收回 `idle`，也只会产出 `StateChanged`，见 `internal/module/orchestration/helpers.go:119-138`、`176-192`

结论：**当前只保证“状态机会发 `StateChanged`”；并不保证“状态转移后必有对应 `TurnStarted` / `TurnCompleted`”。**

### 9.4 sideband agent 事件也不是状态机产物

- `publishAgentRecovering()` 在真正 `TriggerRecoverRequested` 之前先发，见 `internal/module/orchestration/recover.go:27-40`
- `publishAgentStopped()` 在 `StopAgent()` 返回路径里手工发，见 `internal/module/orchestration/service.go:127-141`
- `publishAgentFailed()` 在 `recordProcessExitError()` 里手工发，然后才做 `process_exited` transition，见 `internal/module/orchestration/service.go:372-394`
