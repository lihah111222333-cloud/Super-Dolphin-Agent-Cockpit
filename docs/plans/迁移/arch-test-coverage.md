...
| `internal/module/orchestration` | 14 | `execution_test.go`, `submission_test.go`, `turn_lifecycle_test.go` |
...
- 当前覆盖较好的核心包主要集中在 `internal/module/orchestration`, `internal/module/skill`, `internal/module/turn`, `internal/platform/bus`, `internal/provider/unified`。
...
| --- | ---: | ---: | ---: | --- |
| `internal/module/orchestration` | 14 | 3 | 12 | 覆盖面在模块层里最好，已覆盖队列、事件、提交参数、turn 生命周期。 |
...
- V3 当前与 schema 有关的测试只看到两类“字段透传”覆盖：
  ...
  - `internal/module/orchestration/submission_test.go` 断言 `output_schema` / `outputSchema` JSON 入参能被反序列化进 `submitParams`。
