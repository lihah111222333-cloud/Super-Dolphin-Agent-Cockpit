...
高优先级问题：
1. `SessionManager.Register` 与 `Remove` 只有 map 级互斥，没有 session 代际保护；并发 create/remove 时，`Remove` 可能删掉并关闭刚注册的新 session。证据：`internal/provider/unified/client.go:47-65`、`internal/provider/unified/session.go:31-85`、`internal/module/orchestration/service.go:104-108`、`internal/module/orchestration/service.go:355-369`。
...
| 位置 | 受保护状态 | 结论 |
| --- | --- | --- |
| `internal/module/orchestration/submission.go:9-12` | `SubmissionQueue.items` | 所有 `Enqueue/Dequeue/Peek/Len/Clear` 都走 `q.mu`，队列本身安全。 |
...
| 位置 | 受保护状态 | 结论 |
| --- | --- | --- |
| `internal/module/orchestration/service.go:29-39` | `agents` 运行态总表 | `Launch/Stop/List/Snapshot/CompleteTurn` 都经 `s.mu`；map 访问安全。 |
...
| `service.agents` | `internal/module/orchestration/service.go:37` | `s.mu` | 通过 |
...
| 各类函数内局部 map | 如 `internal/module/skill/skills_match.go:117`、`internal/module/orchestration/helpers.go:17` | goroutine 局部对象，不形成共享状态 | 不构成并发问题 |
...
- 删除入口：`orchestration.service.removeSession -> SessionManager.Remove`，见 `internal/module/orchestration/service.go:104-108`。
...
| `internal/module/orchestration/runner_actor.go:48-60` | `cmd.Wait()` 结束；actor 退出时 `StopAllAgents()` 杀进程 | 通过 |
...
| `results chan waitResult` | `internal/module/orchestration/runner_actor.go:31-45` | actor 生命周期内常驻，不要求关闭 | 通过 |
...
- orchestration 在 `StopAgent`、`StopAllAgents`、`handleProcessExit` 路径都会调用 `removeSession`，见 `internal/module/orchestration/service.go:127-153`、`internal/module/orchestration/service.go:355-369`。
...
| 锁顺序 | 位置 | 结论 |
| --- | --- | --- |
| `service.mu -> queue.mu` | `internal/module/orchestration/service.go:166-187`、`internal/module/orchestration/service.go:291-324`、`internal/module/orchestration/helpers.go:119-138` | 只发现这一种方向，未发现 `queue.mu -> service.mu` 反向路径 |
