...
- `agent.list` -> 没有 `ListAgents`。当前只有 `internal/module/orchestration/helpers.go:123-137` 的私有 `listAgents() []agentRuntime`，RPC 不能把它当稳定 contract。
...
- `task/dag/*` 完全没有对应方法。`internal/module/orchestration/service.go` 的 `service` 结构当前只有 logger、event bus、session cleaner、state machine、agents map，没有 `taskdag.Store` 或其他 DAG 依赖。
