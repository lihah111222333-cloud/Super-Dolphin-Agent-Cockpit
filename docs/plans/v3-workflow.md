...
- 任务描述：建立 `internal/platform/statemachine` 工厂和 `internal/module/orchestration` 中的 transition table。
- 输入：Day 1 transition spec、`stateless` 契约、P0 模块骨架。
- 输出：`internal/platform/statemachine/*`、`internal/module/orchestration/transitions.go`。
...
- 输出：`internal/module/orchestration/actions.go`、`recovery.go`、动作依赖接口。
...
- 输出：`internal/module/orchestration/provider_bridge.go`、统一 provider registry。
...
go test ./internal/module/orchestration/... -count=1
```
...
```bash
go test ./internal/module/orchestration/... -run Matrix -count=1
go test ./internal/module/orchestration/... -run Recover -count=1
go test ./internal/module/orchestration/... -race -count=1
```
