...
> 状态：**骨架阶段已封板✅**；**T 阶段快车道 done🟡**（T0.4/5/6/8 + T1.1 + T2.1+T2.2 + T4.1+T4.4 完成；剩余 task 入 PG 环境/前端方案阔）
> 关系：本文是 `docs/plans/迁移/p23/` 的上位重排，不是替换；p23 各 Phase 的去留在第 11 节给出映射表。
> 历史：2026-05-09 初稿；2026-05-10 合并 14 处骨架补丁、4 阶段路径、讨论决策；2026-05-10 二修（用户场景例）；2026-05-10 三修（2-pass DAG 审查）；**2026-05-10 骨架阶段封板：14 commit / 28 files / 3113 insertions / 67 单测全 PASS / ADR 0001 固化全部契约**
