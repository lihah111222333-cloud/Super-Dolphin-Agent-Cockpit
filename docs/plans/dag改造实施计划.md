...
| ~~**F7.3**~~ ✅ done（新增 / ADR-014 §2.3） | **prompt_template seed 库**：13 张通用 AI 微技能已落地（`morning_briefer` / `paper_summarizer` / `pr_summarizer` / `weekly_reviewer` / `data_inspector` / `email_drafter` / `health_reporter` / `topic_curator` / `source_monitor` / `note_organizer` / `todo_prioritizer` / `learning_card` / `trip_briefer`）。字段约束已守住：`variables='{}'::jsonb`、`prompt_text` 不写 `{{var}}` 替换符、tags 仅供 UI/admin 列表元数据、不写已 drop 的 `router_priority`。 | `migrations/0086_prompt_template_manually_edited.sql` + `migrations/0087_seed_prompt_template_skill_cards.sql` + `cmd/mcp-orch/store/prompt/*` + `cmd/mcp-orch/sql/queries/prompt_template.sql` + `cmd/mcp-orch/store/sqlc/prompt_template.sql.go` + `cmd/mcp-orch/tools/prompt_tools.go` + `internal/archtest/dag_skill_seeds_test.go` / commits `2a7abbbf` `ac5bbb20` | 单测/archtest：`PromptTemplate` mapping 覆盖 `ManuallyEdited`；0086 DDL 守卫；0087 required key / guarded DO UPDATE / 禁 dead routing 与模板占位符守卫。验证：`go test ./cmd/mcp-orch/store/prompt ./cmd/mcp-orch/tools ./internal/archtest -count=1` PASS。人工改 prompt 后重跑 0087 不覆盖由 SQL `WHERE public.prompt_templates.manually_edited = FALSE` 保证。 | T4.2 prompt_list ✅ / F1.1 ✅ / F1.2 ✅ / **F1.3-rework via C-A A2** ✅ / F7.1 ✅ | M | Y |
