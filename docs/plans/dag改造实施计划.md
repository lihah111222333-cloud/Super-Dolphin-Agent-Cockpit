...
**2026-05-14 final output MVP 结果**：backend dogfood 后的产品入口问题先做最小闭环。run 完成后的最终产物可以继续存在于 Shared Files 中；后端通过 `task_dag_runs.metadata.final_output` 暴露“哪个 sharedfile/text/json 是本次可收最终产物”的结构化指针。Shared Files 页面后续应基于该指针高亮最终产物，默认折叠 working/debug 中间文件。
...
| H14 | **final_output 前端入口**：run/task 详情页展示 `task_dag_runs.metadata.final_output`；Shared Files 页面默认折叠 working/debug 中间产物，并高亮/筛选被 final_output 引用的文件；文件型 final_output 直连 `shared_file_read` / 下载入口，小 JSON/text 直接展示摘要 | 用户反馈“产物在哪找”或进入 UI/T5-T8 前端闭环 | 中 |
