...
| F 功能 | 38 | 🟡 **19 done**（+ F1.5 / F4.1 / F6.3 于 2026-05-11；+ F1.2 / F2.2 / F4.2 / F7.1 于 2026-05-12 第二轮；+ F4.5 于 2026-05-12 round-3 wiring batch；+ F7.2 于 2026-05-12 第四轮 worktree 并行 + 互审；+ F14.1 于 2026-05-12 第五轮 codex 实装 + claude 互审；**~~F1.3~~ 于 2026-05-12 从 ✅ done 降为 🟡 部分完成；ADR-015 v1-v3 + ADR-016 五轮迭代被 4 轮实证证伪后删除，新方向走 C-A 实施计划，详 `docs/plans/dag-lifecycle-c-a-implementation.md` + `docs/design/F1-lifecycle-audit-2026-05-12.md` §8**） / 1 完成占位 F6.1 / 14 未开工（含 F1.3-rework + F1-followup-3 合并 ticket）/ 4 降 H（F3.1-3.4，详 ADR-014） / 1 新增 F7.3 prompt_template seed 库。另：dispatcher wiring 全链接通 + size_cap 4KB enforce + events ring trim N=50 + Tarjan SCC + task_dispatch_node MCP tool + schema sanity gate（不在 F 表位，但在 round-3 batch 中携带落地） | ~50% | 行为兑现：cron 真跑、AI 设计师上岗（含 seed 库菜单）、智能重试落地 |
...
> **口径说明**：38 表位 ÷ 状态 = 19 条 strikethrough ✅ done（F1.1 / F1.2 / F1.5 / F2.0 / F2.1 / F2.2 / F4.0 / F4.1 / F4.2 / F4.5 / F5.1 / F5.2 / F5.3 / F6.2 / F6.3 / F6.4 / F7.1 / **F7.2** / **F14.1**）+ 1 条 F6.1 完成占位（由 T1.2-mid 接手 snapshot）+ 4 条 ⏸ 降 H（F3.1 / F3.2 / F3.3 / F3.4，详 ADR-014：prompt_template-first 路线下 hybrid 节点可由 agent+depends_on 两节点表达）+ 1 条新增 F7.3（prompt_template seed 库，详 ADR-014 §2.3）+ 13 条未开工。
>
...
| 🟡 **F1.3**（部分完成 / 待 rework） | `AgentExecutor` 处理 `outputs`：写 sharedfile / node.result。**2026-05-12 降级**：单测漏断言「outputs 写的是 thread 真产出」（实际写的是 launch 元数据 `{thread_id, agent_key}`），与 F7.3 依赖列「F1.3 是下游拿数据红线」冲突 + 与 M3 验收硬阈值（≥ 10 节点 multi-agent-node DAG 跑通）冲突 — 详 `docs/design/F1-lifecycle-audit-2026-05-12.md` §1.3。**rework 与 F1-followup-3 合并，按 C-A 路径分阶段落地（先 provider 层补完 TurnCompleted.Result + spawned agent 自动 stop，再 DAG 层 subscriber，最后 A2 物化真实 outputs），详 `docs/plans/dag-lifecycle-c-a-implementation.md` + ADR-018（5 个 ADR + 跨 4 层 + 情况 A ~3300-4080 行 / 18-23 commit；情况 B ~3530-4310 行 / 20-25 commit，v2.6 ADR-018 初稿同步后值）**。**边界**：仅负责本节点输出落地，**不得调外部 webhook / 命令卡**——那属于 F3.2（hybrid agent→automation）路径，详 ADR-011 §4 Q3。**注**：F3.2 已 ⏸ 降 H（详 ADR-014），本期 agent → 外部能力整体不做，但边界规则不变（agent 节点 outputs 仍然不允许声明 webhook/command_ref） | `cmd/mcp-orch/orchestration/nodeexec/executor_agent.go` + `executor_agent_outputs_test.go` + `executor_automation.go`（抽公共 `validateOutputsForbiddenKeys`，automation 行为守恒）/ commits `ae35c0a2` `f985e83d` + merge `b0fcf77b`（2026-05-12 第四轮 worktree 并行 + 互审）。**老 commit 保留不 revert**：C-A A2 是**增量改造**（launch 成功不再写 metadata outputs；subscriber 基于 `ev.Result` + `config.outputs` 写真实输出；复用 `CompleteNodeAndScheduleDownstream` result 更新，不新增 SQL/sqlc），不是回滚 — 详 `docs/plans/dag-lifecycle-c-a-implementation.md` §3.2 / ADR-018| 单测：launch 成功不写 metadata outputs + 默认 node_result 真输出 + sharedfile 真输出 + node_result/sharedfile 同时配置 + webhook_url/command_ref 负例 + 4KB size_cap validation + SharedFileWriter 未注入（infra 类）+ IO 写失败（infra 类，避免 F1.4 dispatcher 误判）| F1.2 ✅ | S | N |
...
| **F7.3**（新增 / ADR-014 §2.3） | **prompt_template seed 库**：10-15 张通用 AI 微技能（覆盖 power user 跨场景 — `morning_briefer` / `paper_summarizer` / `pr_summarizer` / `weekly_reviewer` / `data_inspector` / `email_drafter` / `health_reporter` / `topic_curator` / `source_monitor` / `note_organizer` / `todo_prioritizer` / `learning_card` / `trip_briefer` ...）。**字段使用约束**：(a) `variables` jsonb 未被 AgentExecutor 消费，**`prompt_text` 不要写 `{{var}}` 替换符**，参数化走 cfg.first_turn 覆盖；(b) `tags` 在 0044_drop_router_priority 后无路由作用（走 explicit agent_key 分发）仍可写供 UI/admin 列表使用，不依赖 tag 触发；(c) `router_priority` 列已被 0044 drop，**不要写以免警告**。 | **前置两个 migration**：`migrations/0086_prompt_template_manually_edited.sql`（加 `manually_edited` BOOLEAN 列，点 docs/migrations/prompt-seed-policy.md §2.1 落地；**2026-05-12 编号让位**：原计划 0085，F7.2 已占 0085，F7.3 后移到 0086/0087）+ `migrations/0087_seed_prompt_template_skill_cards.sql`（seed 10-15 张，走 DO UPDATE + WHERE manually_edited=FALSE 模板）。**配套代码**：`store/prompt/store.go::UpsertPromptTemplate` 扩 manually_edited flag 接参。**测试**：`internal/archtest/dag_skill_seeds_test.go`守卫每张 seed 关键字段完整 + manually_edited=FALSE + tags 不含依赖 router 的描述。 | 集成测试：`prompt_list` 返 ≥ 10 张 + enabled=TRUE；AI designer thread 里能挑任意一张组 DAG 跑通；M3 demo `morning_briefer` round-trip；人工改 prompt 后（manually_edited=TRUE）重跑 0087 不覆盖 | **前置顺序**：0086 列 → UpsertPromptTemplate store 扩 → **F1.3-rework via C-A §3.2 A2 / ADR-018**（agent 节点真实输出写 `node.result` / 显式 sharedfile，是 seed 卡跑通后下游拿数据的红线；A2 不新增 SQL/sqlc、不做 automation 输出改造）→ 0087 seed。依赖：T4.2 prompt_list ✅ / F1.1 ✅ / F1.2 ✅ / **F1.3-rework via C-A A2**（必须先做）/ F7.1 ✅ | M | Y |
