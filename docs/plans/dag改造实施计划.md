...
> 2026-05-14 第十四轮 **final output 后端 MVP 落地**：commit `362be7f0` 将最终产物入口从 Shared Files 页面噪音中抽出来：DAG 模板用 `task_dags.metadata.final_node_key` 显式声明最终节点，run 成功终态时 `FinalizeTaskDagRunIfAllNodesTerminal` 把 final node 的 sharedfile/text/json 结果升格到 `task_dag_runs.metadata.final_output`；`task_get_run` 通过既有 `Run.Metadata` 暴露，不新增 migration/UI。sharedfile 仍是存储层和节点协作载体，不是主用户入口；UI/清理策略后续另立 H14/H15。2 个 reviewer + 1 个复审 agent 通过；验证：`go test ./cmd/mcp-orch/... -count=1`、关键 archtest、`git diff --check`、临时单 query `sqlc compile` PASS；全目录 `sqlc compile` 仍受既有 0083 schema list 缺 `spawning_thread_id` 阻塞。**F 阶段 done 计数仍 23**。
...
| F 功能 | 38 | 🟡 **23 done**（+ F1.5 / F4.1 / F6.3 于 2026-05-11；+ F1.2 / F2.2 / F4.2 / F7.1 于 2026-05-12 第二轮；+ F4.5 于 2026-05-12 round-3 wiring batch；+ F7.2 于 2026-05-12 第四轮 worktree 并行 + 互审；+ F14.1 于 2026-05-12 第五轮 codex 实装 + claude 互审；**F1.3 于 2026-05-13 经 C-A A2 / ADR-018 `3e70e468` + review-fix `02009e22` 重新升 ✅ done**；**F7.3 于 2026-05-13 `329d525d` + `d0de46ed` 升 ✅ done**；**F1.4 于 2026-05-13 `e64e4234` + review-fix `02009e22` 升 ✅ done**；**F15.1 于 2026-05-13 `e2d8aa6c` 升 ✅ done**） / 1 完成占位 F6.1 / 10 未开工 / 4 降 H（F3.1-3.4，详 ADR-014）。另：dispatcher wiring 全链接通 + size_cap 4KB enforce + events ring trim N=50 + Tarjan SCC + task_dispatch_node MCP tool + schema sanity gate（不在 F 表位，但在 round-3 batch 中携带落地）；**Phase 4 / M3 backend dogfood 已通过 `b9e8269d` / `ddf1b16f` / `019cf8a5`** | ~61% | 行为兑现：cron 真跑、AI 设计师上岗（含 seed 库菜单）、智能重试/dispatch 观测落地；backend dogfood 10 节点闭环已绿 |
...
## 3. 阶段 F 功能（38 行 / 10 未开工 + 23 ✅ done + 1 完成占位 + 4 降 H：详 ADR-014）

> **口径说明**：38 表位 ÷ 状态 = 23 条 strikethrough ✅ done（F1.1 / F1.2 / **F1.3** / **F1.4** / F1.5 / F2.0 / F2.1 / F2.2 / F4.0 / F4.1 / F4.2 / F4.5 / F5.1 / F5.2 / F5.3 / F6.2 / F6.3 / F6.4 / F7.1 / **F7.2** / **F7.3** / **F14.1** / **F15.1**）+ 1 条 F6.1 完成占位（由 T1.2-mid 接手 snapshot）+ 4 条 ⏸ 降 H（F3.1 / F3.2 / F3.3 / F3.4，详 ADR-014：prompt_template-first 路线下 hybrid 节点可由 agent+depends_on 两节点表达）+ 10 条未开工。
>
...
| ~~**F15.1**~~（H6 前置部分）✅ done | dispatch / retry metric：`dispatch_failed_total` / `retry_count_per_node` 计数器 + 节点重试 ≥ 3 次告警 webhook。H6a 已前置到 F 阶段，避免「上层调度看不见下层执行」瞎区到 M3 后才补；未提前实现 H6b / F12.1。详 ADR-010 | `pkg/dagmetrics` + `internal/platform/metrics/dag.go` + `cmd/mcp-orch/orchestration/dispatch_agent_running_metric.go` + `wakeup_dispatcher.go` + `cmd/mcp-orch/notify/dispatch_retry_alert.go` + `cmd/mcp-orch/fx.go` / commit `e2d8aa6c` | 单测/集成：dispatcher 5 节点 batch mark-sent 跑通 + node-3 retry=3 alert；max-attempt final fail 仍 alert；真实 TLS httptest webhook capture；`/metrics` 读取 `dispatch_failed_total` / `retry_count_per_node`；per-node series cap + overflow counter | F1.4 ✅ / F6.3 ✅ | M | Y |
...
**2026-05-14 final output MVP 结果**：backend dogfood 后的产品入口问题先做最小闭环。run 完成后的最终产物不再要求用户去 Shared Files 海量中间产物里找；后端通过 `task_dag_runs.metadata.final_output` 暴露“可收最终产物”的结构化指针。Shared Files 页面后续只应突出被 `final_output` 引用的文件，默认折叠 working/debug 中间文件。
...
| H14 | **final_output 前端入口**：run/task 详情页优先展示 `task_dag_runs.metadata.final_output`；Shared Files 页面默认折叠 working/debug 中间产物，只突出被 final_output 引用的文件；文件型 final_output 直连 `shared_file_read` / 下载入口，小 JSON/text 直接展示摘要 | 用户反馈“产物在哪找”或进入 UI/T5-T8 前端闭环 | 中 |
| H15 | **sharedfile retention / cleanup**：按 run 状态、引用关系和 TTL 清理未被 `final_output` 引用的中间 sharedfiles；final_output 引用文件按用户可收产物保留或长 TTL，删除前需要 UI 可见确认/导出策略 | sharedfile 存储增长明显或进入定时任务产品化 | 中 |
