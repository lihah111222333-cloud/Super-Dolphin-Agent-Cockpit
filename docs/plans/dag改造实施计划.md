...
| **F7.3**（新增 / ADR-014 §2.3） | **prompt_template seed 库**：10-15 张通用 AI 微技能（覆盖 power user 跨场景 — `morning_briefer` / `paper_summarizer` / `pr_summarizer` / `weekly_reviewer` / `data_inspector` / `email_drafter` / `health_reporter` / `topic_curator` / `source_monitor` / `note_organizer` / `todo_prioritizer` / `learning_card` / `trip_briefer` ...）。**字段使用约束**：(a) `variables` jsonb 未被 AgentExecutor 消费，**`prompt_text` 不要写 `{{var}}` 替换符**，参数化走 cfg.first_turn 覆盖；(b) `tags` 在 0044_drop_router_priority 后无路由作用（走 explicit agent_key 分发）仍可写供 UI/admin 列表使用，不依赖 tag 触发；(c) `router_priority` 列已被 0044 drop，**不要写以免警告**。 | **前置两个 migration**：`migrations/0085_prompt_template_manually_edited.sql`（加 `manually_edited` BOOLEAN 列，点 docs/migrations/prompt-seed-policy.md §2.1 落地）+ `migrations/0086_seed_prompt_template_skill_cards.sql`（seed 10-15 张，走 DO UPDATE + WHERE manually_edited=FALSE 模板）。**配套代码**：`store/prompt/store.go::UpsertPromptTemplate` 扩 manually_edited flag 接参。**测试**：`internal/archtest/dag_skill_seeds_test.go`守卫每张 seed 关键字段完整 + manually_edited=FALSE + tags 不含依赖 router 的描述。 | 集成测试：`prompt_list` 返 ≥ 10 张 + enabled=TRUE；AI designer thread 里能挑任意一张组 DAG 跑通；M3 demo `morning_briefer` round-trip；人工改 prompt 后（manually_edited=TRUE）重跑 0086 不覆盖 | **前置顺序**：0085 列 → UpsertPromptTemplate store 扩 → **F1.3 AgentExecutor outputs**（agent 节点 result 落 sharedfile，是 seed 卡跑通后下游拿数据的红线）→ 0086 seed。依赖：T4.2 prompt_list ✅ / F1.1 ✅ / F1.2 ✅ / **F1.3**（必须先做）/ F7.1 ✅ | M | Y |
