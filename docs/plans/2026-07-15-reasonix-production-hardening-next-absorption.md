...
> 吸收决策状态：`absorption_decision_complete=true`
...
> 当前返修状态：`ninth_review_findings=applied_pending_independent_recheck`
...
`p0_executable=false` 不是否定 P0，而是明确当前仍有四个执行前阻断：

- 本次 docs-only 会话没有暴露仓库要求的 `mcp__lsp.*` 工具，尚未形成 locate、inspect、xref、read 和 diagnostics 五类新鲜证据。
- 当前 v3 checkout 是用户脏分支，不能用它作为生产实现 worktree；执行必须从已复核的 `origin/main` 建立隔离 worktree，并重新确认远端 SHA。
- 第九次两个新 Reviewer 已对第八次8项写回形成的832行对象做独立审查，因此第八次pending recheck不再单列；两个 Reviewer 原始报告共17个P1，Root合并重复根因并补充一个直接状态机矛盾后接受15个P1。该返修前对象SHA-256为`c425bb5ccd45a663c9a78e7496c8c63a613cff7125f9c4404964cebc8d976965`，本次写回后的新对象仍须独立复核。
- 最终产物实物清单、内嵌依赖component graph、含release-unit节点的无环release digest/signing DAG、可信attribution与CLI-matrix signer、不可替换的第三方CLI launch chain/process identity、判别式capability evidence、最早provider ingress gate及`ValidatedProviderEvent`成功链、代际绑定MCP readiness、provider/peer runtime authority分离、typed Claude MCP状态、host-owned drift routing、精确call admission和逐producer字段链目前仍只是计划契约；Task 0尚未以LSP、RED、真实CLI/package evidence冻结到实现边界，故`implementation_design_complete`与`p0_executable`都必须保持false。

上述 blocker 全部闭合，并由 Task 0 把本文已固定的 v3-owned/third-party product IP boundary、`ObservedArtifactInventory`、release-member primary/embedded component graph、含release-unit节点的`ReleaseDigestGraph`、`ReleaseAttributionBundleDescriptor`/`ReleaseAttributionTrustPolicy`、`ResolvedProviderExecutable`/`ProviderExecutableLaunchPlan`/`PreparedProviderExec`/`ProviderProcessIdentity`、`ProviderExecutionComponentPolicy`、判别式provider capability evidence与`ProviderCLICompatibilityMatrix`/`ProviderCLICompatibilityTrustPolicy`、`ProviderExecutableAttestation`/`ExecutionLayerFailure`脱敏投影、最早`ProviderIngressAuthority`/`ProviderIngressEnvelope`/`ProviderProtocolGate`/`ValidatedProviderEvent`/`ProviderProtocolDrift`、generation-bound `MCPReadinessAttestation`、`ProviderMCPCapabilityStatus`、provider observation/peer report/`MCPPeerRuntimeAuthority`分离、bundled-only release compatibility preflight、package-owned update trust keyring + package signer policy、transaction-bound trust generation、签名 update-source 引导、probation/普通启动失败分支、normal re-exec environment、pre-healthy write-set transaction、supervisor 有界状态机、Recovery-only backend graph、config-generation-bound MCP trusted carrier、workspace aggregate snapshot、精确`AdmissionGrant`、prepared takeover、authenticated RPC principal/workspace handshake、Codex-only quarantine scope 和逐producer跨 P0 字段守卫实证到 exact landing files 后，才允许同时切换为 `implementation_design_complete=true` 和 `p0_executable=true`。任何 Agent 不得跳过当前对象复核或 Task 0 直接修改生产代码。
...
11. pending update、rollback、commit 与 `TrustGenerationState{committed_generation,pending_generation,transaction_id}` 必须跨进程序列化。candidate N+1 只写 transaction-scoped pending trust；probation 期间禁止用 pending trust开放新的 Check/Install authority。rollback 丢弃 pending并恢复 committed N；只有 healthy 证据完整后，才按冻结的 file fsync -> release commit marker -> pending-to-committed trust promotion -> backup deletion 顺序收敛。锁失败、事务损坏、hash mismatch、trust floor冲突或补偿失败都 fail-closed，不能把 rename/ACK 单独当作 durable。
...
| Path | Owner/职责 | 约束 |
| --- | --- | --- |
| `internal/platform/toolbridge/schema_compile.go` | canonicalize、resource limits、compile、stable error classification | 纯 schema contract，不决定 lifecycle |
...
| `internal/contract/mcp_control.go` | opaque identity/resolved manifest、`MCPManifestAuthoritySnapshot`、current authority read/CAS port、classifier 和 managed registry | owner generation/digest/workspace/agent/thread/session binding不可序列化；config删除/禁用/换代撤销旧 carrier |
...
| `internal/contract/mcp_control.go`、v3-owned peer/client registry、refresh coordinator、app authority adapter 与 tests | 基于current committed member+exact peer/client/process lease私有签发不可序列化`MCPPeerRuntimeAuthority` | provider readiness不能转换；Claude direct路径无authority；peer/client/process/lease/fence换代立即revoked |
...
| `internal/contract/toolbridge.go`、`mcp_control.go`、app RPC、rpc server/WS/control/module、`internal/ui/wails/http_server.go`/binding/window lifecycle 与 tests | `AuthenticatedRPCConnection` proof、`RPCSessionPrincipal`、workspace aggregate resolve/token和 committed projection | Wails guard/control register后才签发；raw `/ws`/generic Dispatch失败；冻结 window或app owner revoke语义 |
...
- [ ] 从final signed/notarized/stapled DMG/ZIP/installer独立解包生成subject-external `ObservedArtifactInventory`，与`ReleaseUnitDescriptor`做双向等集；从Go build info、`go.mod/go.sum`、npm lock/bundle metadata、native scan和bundle manifest生成`ReleaseComponentGraph`，与registry/SBOM node及relationship做双向差集。冻结具名排除项的schema version、direction、owner、reason与fixture；不得用package声明或手写required list生成observed truth。
...
- [ ] 建立第三方CLI兼容矩阵RED：Codex bundled/PATH/managed的compatible、too-old、too-new、unknown、protocol drift和silent-latest/source-switch；Claude missing、out-of-range、auth expired、vendor outage、pre-turn evidence missing与critical event schema drift。每个evidence variant严格校验必填/禁填字段；matrix绑定provider/component/source/OS/arch/launcher class/version/protocol/schema/capability/trust-policy exact subject，并只接受package-anchored `provider_cli_matrix` usage授权的algorithm/key material/generation/validity/revocation；payload self-declared、wrong/expired/revoked/wrong-usage signer和跨subject重放均RED。禁止用`--help`、硬编码capability或post-first-message `system:init`冒充兼容；managed install必须显式同意并固定不可变上游版本。
...
- [ ] 用状态图和 RED 固定 macOS 双触发 supervisor：detached updater 监督首次 probation 并在硬崩溃后即时 exact-transaction rollback；updater/主机中断时，下次 `agent-terminal` preflight 只负责启动 detached Guard worker 并退出，worker restore/parity/reopen。不得把“下次用户启动才恢复”误写成首次 crash 的即时自动回滚。
...
- [ ] 用 fail-first archtest 证明新增 `cmd/super-dolphin-guard` 未登记时会失败，再最小扩展 canonical registry；不得 broad allowlist。
...
- [ ] 按 §4.3 为全部 P0 producer 分别建立字段守卫：动态枚举 producer/registry，验证 mapper/SQL/RPC/frontend/schema consumer 的 missing/stale/roundtrip/deep-copy；每条链至少一次真实 mapper/consumer mutation RED。禁止只守 opaque MCP identity 就声称 D17 全覆盖；无效 exemption 必须阻断并接入对应 versioned gate。
...
- [ ] 先写RED：任一third-party member缺`origin`、`vendor`、`license_expression`、`license_text_hash`、`notice_paths`、`distribution_mode`或`protocol_compatibility`时，release manifest、package、NOTICE/SBOM和About共同fail-first；package signer、v3签名或复制进bundle都不得改写ownership。
...
- [ ] `service.Install` 在 helper 启动前 durable prepare；helper 不得自行猜测 from/to version 或 release-unit 成员。
...
- [ ] 对本文最新SHA重新做至少两条独立对抗审查，逐项确认final artifact inventory、embedded dependency/SBOM graph、含release-unit节点的无环digest DAG与trusted attribution、prepared launch chain/process identity、strict capability union/matrix及package-anchored matrix trust、bundled-only release preflight、最早provider ingress gate和`ValidatedProviderEvent`成功链、generation-bound Claude MCP readiness/typed UI状态、provider-peer authority分离、host-bound drift、精确single-use `AdmissionGrant`、failure attribution/redaction、package trust/trust generation、normal re-exec/pre-healthy writer、authority freshness、aggregate snapshot、prepared takeover、authenticated principal和§4.3全部独立producer字段链已落到不变量/owner/RED/E2E；旧`c425bb5c...`第九轮findings对象不能冒充新对象PASS。
...
- 审查对象依次绑定初稿、`f6b7...`、`19e79...`、`750680...`、`42e4cc5b...`、`29924f34...`、第六轮658行`c02d8d09...`、第七轮707行`2c704f52...`、第八轮785行`87ad8f02...`和第九轮832行`c425bb5c...`；第九轮两个Reviewer原始共返回17个P1，Root去重并补充直接矛盾后接受15个P1。15项写回后的当前对象尚未新独立recheck，只能标记findings applied，不能标记review PASS。
...
- 产品声明把第三方CLI/依赖写成v3自研或全自有IP，或以v3打包、签名、安装行为改变上游ownership；任一第三方release member缺origin/vendor/version/digest/signature/license/NOTICE/SBOM/protocol/distribution字段。
...
吸收决策文档完成条件（对应 `absorption_decision_complete=true`）：

- 初稿三个独立 Reviewer 都返回 coverage、finding 和 residual risk，且绑定初稿 review object。
- Root agent 已逐条裁决初稿和九轮返修 finding；第三轮两个对抗 Reviewer 绑定 `750680...`，第四轮 Root 对抗审查绑定 `42e4cc5b...`，第五轮两个对抗 Reviewer 绑定 617 行对象 `29924f34...` 并返回 6 个 P1，第六轮两个新 Reviewer 绑定 658 行对象 `c02d8d09...` 并返回 9 个 P1，第七轮两个新 Reviewer 绑定 707 行对象 `2c704f52...`，原始返回4/5个P1、Root去重为6个P1；第八轮两个新Reviewer绑定785行对象`87ad8f02...`、各返回4个P1，Root接受8个P1；第九轮两个新Reviewer绑定832行对象`c425bb5c...`、原始共17个P1，Root合并重复根因并补充直接状态机矛盾后接受15个P1，均已写回当前计划；不把任一旧hash的结论冒充当前对象PASS。
