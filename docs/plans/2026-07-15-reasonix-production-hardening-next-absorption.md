...
`p0_executable=false` 不是否定 P0。Task 0 已在 `origin/main@b40867229af8e17916c00393639ccb0fcb4bf6fc` 建立隔离执行 worktree 并取得 locate、inspect、xref、read、diagnostics 五类新鲜 LSP 证据；Task 0-D0 已清零两个既有 owner 文件的 12 条 LSP diagnostics，并通过聚焦测试、完整影响包测试和仓库门禁。Task 0-Design 已把 release、provider、update、MCP authority、字段守卫和 compiler 冻结到 exact owner/path/schema/test/fixture/budget，设计 payload 已提交为 immutable SHA；当前只剩一类执行前阻断：

- 历史九轮 findings 已写回上一提交基线，但任何旧 hash 都不能证明当前对象。复核调用方必须以 `git rev-parse HEAD` 解析并向两条 fresh reviewer lane 传入同一个 exact path/line/bytes/SHA；reviewer 输出绑定该 SHA。不得为把当前 SHA 写回当前提交而修改被复核对象，否则会形成新的未复核对象。当前对象是否通过只由这两份外部证据判定：P0/P1 必须为零；不影响正确性的 P2/P3 记录后续 tranche 后不阻断。
