...
**现有订阅**：`cmd/mcp-orch/orchestration/service.go:269-274` 通过 `bus.ResilientSubscribe` 订阅，但**只用来推进 agent 状态机**（`handleTurnCompletedEventWithCtx` → `svc.CompleteTurn`），**不推进 DAG 节点状态**。
...
**链路**：
1. 新加 `cmd/mcp-orch/orchestration/dag_turn_completed_subscriber.go`：订阅 `turndto.TurnCompleted`
...
2. ✅ `cmd/mcp-orch/orchestration/service.go:269` 已有 TurnCompleted 订阅基础设施
