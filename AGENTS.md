...
- 用户点名技能时，本回合必须使用该技能。
- 仅当技能提供 super-agent-v3 专属路径、协议、守卫、架构边界、生成器或验证命令，且与当前文件类型、目录或变更面直接相关时，才自动路由到覆盖范围最小的匹配技能。
- 模型已经具备的通用能力默认内化执行，不因任务语义相似而自动加载同名技能。包括但不限于：思考与决策、信息提取与总结、头脑风暴、计划编写与执行、系统化调试、TDD、重构方法、任务拆分与子代理调度、接收或请求代码审查、完成前验证、开发分支收尾。此类技能仅在用户明确点名该技能，或任务明确要求其中的仓库专属模板/资产时加载。
- 自动路由仅限窄领域事实技能：`前端`、`后端`、`MCP协议`、`架构设计`、`日志与错误处理`、`测试规范`、`注释规范`、`全量项目地图生成`、`编写技能`、`trace-diagnosis`，以及满足显式依赖条件的 `TailwindCSS 样式规范`。即使包含少量项目事实，`代码审查维度`、`安全工程师规范`、`super-dolphin-workflow`、`refactoring-guardrails`、Git/worktree/分支流程和所有代理编排技能也不得自动加载。
