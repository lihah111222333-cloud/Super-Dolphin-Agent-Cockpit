...
- 用户点名技能时，本回合必须使用该技能。
- 自动加载仅限三个技能：Go 后端工作使用 `后端`，代码审查使用 `代码审查维度`，前端规范工作使用 `前端`。只有任务与对应变更面直接相关时，才自动加载其中覆盖范围最小的技能。
- 除上述三个技能外，其他所有技能都必须由用户明确点名后才能加载；不得因任务语义相似、技能包含项目事实、显式依赖条件或需要仓库专属模板/资产而自动加载。
