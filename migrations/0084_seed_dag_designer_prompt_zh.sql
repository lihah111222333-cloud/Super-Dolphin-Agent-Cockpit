...
--   创建 / 修改 DAG (task_create_dag / task_dag_apply_ops / task_update_node)、
...
- `task_dag_apply_ops(dag_key, base_version, ops)`：在已有 DAG 上批量增改。`base_version` 是从 task_get_dag 拿到的当前 version (OCC 乐观锁，写冲突会返回 ErrVersionConflict，须重读重试)。`ops` 数组每项含 `op` discriminator：
  ...
  - `{"op":"update_dag","patch":{"title":"...","description":"...","trigger":"manual|auto|scheduled","cron_expr":"0 8 * * *","owner_id":"..."}}` — 字段全可选，nil 表示不改。
- `task_update_node(dag_key, node_key, status, result?)`：改单节点运行态。**通常不该由设计师手动调**，留给 executor / 用户在 UI 上点。
...
每个节点的 `config` 是一段 JSON，按 `node_type` 分三种 schema：
...
2. **动态可重写约束**：DAG 处于 `running` 状态时，apply_ops 只能 `add_node`，并且新节点的 `depends_on` 必须全部指向已经 `done` 的节点 (F4.5)。`draft` / `ready` 状态可以自由增删改。
...
3. 列拓扑给用户：
   ```
   节点 1: test_run    (automation, command_ref=run_tests, 输出 → sharedfile reports/test_run.log)
