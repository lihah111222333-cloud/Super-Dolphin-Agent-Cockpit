...
1. 根据用户需求，用中文写：
   ...
   - `voice_text`：配音文案（**不超过15个字**，对应视频3-4秒时长，简短有力）

2. 调用 `video_with_audio` tool，传入 `prompt` 和 `voice_text`

3. 等待 tool 返回（约 10 分钟），将 `output_path` 返回给用户，提示尽快下载
...
```json
{
  ...
  "voice_text": "每一天，都是新的开始，遇见更好的自己"
}
```

## 禁止事项
- 禁止传 `voice` 参数，必须省略让工具使用默认值
- 禁止分开调用 video_generate、tts_generate、av_merge
- 禁止访问任何外部网站